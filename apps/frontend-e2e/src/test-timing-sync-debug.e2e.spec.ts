...
test.describe('Timing Sync Debug', () => {
  test('should log initial sync calculation', async ({ page }) => {
    ...
    page.on('console', (msg) => {
      const text = msg.text();
      if (
        text.includes('Initial sync calculated') ||
    ...
    const initialSyncLog = syncLogs.find((log) =>
      log.includes('Initial sync calculated'),
    );
    ...
    if (initialSyncLog) {
      ...
      if (offsetMatch) {
        ...
        console.log(
          `Calculated offset: ${offset}s (${(offset * 1000).toFixed(0)}ms)`,
        );
