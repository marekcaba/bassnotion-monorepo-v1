...
test.describe('Widget Playback State Synchronization', () => {
  ...
  test.beforeEach(async ({ page: testPage }) => {
    ...
    page.on('console', (msg) => {
      ...
      if (text.includes('Sync') || text.includes('isPlaying') || text.includes('Transport')) {
  ...
  test('widgets start playing when transport starts', async () => {
    ...
    const metronomePlay = logs.find(log => 
      log.includes('useWidgetSync[enhanced-metronome-widget]: Received PLAY event')
    );
  ...
  test('widgets maintain correct isPlaying state', async () => {
    ...
    const isPlayingLogs = logs.filter(log => 
      log.includes('isPlaying')
    );
    ...
    const playingStates = logs.filter(log => 
    ...
    const stoppedStates = logs.filter(log => 
  ...
  test('transport state vs widget state mismatch', async () => {
    ...
    const stateLogs = logs.filter(log => 
      log.includes('Sync state changed')
    );
    
    // Look for the specific mismatch pattern
    stateLogs.forEach(log => {
      if (log.includes("Transport?.state: 'started'") && log.includes('isConnected: false')) {
        console.error('MISMATCH: Transport is started but widget is not connected!');
      }
      if (log.includes("Transport?.state: 'stopped'") && log.includes('syncProps.isPlaying: true')) {
  ...
  test('widget event processing performance', async () => {
    ...
    const processingTimes = logs.filter(log => 
      log.includes('Processed event') && log.includes('ms')
    );
