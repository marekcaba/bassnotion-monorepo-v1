...
test.describe('Transport Full Duration Playback', () => {
  test('transport plays for full exercise duration (8 seconds)', async ({
    ...
    const startedStates = playbackData.states.filter(
      (s) => s.state === 'started',
    );
    const stoppedStates = playbackData.states.filter(
      (s) => s.state === 'stopped',
    );

    console.log(
      `Transport was in 'started' state ${startedStates.length} times`,
    );
    console.log(
      `Transport was in 'stopped' state ${stoppedStates.length} times`,
    );
    ...
    if (startedStates.length > 0) {
      ...
      console.log(
        `First 'started' at: ${firstStarted.time}ms, position: ${firstStarted.position}s`,
      );
      console.log(
        `Last 'started' at: ${lastStarted.time}ms, position: ${lastStarted.position}s`,
      );
    ...
    const syncTimeoutLogs = logs.filter(
      (log) =>
    ...
    const heartbeatLogs = logs.filter(
      (log) => log.includes('heartbeat') || log.includes('💓'),
    );
  ...
  test('widget sync heartbeat prevents timeout during playback', async ({
    page,
  }) => {
    await page.goto('http://localhost:3001/test-transport');
    await page.waitForSelector('h1:has-text("Global Transport")', {
    ...
    if (heartbeatEvents.length > 0) {
      ...
      console.warn(
        'No heartbeat events captured - sync timeout prevention may not be working',
      );
    }
  ...
  test('transport continues playing without interruption', async ({ page }) => {
          ...
          if (startTime !== null) {
            ...
            if (samples.length > 1) {
              ...
              if (currentSample.position < lastSample.position - 0.1) {
                interrupted = true;
                console.error(
                  `Position went backwards: ${lastSample.position} -> ${currentSample.position}`,
                );
              }

              // State should not change to stopped before 8 seconds
              if (
                currentSample.state === 'stopped' &&
                currentSample.time < 7500
              ) {
                interrupted = true;
                console.error(
                  `Transport stopped early at ${currentSample.time}ms`,
                );
              }
            }
    ...
    const result = (await monitorPromise) as {
      samples: any[];
      interrupted: boolean;
    };
    ...
    if (result.samples.length > 0) {
      const playingSamples = result.samples.filter(
        (s) => s.state === 'started',
      );
