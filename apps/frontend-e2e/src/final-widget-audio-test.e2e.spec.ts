...
test.describe('Final Widget Audio Test', () => {
  test('verify widgets now play audio with +0.1 start time fix', async ({
    ...
    await page.evaluate(() => {
      ...
      if (Tone) {
        ...
        if (Tone.MetalSynth && Tone.MetalSynth.prototype.triggerAttackRelease) {
          const originalTrigger =
            Tone.MetalSynth.prototype.triggerAttackRelease;
    ...
    if (results.audioTriggers.length > 0) {
      console.log('\n✅ AUDIO IS PLAYING!');
      const kickCount = results.audioTriggers.filter(
        (t: any) => t.type === 'kick',
      ).length;
      const snareCount = results.audioTriggers.filter(
        (t: any) => t.type === 'snare',
      ).length;
      const hihatCount = results.audioTriggers.filter(
        (t: any) => t.type === 'hihat',
      ).length;
      console.log(
        `Kicks: ${kickCount}, Snares: ${snareCount}, Hihats: ${hihatCount}`,
      );
    }
