  ...
  test('verify all widgets produce audio without errors', async ({ page }) => {
    ...
    await page.evaluate(() => {
      ...
      if (Tone.Sampler && Tone.Sampler.prototype.triggerAttackRelease) {
        const originalSamplerTrigger =
          Tone.Sampler.prototype.triggerAttackRelease;
      ...
      if (
        Tone.ToneAudioNode &&
        Tone.ToneAudioNode.prototype.triggerAttackRelease
      ) {
        const originalTrigger =
          Tone.ToneAudioNode.prototype.triggerAttackRelease;
        Tone.ToneAudioNode.prototype.triggerAttackRelease = function (
          ...args: any[]
        ) {
          const className = this.constructor.name;
          if (
            className.includes('MembraneSynth') ||
            className.includes('NoiseSynth')
          ) {
    ...
    const scheduleErrors = errors.filter(
      (err) => err.includes('scheduleRepeat') || err.includes('Transport'),
    );
    ...
    const widgetExecutions = logs.filter(
      (log) =>
    ...
    const totalAudioEvents =
      (audioStats.drumHits || 0) +
    ...
    if (audioStats.harmonyNotes > 0) {
      console.log(
        `✅ Harmony widget working: ${audioStats.harmonyNotes} notes`,
      );
    }
    ...
    if (bufferErrors.length > 0) {
      ...
      console.log(
        `Notes played despite errors: ${audioStats.harmonyNotes > 0 ? 'YES' : 'NO'}`,
      );
