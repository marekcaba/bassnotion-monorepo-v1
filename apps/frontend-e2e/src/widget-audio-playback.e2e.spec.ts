...
test.describe('Widget Audio Playback Investigation', () => {
  test('check if widgets are attempting to play sounds', async ({ page }) => {
    ...
    await page.evaluate(() => {
      ...
      const checkAndPatch = () => {
        ...
        if (Tone.Sampler) {
          const originalTriggerAttackRelease =
          ...
          Tone.Sampler.prototype.triggerAttackRelease = function (
            ...
            console.log(
              `🎵 Sampler.triggerAttackRelease called: note=${note}, duration=${duration}, time=${time}, velocity=${velocity}`,
            );
    ...
    if (playbackAttempts.length === 0) {
      ...
      console.log(
        'Widgets are not calling triggerAttackRelease or start methods.',
      );
    } else {
      console.log('\n✅ Audio playback attempts detected.');

      // Check if samplers are loaded
      const samplerAttempts = playbackAttempts.filter(
        (a: any) => a.type === 'Sampler',
      );
  ...
  test('verify widget schedules are executing their callbacks', async ({
    page,
  }) => {
    await page.goto('http://localhost:3001/test-transport');
    await page.waitForLoadState('networkidle');
