...
test.describe('Harmony Schedule Debug', () => {
  test('check what happens in harmony schedule callback', async ({ page }) => {
    ...
    const processorLogs = allLogs.filter(
      (log) =>
    ...
    const firstChordLogs = allLogs.filter(
      (log) =>
        log.includes('first chord check') || log.includes('First chord check'),
    );
    ...
    console.log('\n=== Processor Logs ===');
    const initLogs = processorLogs.filter(
      (log) =>
