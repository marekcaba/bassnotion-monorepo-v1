...
test.describe('Widget Schedule Execution Test', () => {
  test('test if widget schedules execute after proper setup', async ({ page }) => {
    ...
    await page.evaluate(() => {
      ...
      Tone.Transport.scheduleRepeat = function(...args: any[]) {
        ...
        try {
          const id = originalScheduleRepeat.call(this, wrappedCallback, interval, startTime);
          scheduleInfo.id = id;
    ...
    page.on('console', (msg) => {
      const text = msg.text();
      if (text.includes('DRUM TRANSPORT SCHEDULE EXECUTED') ||
    ...
    results.test.schedules.forEach((schedule: any) => {
      ...
      const executions = results.test.executions.filter((e: any) => e.scheduleId === schedule.id);
    ...
    if (results.test.executions.length === 0) {
      ...
      console.log('\n=== Widget Logs ===');
      const widgetLogs = importantLogs.filter(log => 
        log.includes('DRUM') || log.includes('HARMONY')
      ).slice(0, 10);
      ...
      const byInterval = results.test.executions.reduce((acc: any, exec: any) => {
