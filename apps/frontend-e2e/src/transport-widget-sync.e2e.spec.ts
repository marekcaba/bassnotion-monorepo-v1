...
test.describe('Transport and Widget Synchronization', () => {
  ...
  test('transport starts and widgets respond to play event', async ({ page }) => {
    // Navigate to the test transport page
    await page.goto('/test-transport');
    
    // Wait for the page to load and transport to initialize
    await page.waitForSelector('button:has-text("▶️ PLAY")', { timeout: 10000 });
    ...
    const transportStartLog = logs.find(log => 
      log.text().includes('TransportController: Transport started successfully')
    );
    ...
    const metronomePlayed = logs.find(log => 
      log.text().includes('useWidgetSync[enhanced-metronome-widget]: Received PLAY event')
    );
  ...
  test('widgets stop when transport stops', async ({ page }) => {
    ...
    const drumLoopStopped = logs.find(log => 
      log.text().includes('DrummerWidget: syncIsPlaying=false, stopping drum loop')
    );
  ...
  test('audio context initialization on user gesture', async ({ page }) => {
    ...
    console.log('Final AudioContext state:', await page.evaluate(() => (window as any).Tone?.context?.state));
  });
  ...
  test('widget sync service event propagation', async ({ page }) => {
    ...
    const widgetReceivedEvents = logs.filter(log => 
      log.text().includes('useWidgetSync[') && log.text().includes('Received PLAY event')
    );
  ...
  test('transport position synchronization', async ({ page }) => {
    ...
    const widgetPositionLogs = logs.filter(log => 
      log.text().includes('position:') && 
      (log.text().includes('DrummerWidget') || log.text().includes('HarmonyWidget'))
    );
  ...
  test('widget connection status', async ({ page }) => {
    ...
    if (connectionTimeouts.length > 0) {
      console.log('Found sync connection timeouts:', connectionTimeouts.map(log => log.text()));
    }
  });
