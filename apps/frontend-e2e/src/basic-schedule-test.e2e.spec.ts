  ...
  test('verify basic Tone.js schedule works at all', async ({ page }) => {
    ...
    const scheduleResults = await page.evaluate(async () => {
      ...
      try {
        ...
        results.scheduleAttempts.push({ type: 'repeat-no-start', error: e.message });
      }
      ...
      try {
        ...
        results.scheduleAttempts.push({ type: 'repeat-+0.1', error: e.message });
      }
    ...
    if (scheduleResults.scheduleFires.length === 0) {
      ...
      console.log('This indicates a fundamental issue with Tone.Transport in the test environment.');
    } else {
