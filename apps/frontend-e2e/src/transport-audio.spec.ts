...
test.describe('Real Transport Audio Tests', () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page.waitForSelector('h1:has-text("Global Transport & Widget Synchronization Test")');
  });
  ...
  test('Transport starts and schedules continue running', async ({ page }) => {
    ...
    await page.waitForSelector('text=Transport State: playing', { timeout: 5000 });
    ...
    await page.waitForSelector('text=Transport State: stopped', { timeout: 5000 });
  });
  ...
  test('Widgets produce actual audio output', async ({ page }) => {
    ...
    page.on('console', msg => {
      const text = msg.text();
      if (text.includes('Playing at volume') || 
  ...
  test('Transport schedules persist across widget re-renders', async ({ page }) => {
    // Start transport
    await page.click('button:has-text("Start Transport")');
    ...
    const slider = await page.locator('input[type="range"][min="60"][max="200"]');
