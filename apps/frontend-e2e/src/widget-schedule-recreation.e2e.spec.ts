...
test.describe('Widget Schedule Recreation Bug', () => {
  test('widgets fail to recreate schedules after stop/play cycle', async ({ page }) => {
    ...
    const firstPlayScheduleLogs = logs.filter(log => 
      log.includes('DRUM TRANSPORT SCHEDULE EXECUTED') ||
      log.includes('ATTEMPTING TO PLAY') ||
      log.includes('Loop callback fired')
    );
    
    console.log(`First play: ${firstPlayScheduleLogs.length} schedule executions`);
    ...
    const clearLogs = logs.filter(log => 
    ...
    const secondPlayScheduleLogs = logs.filter(log => 
      log.includes('DRUM TRANSPORT SCHEDULE EXECUTED') ||
      log.includes('ATTEMPTING TO PLAY') ||
      log.includes('Loop callback fired')
    );
    
    console.log(`Second play: ${secondPlayScheduleLogs.length} schedule executions`);
    
    // Check for "creating" logs that indicate schedule recreation
    const recreationLogs = logs.filter(log => 
      log.includes('creating drum loop') ||
      log.includes('Creating drum loop') ||
      log.includes('Transport schedule created')
    );
    console.log(`Second play: ${recreationLogs.length} schedule recreation attempts`);
    ...
    if (secondPlayScheduleLogs.length === 0) {
      ...
      console.log('The drum/harmony widgets check for !loopRef.current which prevents recreation');
    }
  ...
  test('verify the schedule recreation prevention condition', async ({ page }) => {
    await page.goto('http://localhost:3001/test-transport');
    await page.waitForLoadState('networkidle');
    await page.waitForTimeout(2000);
    ...
    await page.evaluate(() => {
      ...
      console.log = (...args) => {
        ...
        if (message.includes('Samples loaded and syncIsPlaying=true') ||
    ...
    // First play
    await page.evaluate(() => { (window as any).__currentPhase = 'first-play'; });
    await page.click('button:has-text("▶️ PLAY")');
    await page.waitForTimeout(1000);
    
    // Stop
    await page.evaluate(() => { (window as any).__currentPhase = 'stop'; });
    await page.click('button:has-text("⏹️ STOP")');
    await page.waitForTimeout(1000);
    
    // Second play
    await page.evaluate(() => { (window as any).__currentPhase = 'second-play'; });
    await page.click('button:has-text("▶️ PLAY")');
    await page.waitForTimeout(1000);
    
    // Get the checks
    const checks = await page.evaluate(() => (window as any).__widgetScheduleChecks);
    ...
    const secondPlayChecks = checks.filter((c: any) => c.phase === 'second-play');
    
    console.log(`\nFirst play: ${firstPlayChecks.length} schedule creation checks`);
    console.log(`Second play: ${secondPlayChecks.length} schedule creation checks`);
