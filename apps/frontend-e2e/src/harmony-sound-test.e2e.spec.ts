...
test.describe('Harmony Sound Test', () => {
  test('test if harmony widget produces sound after fix', async ({ page }) => {
    ...
    // Check logs
    const harmonyLogs = allLogs.filter(log => 
      log.includes('HARMONY') || log.includes('Harmony')
    );
    ...
    if (results.samplerTriggers?.length > 0) {
      ...
      const loadedTriggers = results.samplerTriggers.filter((t: any) => t.loaded);
      const unloadedTriggers = results.samplerTriggers.filter((t: any) => !t.loaded);
