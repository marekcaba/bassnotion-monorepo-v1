...
test.describe('Test Sampler Loading', () => {
  test('check sampler loading and fallback', async ({ page }) => {
    ...
    page.on('console', (msg) => {
      const text = msg.text();
      if (text.includes('🎹') || text.includes('Sampler') || text.includes('layer')) {
        logs.push(text);
      }
    });
    ...
    logs.filter(log => log.includes('loaded') || log.includes('Sampler')).forEach(log => console.log(log));
    ...
    console.log('\n=== Error Logs ===');
    logs.filter(log => log.includes('No loaded sampler')).forEach(log => console.log(log));
  });
});