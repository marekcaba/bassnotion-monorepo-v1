...
test.describe('Transport Stopped Schedule Bug', () => {
  test('verify schedules created when Transport is stopped dont work', async ({ page }) => {
    ...
    const testResults = await page.evaluate(async () => {
      ...
      const id3 = Tone.Transport.scheduleRepeat((time: number) => {
        scenario3Fires++;
    ...
    testResults.scenarios.forEach(scenario => {
      ...
      console.log(`  - Transport state at creation: ${scenario.transportStateAtCreation}`);
      console.log(`  - Fires: ${scenario.fires}`);
      console.log(`  - Result: ${scenario.fires > 0 ? '✅ WORKS' : '❌ BROKEN'}`);
    });
    
    // The bug: schedules created when Transport is stopped don't work
    const stoppedScenario = testResults.scenarios.find(s => s.name === 'Schedule when stopped');
    const startedScenario = testResults.scenarios.find(s => s.name === 'Schedule after started');
    
    if (stoppedScenario?.fires === 0 && startedScenario?.fires > 0) {
      console.log('\n❌ BUG CONFIRMED: Schedules created when Transport is stopped DO NOT WORK!');
      console.log('This is why widget audio doesn\'t play - widgets create schedules before Transport starts.');
    }
  });
});