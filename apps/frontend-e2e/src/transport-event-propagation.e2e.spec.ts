...
test.describe('Transport Event Propagation Diagnostics', () => {
    ...
    await page.addInitScript(() => {
      ...
      console.log = (...args) => {
        ...
        if (message.includes('event') || message.includes('Event') || 
            message.includes('Sync') || message.includes('Transport')) {
    ...
    while (Date.now() - startTime < 35000) {
      ...
      if (timeoutLogs.length > 0) {
        ...
        timeoutLogs.forEach(log => {
          ...
          const match = log.text.match(/\[([^\]]+)\] Sync connection lost \((\d+)ms/);
          if (match) {
  ...
  test('check widget registration and connection', async () => {
    ...
    const widgetInfo = await page.evaluate(() => {
      ...
      const widgetTypes = ['drummer-widget', 'harmony-widget', 'enhanced-metronome-widget'];
  ...
  test('verify transport controller event emission', async () => {
    ...
    await page.evaluate(() => {
      ...
      const checkController = setInterval(() => {
        ...
        const controller = (window as any).TransportController || 
  ...
  test('identify sync heartbeat mechanism', async () => {
    ...
    page.on('console', (msg) => {
      ...
      if (text.includes('heartbeat') || text.includes('ping') || 
          text.includes('keepalive') || text.includes('sync check')) {
        heartbeatLogs.push({
          time: Date.now(),
          text
        });
    ...
    if (heartbeatLogs.length === 0) {
      console.error('NO HEARTBEAT MECHANISM DETECTED - This is likely why sync times out!');
    }
  });
  ...
  test('check for widget update loops', async () => {
    ...
    page.on('console', (msg) => {
      const text = msg.text();
      if (text.includes('DrummerWidget:') || text.includes('HarmonyWidget:') || 
