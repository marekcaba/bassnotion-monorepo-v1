...
test.describe('Comprehensive Transport-to-Audio Pipeline', () => {
  ...
  test('Complete audio pipeline workflow', async () => {
    ...
    await test.step('Verify UnifiedTransport initialization', async () => {
      ...
      const debugInfo = await page.evaluate(() => {
        ...
        return {
          ...
          hasGetUnifiedTransport: typeof services?.getUnifiedTransport === 'function',
          hasTransport: !!transport,
          hasGetPatternScheduler: typeof transport?.getPatternScheduler === 'function',
          servicesMethods: services ? Object.keys(services).filter(k => typeof services[k] === 'function') : []
        };
      });
      
      console.log('🔍 Transport debug info:', JSON.stringify(debugInfo, null, 2));
      
      // Wait for transport to be available
      const transportReady = await page.waitForFunction(() => {
        const services = (window as any).__globalCoreServices;
        if (!services) return false;
        
        const transport = services.getUnifiedTransport?.();
        return transport && typeof transport.getPatternScheduler === 'function';
    ...
    await test.step('Verify widget singleton system', async () => {
      ...
      console.log('✅ Widget singleton system active:', activeWidgets.length, 'widgets');
    });
    ...
    await test.step('Verify pattern registrations', async () => {
      ...
      console.log('✅ Pattern registrations:', JSON.stringify(patternInfo, null, 2));
      
      // Verify we have essential patterns
      const widgetTypes = Object.values(patternInfo).map((p: any) => p.widgetType);
    ...
    await test.step('Verify timing processor functionality', async () => {
      ...
      const timingUpdatesReceived = await page.waitForFunction(() => {
        return new Promise(resolve => {
          ...
          const checkUpdates = () => {
            updateCount++;
            if (updateCount >= 3) { // Wait for at least 3 timing updates
              resolve(true);
            } else {
    ...
    await test.step('Verify no critical errors in console', async () => {
      ...
      const criticalErrors = messages.filter((msg: any) => 
        msg.includes('Error:') && 
      ...
      console.log(`✅ Console check complete. Found ${criticalErrors.length} critical errors`);
    });

    console.log('🎉 Comprehensive transport-audio pipeline test completed successfully!');
  });

  test('Widget pattern registration race condition resistance', async () => {
    console.log('🧪 Testing widget pattern registration race condition resistance');
    ...
    await test.step('Verify pattern registration after remounts', async () => {
      ...
      console.log(`✅ Pattern registration stable after remounts: ${patternCount} patterns`);
    });
