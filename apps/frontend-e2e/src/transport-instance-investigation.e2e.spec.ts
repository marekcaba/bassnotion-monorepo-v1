...
test.describe('Transport Instance Investigation', () => {
  test('deep dive into Transport instance and timing issues', async ({ page }) => {
    ...
    const initialState = await page.evaluate(() => {
      ...
      const harmonyWidget = document.querySelector('[data-testid="harmony-widget"]');
    ...
    if (monitorData.transportStarts?.length > 0 && monitorData.scheduleCreations?.length > 0) {
      ...
      monitorData.scheduleCreations.forEach((schedule: any, i: number) => {
        ...
        console.log(`Schedule ${i + 1} created ${timeDiff}ms ${timeDiff > 0 ? 'AFTER' : 'BEFORE'} Transport.start`);
        console.log(`  - Transport state when schedule created: ${schedule.transportState}`);
        console.log(`  - Context state when schedule created: ${schedule.contextState}`);
      });
    }
    
    // PHASE 5: Check for widget-specific logs
    console.log('\n=== PHASE 5: Widget Logs ===');
    const widgetLogs = allLogs.filter(log => 
