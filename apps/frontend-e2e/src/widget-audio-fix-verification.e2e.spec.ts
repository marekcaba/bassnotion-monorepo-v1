...
test.describe('Widget Audio Fix Verification', () => {
  test('verify widgets now play audio after schedule timing fix', async ({ page }) => {
    ...
    await page.evaluate(() => {
      ...
      if (Tone) {
        ...
        Tone.Transport.scheduleRepeat = function(...args: any[]) {
          ...
          console.log(`📅 Schedule created: interval=${interval}, startTime=${startTime || 'current'}`);
    ...
    console.log(`Schedule callbacks executed: ${results.scheduleCallbacks.length}`);
    ...
    if (results.audioAttempts.length > 0) {
      ...
      console.log('First few audio attempts:', results.audioAttempts.slice(0, 5));
    } else {
