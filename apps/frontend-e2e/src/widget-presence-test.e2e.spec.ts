...
test.describe('Widget Presence Test', () => {
  test('check if widgets are actually rendered and creating schedules', async ({
    ...
    const widgetInfo = await page.evaluate(() => {
      ...
      const metronome = document.querySelector(
        '[data-testid="metronome-widget"]',
      );
      const enhancedMetronome = document.querySelector(
        '[data-testid="enhanced-metronome-widget"]',
      );

      // Check if widgets are visible
      const drummerVisible = drummer
        ? window.getComputedStyle(drummer).display !== 'none'
        : false;
      const harmonyVisible = harmony
        ? window.getComputedStyle(harmony).display !== 'none'
        : false;
    ...
    if (showWidgetsButton) {
      ...
      const afterShow = await page.evaluate(() => {
        const drummer = document.querySelector(
          '[data-testid="drummer-widget"]',
        );
        const harmony = document.querySelector(
    ...
    const scheduleLogs = logs.filter(
      (log) => log.includes('schedule') && log.includes('created'),
    );
