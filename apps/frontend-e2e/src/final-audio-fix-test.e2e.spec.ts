...
test.describe('Final Audio Fix Test', () => {
  test('verify widgets play audio after Transport state check fix', async ({ page }) => {
    ...
    await page.evaluate(() => {
      ...
      if (Tone) {
        ...
        ['MembraneSynth', 'NoiseSynth', 'MetalSynth'].forEach(synthType => {
          if (Tone[synthType] && Tone[synthType].prototype.triggerAttackRelease) {
            const original = Tone[synthType].prototype.triggerAttackRelease;
            Tone[synthType].prototype.triggerAttackRelease = function(...args: any[]) {
    ...
    console.log(`Schedule executions: ${results.test.scheduleExecutions.length}`);
    ...
    results.test.scheduleCreations.forEach((creation: any, i: number) => {
      console.log(`Schedule ${i + 1}: ${creation.interval}, Transport: ${creation.transportState}`);
    });
