...
test.describe('Bassline Persistence E2E Tests', () => {
  ...
  test.describe('Bassline Creation and Saving', () => {
    test('should create and save a new bassline successfully', async ({
      page,
    }) => {
      ...
      await test.step('Create bassline notes', async () => {
        ...
        const noteElements = await page
          .locator('[data-testid="exercise-note"]')
          .count();
      ...
      await test.step('Open save dialog', async () => {
        ...
        expect(
          await page
            .locator('[data-testid="save-bassline-dialog"]')
            .isVisible(),
        ).toBe(true);
      });
      ...
      await test.step('Fill bassline details', async () => {
        // Fill in name
        await page.fill(
          '[data-testid="bassline-name-input"]',
          testBassline.name,
        );
        ...
        expect(
          await page.locator('[data-testid="tag-badge"]').textContent(),
        ).toContain('e2e-test');
      });
      ...
      await test.step('Save bassline', async () => {
        ...
        expect(
          await page.locator('[data-testid="success-toast"]').textContent(),
        ).toContain('saved successfully');

        // Dialog should close
        await expect(
          page.locator('[data-testid="save-bassline-dialog"]'),
        ).not.toBeVisible();
      });
      ...
      await test.step('Verify saved bassline appears in list', async () => {
        ...
        const basslineCard = page
          .locator('[data-testid="bassline-card"]')
          .first();
        expect(
          await basslineCard
            .locator('[data-testid="bassline-name"]')
            .textContent(),
        ).toBe(testBassline.name);
        expect(
          await basslineCard
            .locator('[data-testid="bassline-description"]')
            .textContent(),
        ).toBe(testBassline.description);
      });
    });
    ...
    test('should handle save conflicts gracefully', async ({ page }) => {
      ...
      await test.step('Handle conflict response', async () => {
        ...
        expect(
          await page.locator('[data-testid="error-toast"]').textContent(),
        ...
        expect(
          await page.locator('[data-testid="overwrite-checkbox"]').isVisible(),
        ).toBe(true);
      });
    ...
    test('should validate form inputs correctly', async ({ page }) => {
      await test.step('Test empty name validation', async () => {
        ...
        expect(
          await page.locator('[data-testid="name-error"]').textContent(),
        ).toContain('required');
  ...
  test.describe('Auto-save Functionality', () => {
    test('should auto-save changes periodically', async ({ page }) => {
      await test.step('Setup auto-save monitoring', async () => {
        ...
        if (
          (await autoSaveToggle.isVisible()) &&
          !(await autoSaveToggle.isChecked())
        ) {
      ...
      await test.step('Trigger auto-save with changes', async () => {
        ...
        await page.waitForSelector('[data-testid="auto-save-indicator"]', {
          timeout: 35000,
        }); // Auto-save typically happens within 30s
      ...
      await test.step('Verify auto-save recovery', async () => {
        ...
        await page.route(
          '**/api/user-basslines/auto-save/recovery',
          async (route) => {
            await route.fulfill({
              status: 200,
              contentType: 'application/json',
        ...
        expect(
          await page
            .locator('[data-testid="recovery-notification"]')
            .textContent(),
        ).toContain('recovered');
    ...
    test('should handle auto-save conflicts', async ({ page }) => {
      ...
      await test.step('Trigger and handle conflict', async () => {
        ...
        await page.waitForSelector(
          '[data-testid="auto-save-conflict-notification"]',
          { timeout: 35000 },
        );

        // Verify conflict handling options are presented
        expect(
          await page.locator('[data-testid="conflict-keep-local"]').isVisible(),
        ).toBe(true);
        expect(
          await page
            .locator('[data-testid="conflict-keep-remote"]')
            .isVisible(),
        ).toBe(true);
        expect(
          await page.locator('[data-testid="conflict-merge"]').isVisible(),
        ).toBe(true);
      });
    });
  });
  ...
  test.describe('Bassline Management', () => {
    test('should load saved basslines successfully', async ({ page }) => {
      ...
      await test.step('Verify basslines display correctly', async () => {
        ...
        expect(
          await firstCard
            .locator('[data-testid="bassline-difficulty"]')
            .textContent(),
        ).toBe('Beginner');
        expect(
          await firstCard
            .locator('[data-testid="bassline-tempo"]')
            .textContent(),
        ).toContain('120 BPM');

        // Verify tags
        const rockTag = firstCard
          .locator('[data-testid="bassline-tag"]')
          .filter({ hasText: 'rock' });
      ...
      await test.step('Load a bassline', async () => {
        ...
        const loadButton = firstCard.locator(
          '[data-testid="load-bassline-button"]',
        );
    ...
    test('should search and filter basslines', async ({ page }) => {
      await test.step('Setup basslines list', async () => {
        ...
        await page.route('**/api/user-basslines*', async (route) => {
          ...
          if (difficulty && difficulty !== 'all') {
            basslines = basslines.filter(
              (b) => b.metadata.difficulty === difficulty,
            );
          }
      ...
      await test.step('Test difficulty filter', async () => {
        ...
        await page.waitForSelector('[data-testid="bassline-card"]');
        expect(
          await page.locator('[data-testid="bassline-card"]').count(),
        ).toBe(3);
    ...
    test('should rename basslines successfully', async ({ page }) => {
      ...
      await test.step('Rename the bassline', async () => {
        ...
        await page.route(
          '**/api/user-basslines/bassline-1/rename',
          async (route) => {
            await route.fulfill({
              status: 200,
              contentType: 'application/json',
        ...
        expect(
          await page.locator('[data-testid="bassline-name"]').textContent(),
        ).toBe('New Bassline Name');
      });
    });
    ...
    test('should delete basslines with confirmation', async ({ page }) => {
      ...
      await test.step('Confirm deletion', async () => {
        ...
        expect(
          await page
            .locator('[data-testid="delete-confirmation-text"]')
            .textContent(),
        ).toContain('Bassline to Delete');
        expect(
          await page
            .locator('[data-testid="delete-confirmation-text"]')
            .textContent(),
        ...
        await page.route(
          '**/api/user-basslines/bassline-to-delete',
          async (route) => {
            await route.fulfill({
              status: 200,
              contentType: 'application/json',
        ...
        expect(
          await page.locator('[data-testid="empty-state"]').textContent(),
        ).toContain('No basslines found');
      });
    });
  });
  ...
  test.describe('Performance and Reliability', () => {
    ...
    test('should maintain data integrity across page refreshes', async ({
      page,
    }) => {
