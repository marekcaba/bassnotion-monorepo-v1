...
test.describe('Debug Buffer Check', () => {
  test('check why no audio is produced', async ({ page }) => {
    ...
    const notLoadedLogs = logs.filter(
      (log) => log.includes('not loaded') || log.includes('not available'),
    );
    ...
    const fallbackLogs = logs.filter(
      (log) => log.includes('fallback') || log.includes('Fallback'),
    );
