...
test.describe('Drum Pattern Debug', () => {
  test('check if drum pattern has events and why no sounds are triggered', async ({
    ...
    const patternLogs = detailedLogs.filter(
      (log) =>
    ...
    const eventLogs = detailedLogs.filter(
      (log) =>
    ...
    const samplerLogs = detailedLogs.filter(
      (log) =>
    ...
    // Look for any errors
    const errorLogs = detailedLogs.filter(
      (log) =>
        log.includes('Error') ||
        log.includes('error') ||
        log.includes('failed') ||
    ...
    const subdivisionLogs = detailedLogs.filter(
      (log) => log.includes('subdivision') || log.includes('Subdivision'),
    );

    console.log('\n=== Subdivision Processing ===');
    if (subdivisionLogs.length === 0) {
      console.log(
        '❌ NO SUBDIVISION LOGS - Pattern events might not be matching!',
      );
    } else {
