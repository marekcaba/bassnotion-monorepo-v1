...
test.describe('Transport 1-Second Playback Issue', () => {
  test('diagnose why transport stops after 1 second', async ({ page }) => {
    ...
    page.on('response', (response) => {
      if (response.url().includes('api') || response.url().includes('exercise')) {
    ...
    const monitorHandle = await page.evaluateHandle(() => {
      const monitor = {
        transportStates: [] as Array<{time: number; state: string; position: number}>,
    ...
    const startedIndex = results.transportStates.findIndex(s => s.state === 'started');
    const stoppedIndex = results.transportStates.findIndex((s, i) => 
      i > startedIndex && s.state === 'stopped'
    );
    ...
    if (startedIndex >= 0) {
      ...
      if (stoppedIndex >= 0) {
        ...
        if (playbackDuration < 1500) {
          console.log('⚠️ ISSUE CONFIRMED: Transport stopped after less than 1.5 seconds!');
    ...
    if (results.loopInfo) {
      ...
      if (typeof results.loopInfo.loopEnd === 'string' && results.loopInfo.loopEnd.includes(':')) {
  ...
  test('compare transport behavior: test page vs widget page', async ({ browser }) => {
    const context = await browser.newContext();
    
    // Test on both pages in parallel
    const [testPageResults, widgetPageResults] = await Promise.all([
      testTransportOnPage(context, 'http://localhost:3001/test-transport', 'Test Page'),
      testTransportOnPage(context, 'http://localhost:3001/exercises', 'Widget Page')
    ]);
...
async function testTransportOnPage(context: any, url: string, name: string) {
  ...
  await page.evaluate(() => {
    ...
    const checkInterval = setInterval(() => {
      ...
      if (Tone.Transport.state === 'started' && !(window as any).__transportStarted) {
        (window as any).__transportStarted = Date.now();
        (window as any).__startPosition = Tone.Transport.seconds;
      }
      
      if (Tone.Transport.state === 'stopped' && (window as any).__transportStarted && !(window as any).__transportStopped) {
  ...
  if (url.includes('test-transport')) {
    ...
    playButton = await page.locator('button').filter({ hasText: /play|▶/i }).first();
  } else {
  ...
  const results = await page.evaluate(() => {
    ...
    return {
      ...
      finalPosition: stopped ? stopPos : ((window as any).Tone?.Transport?.seconds || 0),
      stopped: !!stopped
    };
  });
