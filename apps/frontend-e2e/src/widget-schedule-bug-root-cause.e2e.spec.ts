...
test.describe('Widget Schedule Bug Root Cause', () => {
  test('investigate why schedules do not fire', async ({ page }) => {
    ...
    const scheduleTest = await page.evaluate(() => {
      ...
      try {
        ...
        const scheduleId = Tone.Transport.scheduleRepeat((time: number) => {
    ...
    console.log('Manual transport start needed:', scheduleTest.manualTransportStarted);
    ...
    console.log('Immediate schedule fired:', finalResults.immediateScheduleFired);
    ...
    if (!finalResults.scheduleFired && finalResults.finalTransportState === 'started') {
      console.log('\n❌ BUG CONFIRMED: Transport is running but schedules are not firing!');
      console.log('This explains why widgets produce no audio - their schedules never execute.');
    }
  ...
  test('check if widget schedules are created at all', async ({ page }) => {
    ...
    if (widgetSchedules.length === 0) {
      ...
      console.log('This confirms widgets are not creating their audio schedules.');
    }
  });
});