...
test.describe('Transport Schedule Timing Bug', () => {
  test('schedules created at time 0 do not fire when transport starts at +0.1', async ({ page }) => {
    ...
    await page.evaluate(() => {
      ...
      const id0 = Tone.Transport.scheduleRepeat((time: number) => {
        (window as any).__scheduleTestResults.schedule0Fired++;
        console.log(`⏰ Schedule at 0 fired! Count: ${(window as any).__scheduleTestResults.schedule0Fired}, time: ${time}`);
      }, '8n', 0);
      console.log('Schedule at 0 created with ID:', id0);
      
      // Schedule at time +0.1 (matching transport start time)
      const id01 = Tone.Transport.scheduleRepeat((time: number) => {
        (window as any).__scheduleTestResults.schedule01Fired++;
        console.log(`⏰ Schedule at +0.1 fired! Count: ${(window as any).__scheduleTestResults.schedule01Fired}, time: ${time}`);
      }, '8n', '+0.1');
      ...
      const idNow = Tone.Transport.scheduleRepeat((time: number) => {
        ...
        console.log(`⏰ Schedule at now fired! Count: ${(window as any).__scheduleTestResults.scheduleNowFired}, time: ${time}`);
      }, '8n');
      ...
      console.log('Transport timeline events:', Tone.Transport._timeline._timeline.length);
    });
    ...
    const { results, transportState, transportSeconds, contextState, scheduleIds, timelineLength } = await page.evaluate(() => {
      const Tone = (window as any).Tone;
      return {
    ...
    if (results.schedule0Fired === 0 && results.schedule01Fired > 0) {
      ...
      console.log('This is why widgets only play for 1 second or not at all - their schedules never start!');
    }
    ...
    await page.evaluate(() => {
      const Tone = (window as any).Tone;
      console.log('Creating schedule after start, Transport state:', Tone.Transport.state);
      console.log('Creating schedule after start, AudioContext state:', Tone.context.state);
      ...
      const idAfter = Tone.Transport.scheduleRepeat((time: number) => {
        ...
        console.log(`⏰ Schedule created AFTER start fired! Count: ${(window as any).__scheduleTestResults.scheduleAfterStartFired}, time: ${time}`);
      }, '8n');
    ...
    console.log(`Schedule after start: ${finalResults.scheduleAfterStartFired} fires`);
  ...
  test('widgets should use +0.1 or current time for schedules', async ({ page }) => {
    await page.goto('http://localhost:3001/test-transport');
    await page.waitForLoadState('networkidle');
    await page.waitForTimeout(2000);
    ...
    const widgetScheduleInfo = await page.evaluate(() => {
      ...
      console.log = (...args) => {
        ...
        if (message.includes('scheduleRepeat') || message.includes('schedule created')) {
          logs.push(message);
        }
    ...
    const logs = await page.evaluate(() => {
      ...
      const drumScheduleCode = document.querySelector('[data-testid="drummer-widget"]');
      const harmonyScheduleCode = document.querySelector('[data-testid="harmony-widget"]');
