  ...
  test('verify the !loopRef.current bug prevents audio after stop/play', async ({
    ...
    await page.evaluate(() => {
      ...
      if (Tone) {
        ...
        Tone.Transport.scheduleRepeat = function (...args: any[]) {
          ...
          console.log(
            `📅 Widget creating schedule: interval=${interval}, startTime=${startTime}`,
          );
    ...
    const firstPlayStats = await page.evaluate(() => {
      ...
      return {
        ...
        scheduleIntervals: [
          ...new Set(debug.schedulesCreated.map((s: any) => s.interval)),
        ],
      };
    });
    ...
    const firstPlayDrumLogs = logs.filter(
      (log) =>
    ...
    const secondPlayDrumLogs = logs.filter(
      (log) =>
        log.includes('creating drum loop') ||
        log.includes('Creating drum loop') ||
    ...
    console.log('\n=== BUG ANALYSIS ===');
    if (
      secondPlayStats.schedulesCreated === 0 &&
      firstPlayStats.schedulesCreated > 0
    ) {
      console.log('❌ BUG CONFIRMED: No schedules created on second play!');
      console.log(
        'This is caused by the !loopRef.current check preventing recreation.',
      );
    }

    if (
      secondPlayStats.audioAttempts === 0 &&
      firstPlayStats.audioAttempts > 0
    ) {
    ...
    if (conditionLogs.length > 0 && secondPlayStats.schedulesCreated === 0) {
      console.log(
        '❌ The condition to create drum loop was met, but loopRef.current blocked it!',
      );
    }
