...
export class SegmentAssessmentRepository {
  ...
  async getSegments(): Promise<VideoSegment[]> {
    this.logger.debug('Getting all segments', {
  ...
  async getSegmentById(id: string): Promise<VideoSegment | null> {
    this.logger.debug('Getting segment by ID', {
      id,
      correlationId: this.correlationId,
    });
    ...
    if (error) {
      ...
      this.logger.error('Failed to get segment', error, {
        id,
        correlationId: this.correlationId,
      });
      throw error;
  ...
  async getSegmentsByTopic(topic: string): Promise<VideoSegment[]> {
    this.logger.debug('Getting segments by topic', {
      topic,
      correlationId: this.correlationId,
    });
    ...
    if (error) {
      this.logger.error('Failed to get segments by topic', error, {
        topic,
        correlationId: this.correlationId,
      });
      throw error;
  ...
  async updateSegment(
    ...
    if (updates.targetBuckets !== undefined)
      updateData.target_buckets = updates.targetBuckets;
  ...
  async getFlowGraph(): Promise<AssessmentFlowGraph> {
    ...
    if (nodesError) {
      this.logger.error('Failed to get flow nodes', nodesError, {
        correlationId: this.correlationId,
      });
      throw nodesError;
    }
    ...
    if (edgesError) {
      this.logger.error('Failed to get flow edges', edgesError, {
        correlationId: this.correlationId,
      });
      throw edgesError;
    }
  ...
  async createFlowNode(node: Omit<FlowNode, 'id'>): Promise<FlowNode> {
    ...
    if (error) {
      this.logger.error('Failed to create flow node', error, {
        correlationId: this.correlationId,
      });
      throw error;
    }
  ...
  async updateFlowNode(
    ...
    if (updates.segmentId !== undefined)
      updateData.segment_id = updates.segmentId;
    if (updates.questionKey !== undefined)
      updateData.question_key = updates.questionKey;
    if (updates.title !== undefined) updateData.title = updates.title;
    if (updates.description !== undefined)
      updateData.description = updates.description;
    if (updates.positionX !== undefined)
      updateData.position_x = updates.positionX;
    if (updates.positionY !== undefined)
      updateData.position_y = updates.positionY;
    if (updates.isActive !== undefined) updateData.is_active = updates.isActive;
    if (updates.isEntryPoint !== undefined)
      updateData.is_entry_point = updates.isEntryPoint;
    ...
    if (error) {
      this.logger.error('Failed to update flow node', error, {
        id,
        correlationId: this.correlationId,
      });
      throw error;
  ...
  async deleteFlowNode(id: string): Promise<void> {
    ...
    if (error) {
      this.logger.error('Failed to delete flow node', error, {
        id,
        correlationId: this.correlationId,
      });
      throw error;
  ...
  async createFlowEdge(edge: Omit<FlowEdge, 'id'>): Promise<FlowEdge> {
    ...
    if (error) {
      this.logger.error('Failed to create flow edge', error, {
        correlationId: this.correlationId,
      });
      throw error;
    }
  ...
  async deleteFlowEdge(id: string): Promise<void> {
    ...
    if (error) {
      this.logger.error('Failed to delete flow edge', error, {
        id,
        correlationId: this.correlationId,
      });
      throw error;
  ...
  async bulkUpdateNodes(nodes: FlowNode[]): Promise<void> {
    this.logger.info('Bulk updating flow nodes', {
      count: nodes.length,
      correlationId: this.correlationId,
    });
    ...
    if (newEntryPoint) {
      ...
      if (clearError) {
        this.logger.error('Failed to clear existing entry points', clearError, {
          correlationId: this.correlationId,
        });
        throw clearError;
      }
    ...
    for (const node of nodes) {
      ...
      if (error) {
        this.logger.error('Failed to upsert flow node', error, {
          nodeId: node.nodeId,
          correlationId: this.correlationId,
        });
        throw error;
  ...
  async bulkUpdateEdges(nodeId: string, edges: FlowEdge[]): Promise<void> {
    this.logger.info('Bulk updating flow edges', {
      nodeId,
      count: edges.length,
      correlationId: this.correlationId,
    });
    ...
    if (deleteError) {
      this.logger.error('Failed to delete existing edges', deleteError, {
        nodeId,
        correlationId: this.correlationId,
      });
      throw deleteError;
    ...
    if (edges.length > 0) {
      ...
      if (insertError) {
        this.logger.error('Failed to insert edges', insertError, {
          nodeId,
          correlationId: this.correlationId,
        });
        throw insertError;
  ...
  async getQuestions(): Promise<SegmentQuestion[]> {
    this.logger.debug('Getting all questions', {
      correlationId: this.correlationId,
    });
  ...
  async getQuestionByKey(questionKey: string): Promise<SegmentQuestion | null> {
    this.logger.debug('Getting question by key', {
      questionKey,
      correlationId: this.correlationId,
    });
  ...
  async updateQuestion(
    ...
    if (updates.questionText !== undefined)
      updateData.question_text = updates.questionText;
    if (updates.description !== undefined)
      updateData.description = updates.description;
    if (updates.questionType !== undefined)
      updateData.question_type = updates.questionType;
    if (updates.options !== undefined) updateData.options = updates.options;
    if (updates.verificationConfig !== undefined)
      updateData.verification_config = updates.verificationConfig;
    if (updates.audioConfig !== undefined)
      updateData.audio_config = updates.audioConfig;
    ...
    if (error) {
      this.logger.error('Failed to update question', error, {
        id,
        correlationId: this.correlationId,
      });
      throw error;
  ...
  async getInsightTemplates(): Promise<CoachInsightTemplate[]> {
    this.logger.debug('Getting all insight templates', {
      correlationId: this.correlationId,
    });
    ...
    if (error) {
      this.logger.error('Failed to get insight templates', error, {
        correlationId: this.correlationId,
      });
      throw error;
    }
  ...
  async findMatchingInsight(
    ...
    if (error) {
      this.logger.error('Failed to find matching insight', error, {
        correlationId: this.correlationId,
      });
      throw error;
    }
    ...
    for (const template of data) {
      ...
      if (struggle && template.target_struggle === struggle) {
        ...
        template.target_struggle &&
        template.target_struggle !== struggle
      ) {
        score -= 50;
      }
      ...
      if (practiceTime && template.target_practice_time === practiceTime) {
        ...
        template.target_practice_time &&
        template.target_practice_time !== practiceTime
      ) {
        score -= 25;
      }
  ...
  async createInsightTemplate(
    ...
    if (error) {
      this.logger.error('Failed to create insight template', error, {
        correlationId: this.correlationId,
      });
      throw error;
    }
  ...
  async updateInsightTemplate(
    ...
    const updateData: Record<string, unknown> = {};
    if (updates.targetBucket !== undefined)
      updateData.target_bucket = updates.targetBucket;
    if (updates.targetGoal !== undefined)
      updateData.target_goal = updates.targetGoal;
    if (updates.targetStruggle !== undefined)
      updateData.target_struggle = updates.targetStruggle;
    if (updates.targetPracticeTime !== undefined)
      updateData.target_practice_time = updates.targetPracticeTime;
    if (updates.insightTitle !== undefined)
      updateData.insight_title = updates.insightTitle;
    if (updates.insightBody !== undefined)
      updateData.insight_body = updates.insightBody;
    if (updates.coachName !== undefined)
      updateData.coach_name = updates.coachName;
    if (updates.coachAvatarUrl !== undefined)
      updateData.coach_avatar_url = updates.coachAvatarUrl;
    if (updates.skillCheckAcknowledgment !== undefined)
      updateData.skill_check_acknowledgment = updates.skillCheckAcknowledgment;
    if (updates.day1Title !== undefined)
      updateData.day1_title = updates.day1Title;
    if (updates.day1Description !== undefined)
      updateData.day1_description = updates.day1Description;
    if (updates.day2Title !== undefined)
      updateData.day2_title = updates.day2Title;
    if (updates.day2Description !== undefined)
      updateData.day2_description = updates.day2Description;
    if (updates.day3Title !== undefined)
      updateData.day3_title = updates.day3Title;
    if (updates.day3Description !== undefined)
      updateData.day3_description = updates.day3Description;
    ...
    if (error) {
      this.logger.error('Failed to update insight template', error, {
        id,
        correlationId: this.correlationId,
      });
      throw error;
  ...
  async deleteInsightTemplate(id: string): Promise<void> {
    ...
    if (error) {
      this.logger.error('Failed to delete insight template', error, {
        id,
        correlationId: this.correlationId,
      });
      throw error;
  ...
  async updateSession(
    ...
    if (updates.visitedNodeIds !== undefined)
      updateData.visited_node_ids = updates.visitedNodeIds;
    if (updates.selfReportedLevel !== undefined)
      updateData.self_reported_level = updates.selfReportedLevel;
    if (updates.determinedBucket !== undefined)
      updateData.determined_bucket = updates.determinedBucket;
    if (updates.skillCheckPassed !== undefined)
      updateData.skill_check_passed = updates.skillCheckPassed;
    if (updates.skillCheckScore !== undefined)
      updateData.skill_check_score = updates.skillCheckScore;
  ...
  async updateUserBucket(userId: string, bucket: SkillBucket): Promise<void> {
    this.logger.info('Updating user bucket', {
      userId,
      bucket,
      correlationId: this.correlationId,
    });
    ...
    if (error) {
      this.logger.error('Failed to update user bucket', error, {
        userId,
        correlationId: this.correlationId,
      });
      throw error;
  ...
  private mapQuestionFromDb(data: Record<string, unknown>): SegmentQuestion {
    return {
      ...
      verificationConfig:
        data.verification_config as SegmentQuestion['verificationConfig'],
  ...
  private mapInsightFromDb(
    ...
    return {
      ...
      skillCheckAcknowledgment: data.skill_check_acknowledgment as
        | string
        | undefined,
