...
export class LogsController {
  ...
  async searchByCorrelationId(
    ...
    success: boolean;
    data: { correlationId: string; logs: BufferedLog[]; count: number };
  }> {
  ...
  async getRecentLogs(
    ...
    success: boolean;
    data: { logs: BufferedLog[]; count: number; limit: number };
  }> {
    const logger = (request as any).logger || this.staticLogger;
  ...
  @HttpCode(HttpStatus.OK)
  async getStats(@Req() request: FastifyRequest) {
    const logger = (request as any).logger || this.staticLogger;
    const correlationId = (request as any).correlationId;
  ...
  async traceRequest(
    ...
    data: {
      ...
      totalLogs: number;
      timeline: any[];
      services: Record<string, BufferedLog[]>;
      summary: any;
    };
  }> {
    const logger = (request as any).logger || this.staticLogger;
    ...
    const startTime =
      logs.length > 0 ? new Date(logs[0].timestamp).getTime() : 0;
    const endTime =
