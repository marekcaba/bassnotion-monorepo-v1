...
class TimingProcessor extends AudioWorkletProcessor {
  ...
  process(inputs, outputs, parameters) {
    ...
    if (!this.processStarted) {
      ...
      console.log(`TimingProcessor[${this.processorId}].process() started, isPlaying: ${this.isPlaying}, updateCount: ${this.updateCount}`);
    }
    ...
    if (this.samplesSinceLastUpdate >= this.samplesPerUpdate) {
      ...
      const playbackFrames = this.isPlaying ? this.totalFrames : this.pauseFrame;
      ...
      if (this.lastUpdateTime > 0) {
        ...
        if (drift > 0.005) { // 5ms threshold - more reasonable for web audio
          this.missedUpdates++;
          this.port.postMessage({
            type: 'timing-warning',
  ...
  handleMessage(event) {
    ...
    switch (event.data.type) {
      ...
      case 'stop':
        ...
        console.log(`TimingProcessor[${this.processorId}] STOPPED: totalFrames=${this.totalFrames}, isPlaying=${this.isPlaying}, sessionId=${this.sessionId}`);
        break;
      ...
      case 'get-stats':
        this.port.postMessage({
          ...
          updateCount: this.updateCount,
          missedUpdates: this.missedUpdates,
          accuracy: ((this.updateCount - this.missedUpdates) / this.updateCount * 100).toFixed(2) + '%',
