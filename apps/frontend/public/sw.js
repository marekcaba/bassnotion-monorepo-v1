    ...
    caches.keys().then((cacheNames) => {
      return Promise.all(
        cacheNames
          .filter((cacheName) => {
            return (
              cacheName.startsWith('bassnotion-') &&
...
self.addEventListener('fetch', (event) => {
  ...
  const isAudioSample = SAMPLE_PATTERNS.some((pattern) =>
...
self.addEventListener('message', (event) => {
  ...
  if (event.data.type === 'GET_CACHE_SIZE') {
    event.waitUntil(
      Promise.all([caches.open(SAMPLE_CACHE), caches.open(STATIC_CACHE)])
        .then(([sampleCache, staticCache]) => {
          return Promise.all([sampleCache.keys(), staticCache.keys()]);
        })
