  ...
  devtools(
    (set, get) => ({
      ...
      getTutorialsByLevel: (level: TutorialLevel) => {
        return Array.from(get().tutorials.values()).filter((tutorial) =>
          tutorial.level.equals(level),
        );
      },
...
export const useTutorial = (id: string) => {
  const tutorial = useTutorialRepositoryStore((state) =>
    state.getTutorialById(id),
  );
...
export const usePublishedTutorials = (options?: PaginationOptions) => {
  const publishedPage = useTutorialRepositoryStore(
    (state) => state.publishedPage,
  );
  const fetchPublished = useTutorialRepositoryStore(
    (state) => state.fetchPublished,
  );
  const isLoading = useTutorialRepositoryStore((state) => state.isLoading);
  const error = useTutorialRepositoryStore((state) => state.error);
