...
export class EffectsChain {
  ...
  addEQ(config?: {
    ...
    const lowShelf = this.nodeManager.createFilter(
      'lowshelf',
      config?.lowShelfFreq || 320,
      1,
    );
    const highShelf = this.nodeManager.createFilter(
      'highshelf',
      config?.highShelfFreq || 3200,
      1,
    );
  ...
  private rebuildChain(): void {
    ...
    logger.debug('Effects chain rebuilt', {
      enabledEffects: this.effectOrder.filter(
        (id) => this.effects.get(id)?.enabled,
      ),
    });
  }
