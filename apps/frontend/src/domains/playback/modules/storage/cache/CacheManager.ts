...
import { SampleCache, type CacheConfig, type CacheOperation } from './SampleCache.js';
...
export class CacheManager {
  ...
  private initializeLayers(): void {
    ...
    logger.info('CacheManager initialized with layers:', Array.from(this.layers.keys()));
  }
  ...
  getStats(): CacheManagerStats {
    ...
    const compressionRatio = this.totalUncompressed > 0
  ...
  private promoteIfNeeded(
    ...
    const entry = currentLayer.cache.getKeys()
      .find(key => key === sampleId);
    ...
    if (targetLayer.cache.getSize().bytes + data.byteLength <= targetLayer.maxSize) {
      logger.debug(`Promoting ${sampleId} from ${currentLayer.name} to ${targetLayer.name}`);
  ...
  updateConfig(newConfig: Partial<CacheManagerConfig>): void {
    ...
    if (newConfig.enableCompression !== undefined) {
      logger.info('Compression setting updated:', newConfig.enableCompression);
    }
  }
   ...
   */
  getLayers(): Array<{ name: string; priority: number; size: number; entries: number }> {
