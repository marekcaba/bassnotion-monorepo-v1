...
export class MusicalExpressionEngine extends EventEmitter {
  ...
  private constructor() {
    ...
    logger.info(
      ...
      '🎵 Initializing Musical Expression Engine...',
    );
  }
  ...
  public async initialize(): Promise<void> {
    if (this.isInitialized) {
      logger.info(
        'MusicalExpressionEngine',
        '⚠️ Musical Expression Engine already initialized',
      );
      return;
    }
    ...
    try {
      ...
      logger.info(
        'MusicalExpressionEngine',
        '✅ Musical Expression Engine initialized successfully',
      );
      this.emit('initialized');
    } catch (error) {
      logger.error(
        'MusicalExpressionEngine',
        `❌ Failed to initialize Musical Expression Engine: ${error instanceof Error ? error.message : String(error)}`,
      );
      throw error;
    }
  }
  ...
  public async applyExpression(
    ...
    try {
      ...
      logger.info(
        'MusicalExpressionEngine',
        `🎵 Applied expression to ${processedNotes.length} notes`,
      );
      return processedNotes;
    } catch (error) {
      logger.error(
        'MusicalExpressionEngine',
        `❌ Error applying musical expression: ${error instanceof Error ? error.message : String(error)}`,
      );
      throw error;
    }
  }
  ...
  public async applyGroove(
    ...
    if (!template) {
      logger.warn(
        'MusicalExpressionEngine',
        `⚠️ Groove template '${grooveName}' not found`,
      );
      return notes;
    }
  ...
  private async loadDefaultGrooveTemplates(): Promise<void> {
    ...
    logger.info(
      'MusicalExpressionEngine',
      `✅ Loaded ${templates.length} groove templates`,
    );
  }
  ...
  public addGrooveTemplate(template: GrooveTemplate): void {
    ...
    logger.info(
      'MusicalExpressionEngine',
      `✅ Added groove template: ${template.name}`,
    );
  }
  ...
  public async dispose(): Promise<void> {
    logger.info(
      'MusicalExpressionEngine',
      '🧹 Disposing Musical Expression Engine...',
    );
    ...
    try {
      ...
      logger.info(
        'MusicalExpressionEngine',
        '✅ Musical Expression Engine disposed successfully',
      );
      this.emit('disposed');
    } catch (error) {
      logger.error(
        'MusicalExpressionEngine',
        `❌ Error disposing Musical Expression Engine: ${error instanceof Error ? error.message : String(error)}`,
      );
      throw error;
    }
  }
...
class ArticulationProcessor {
  ...
  private determineArticulation(
    ...
    // Previous note is available but not used in current implementation
    // const prevNote = allNotes[index - 1];
