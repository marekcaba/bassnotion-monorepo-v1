...
export async function createElement(plugin: WamKeyboard): Promise<HTMLElement> {
  ...
  const instrumentSelect = container.querySelector(
    '#wam-keyboard-instrument',
  ) as HTMLSelectElement;
  const volumeSlider = container.querySelector(
    '#wam-keyboard-volume',
  ) as HTMLInputElement;
  const volumeDisplay = container.querySelector(
    '#wam-keyboard-volume-display',
  ) as HTMLElement;
  const sustainCheckbox = container.querySelector(
    '#wam-keyboard-sustain',
  ) as HTMLInputElement;
