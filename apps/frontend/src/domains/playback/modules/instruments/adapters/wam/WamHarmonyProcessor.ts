...
export class WamHarmonyProcessor {
  ...
  async initialize(audioContext: AudioContext): Promise<void> {
    ...
    try {
      ...
      this.wamPlugin = await wamPluginSingleton.getOrCreateKeyboardPlugin(audioContext);
      ...
      this.logger.info('WAM harmony processor initialized with singleton plugin instance');
   ...
   */
  async setInstrument(instrument: 'salamander' | 'rhodes' | 'wurlitzer'): Promise<void> {
    if (!this.wamPlugin) {
  ...
  triggerChord(params: ChordTriggerParams): void {
    ...
    const { chord, notes, velocity = 0.8, time, duration = '4n' } = params;
    ...
    try {
      ...
      this.logger.debug(`Triggered chord: ${chord} with ${notes?.length || 0} notes`);
    } catch (error) {
