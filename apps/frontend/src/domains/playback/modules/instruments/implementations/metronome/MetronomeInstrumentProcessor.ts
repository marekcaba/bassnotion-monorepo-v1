...
export enum Subdivision {
  ...
  QUINTUPLET = 'quintuplet', // 1/5 division
  SEXTUPLET = 'sextuplet', // 1/6 division
  SEPTUPLET = 'septuplet', // 1/7 division
  CUSTOM = 'custom',
}
...
export class MetronomeInstrumentProcessor {
  ...
  private subdivisionEnabled = false;
  private muted = false;
  ...
  private async ensureToneLoaded(audioEngine?: any): Promise<void> {
    if (!Tone) {
      // Use injected audioEngine if available, otherwise fall back to global
  ...
  public async initialize(
    ...
    try {
      ...
      logger.error('Failed to initialize MetronomeInstrumentProcessor:', error);
      throw error;
    }
  }
  ...
  public setTimeSignature(
    ...
    if (timeSignature.numerator <= 0 || timeSignature.denominator <= 0) {
      logger.warn('Invalid time signature', { timeSignature });
      return; // Don't update if invalid
    }

    // Create full TimeSignature object with defaults
    const fullTimeSignature: TimeSignature = {
      ...
      accentBeats:
        timeSignature.accentBeats ||
        this.getDefaultAccentBeats(timeSignature.numerator),
      strongBeats:
        timeSignature.strongBeats ||
        this.getDefaultStrongBeats(timeSignature.numerator),
      grouping: timeSignature.grouping,
    };

    this.config.timeSignature = fullTimeSignature;
  ...
  /**
   * Set accent pattern (with test compatibility)
   */
  public setAccentPattern(pattern: AccentLevel[] | AccentPattern): void {
    if (Array.isArray(pattern)) {
      // Convert array to AccentPattern for test compatibility
      const accentPattern: AccentPattern = {
        name: 'Custom',
        pattern: pattern.map((level, index) => ({
          beat: index,
          subdivision: 0,
          accentLevel: level,
        })),
      ...
      this.config.accentPattern = accentPattern;
    } else {
  ...
  public setGrooveTemplate(template: GrooveTemplate | null): void {
    ...
    if (template && template.swingRatio !== undefined) {
      // Convert swing ratio (0-1) to swing amount percentage (0-100)
      this.config.swingAmount = template.swingRatio * 100;
    }
  ...
  /**
   * Get current beat position information
   */
  public getBeatPosition(): {
    currentBeat: number;
    totalBeats: number;
    ...
    return {
      ...
      totalBeats: this.config.timeSignature.numerator,
      measureNumber: this.state.currentMeasure,
      beatProgress: this.state.currentSubdivision / this.getSubdivisionCount(),
    };
  }

  /**
   * Get state in test-compatible format
   * Combines state and config for backward compatibility
   */
  public getTestState(): any {
    return {
      ...
      clickSound: this.config.clickSounds.currentPreset,
      subdivision: this.config.subdivision,
      volume: this.config.clickSounds.accent.volume,
      subdivisionEnabled: this.subdivisionEnabled ?? false,
      grooveStyle: this.config.grooveTemplate?.style ?? GrooveStyle.STRAIGHT,
      swingAmount: this.config.swingAmount,
      humanization:
        this.config.grooveTemplate?.humanization?.velocityVariation ?? 0,
      timingPrecision: this.config.advancedTiming.precisionMode,
      clockSource: this.config.advancedTiming.clockSource,
      muted: this.muted ?? false,
    };
  }

  // Test compatibility methods
  public setClickSound(sound: ClickSoundType): void {
    this.setClickPreset(sound as any);
  }

  public getAccentPattern(): AccentLevel[] {
    return this.config.accentPattern.pattern.map((p) => p.accentLevel);
  }

  public setSubdivisionEnabled(enabled: boolean): void {
    this.subdivisionEnabled = enabled;
    this.emitStateChange();
  }

  public setGrooveStyle(style: GrooveStyle): void {
    if (!style) return;

    const template: GrooveTemplate = {
      name: style,
      style,
      swingRatio: style.toString().toUpperCase().includes('SWING') ? 0.67 : 0,
      microTiming: new Map(),
    ...
    this.setGrooveTemplate(template);
  }

  public applyGrooveTemplate(template: GrooveTemplate): void {
    this.setGrooveTemplate(template);
  }

  public async tempoRamp(
    fromBPM: number,
    toBPM: number,
    duration: number,
  ): Promise<void> {
  ...
  public setTimingPrecision(precision: TimingPrecision): void {
    this.config.advancedTiming.precisionMode = precision;
    this.emitStateChange();
  }

  public setClockSource(source: ClockSource): void {
    this.config.advancedTiming.clockSource = source;
    this.emitStateChange();
  }
  ...
  public setVolume(volume: number): void {
    ...
    this.config.clickSounds.accent.volume = volume;
    this.config.clickSounds.regular.volume = volume * 0.7;
    this.config.clickSounds.subdivision.volume = volume * 0.5;
    this.emitStateChange();
  }
  ...
  private createDefaultConfig(
    ...
    if (config) {
      if (config.tempo !== undefined) defaultConfig.tempo = config.tempo;
      if (config.timeSignature !== undefined)
        defaultConfig.timeSignature = config.timeSignature;
      if (config.subdivision !== undefined)
        defaultConfig.subdivision = config.subdivision;
      if (config.accentPattern !== undefined)
        defaultConfig.accentPattern = config.accentPattern;
      if (config.grooveTemplate !== undefined)
        defaultConfig.grooveTemplate = config.grooveTemplate;
      if (config.swingAmount !== undefined)
        defaultConfig.swingAmount = config.swingAmount;

      // Deep merge for nested objects
      if (config.clickSounds) {
        Object.assign(defaultConfig.clickSounds, config.clickSounds);
      }
      if (config.visualSync) {
        Object.assign(defaultConfig.visualSync, config.visualSync);
      }
      if (config.advancedTiming) {
        Object.assign(defaultConfig.advancedTiming, config.advancedTiming);
      }
      if (config.midiSync) {
        Object.assign(defaultConfig.midiSync, config.midiSync);
      }
    }
  ...
  private createInitialState(): MetronomeState {
    return {
      isRunning: false,
      currentTempo: this.config.tempo,
      currentMeasure: 0,
      currentBeat: 0,
  ...
  private getDefaultAccentBeats(numerator: number): number[] {
    // Default: accent first beat
    if (numerator === 6) {
      return [1, 4]; // Common 6/8 pattern: strong on 1, medium on 4
    }
    return [1];
  }

  private getDefaultStrongBeats(numerator: number): number[] {
    // Default strong beats based on common patterns
    if (numerator === 4) return [1, 3];
    if (numerator === 3) return [1];
    if (numerator === 6) return [1]; // In 6/8, only beat 1 is strong, beat 4 is medium
    return [1];
  }
