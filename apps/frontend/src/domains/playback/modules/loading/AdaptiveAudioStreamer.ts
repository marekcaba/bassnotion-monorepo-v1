...
export class AdaptiveAudioStreamer implements IAdaptiveAudioStreamer {
  ...
  private formatCache = new Map<string, { format: AudioSampleFormat; optimized: ArrayBuffer }>();
   ...
   */
  static getInstance(config?: AdaptiveAudioStreamingConfig): AdaptiveAudioStreamer {
    if (!AdaptiveAudioStreamer.instance) {
      if (!config) {
        throw new Error('Config required for first instantiation of AdaptiveAudioStreamer');
      }
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      logger.error('❌ Failed to initialize Adaptive Audio Streamer:', error);
      throw error;
    }
  }
  ...
  async streamSample(
    ...
    try {
      ...
      const session = await this.createStreamingSession(sessionId, sampleId, metadata, options);
      ...
      if (this.config.enableProgressiveLoading && this.shouldUseProgressiveLoading(metadata)) {
        result = await this.streamWithProgressiveLoading(session, streamingStrategy);
      } else {
      ...
      logger.error('❌ Streaming failed:', error, { sampleId, sessionId });
  ...
  private async createStreamingSession(
    ...
    const optimalQuality = this.determineOptimalQuality(metadata, options.preferredQuality);
  ...
  private determineOptimalQuality(
    ...
    if (bandwidth > thresholds.excellent && latency < latencyThresholds.excellent) {
  ...
  private determineStreamingStrategy(session: StreamingSession): 'progressive' | 'standard' | 'optimized' {
    const { metadata } = session;
    
    // Large files benefit from progressive loading
    if (metadata.size > 5 * 1024 * 1024 && this.config.enableProgressiveLoading) {
      return 'progressive';
    }
    
    // Files that need format optimization
    if (this.config.enableFormatOptimization && this.shouldOptimizeFormat(metadata)) {
      return 'optimized';
    }
  ...
  private async streamWithProgressiveLoading(
    ...
    try {
      ...
      const initialChunks = Math.min(this.config.preloadChunks, session.totalChunks);
  ...
  private async streamStandard(
    ...
    try {
      ...
      logger.error('Standard streaming failed:', error);
      throw error;
    }
  }
  
  // Chunk management
  private async loadChunk(session: StreamingSession, chunkIndex: number): Promise<StreamingChunk> {
    ...
    try {
      ...
      const chunkEnd = Math.min(chunkStart + this.config.chunkSize, session.metadata.size);
      ...
      logger.error(`Failed to load chunk ${chunkIndex}:`, error);
      throw error;
    }
  }
  ...
  private async loadRemainingChunks(
    ...
    for (let i = 0; i < remainingIndices.length; i += maxConcurrent) {
      ...
      const batchPromises = batch.map((index) => this.loadChunk(session, index));
  ...
  // Quality adaptation
  private async shouldAdaptQuality(session: StreamingSession): Promise<boolean> {
    ...
    const bandwidthChange = Math.abs(
      currentConditions.bandwidth - sessionConditions.bandwidth
    ) / sessionConditions.bandwidth;
    
    const latencyChange = Math.abs(
      currentConditions.latency - sessionConditions.latency
  ...
  private async optimizeFormat(
    ...
    try {
      ...
      logger.info(`🗜️ Format optimized: ${session.metadata.format} → ${optimalFormat}`, {
        originalSize,
        optimizedSize,
  ...
  private getCompressionRatio(format: AudioSampleFormat): number {
    switch (format) {
      case 'wav': return 1.0; // No compression
      case 'flac': return 0.6; // Lossless compression
      case 'mp3': return 0.1; // High compression
      case 'ogg': return 0.08; // Very high compression
      default: return 0.8;
    }
  }
  ...
  private updateConnectionStability(): void {
    ...
    if (bandwidth > thresholds.good && latency < 100) {
      this.networkConditions.stability = Math.min(1.0, this.networkConditions.stability + 0.1);
    } else if (bandwidth < thresholds.poor || latency > 500) {
      this.networkConditions.stability = Math.max(0.1, this.networkConditions.stability - 0.2);
    }
  }
  
  // Performance calculation
  private calculatePerformanceMetrics(session: StreamingSession): StreamingPerformanceMetrics {
  ...
  private calculateQualityStability(session: StreamingSession): number {
    ...
    const stabilityScore = Math.max(0, 1 - session.qualityTransitions.length / maxTransitions);
    
    return stabilityScore;
  }
  
  // Utility methods
  private mapQualityToPreference(quality: AudioSampleQualityProfile): 'speed' | 'quality' | 'balanced' {
    switch (quality) {
      case 'studio':
      case 'performance':
        return 'quality';
  ...
  private mapQualityToCompressionLevel(quality: AudioSampleQualityProfile): 'low' | 'medium' | 'high' {
    switch (quality) {
      case 'studio':
      case 'performance':
        return 'low';
