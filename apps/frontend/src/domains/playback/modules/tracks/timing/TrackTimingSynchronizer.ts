...
import { TimingStateManager } from './TimingStateManager.js';
...
export class TrackTimingSynchronizer implements ITrackTimingSynchronizer {
  ...
  async initialize(transport: Transport, eventBus: EventBus): Promise<void> {
    ...
    logger.info(
      '🎯 TrackTimingSynchronizer initialized with AudioWorklet master clock',
    );
  ...
  registerTrack(trackId: string, options: TrackRegistrationOptions = {}): void {
    ...
    logger.info(
      `🎯 Registered track ${trackId} with priority ${options.priority || 50}`,
    );
  ...
  scheduleTrackEvent(
    ...
    const compensatedTime =
      options.compensate !== false
        ? this.driftCompensator.applyCompensation(
            scheduledTime,
            timingState.compensationOffset,
          )
        : scheduledTime;
  ...
  updateConfig(config: Partial<TimingSyncConfig>): void {
    ...
    if (
      config.driftTolerance !== undefined ||
  ...
  private executeTrackEvent(eventId: string, actualTime: number): void {
    ...
    // Measure drift
    const drift = this.driftCompensator.measureDrift(
      event.scheduledTime,
      actualTime,
    );
  ...
  private handleDriftViolation(trackId: string, drift: number): void {
    ...
    this.stateManager.recordError(
      trackId,
      `Drift violation: ${drift.toFixed(3)}ms`,
    );
  ...
  private applyDriftCompensation(trackId: string): void {
    ...
    const compensation =
      this.driftCompensator.calculateCompensation(timingState);
    this.stateManager.applyCompensation(trackId, compensation);

    this.eventBus?.emit('timing:compensationApplied', {
      trackId,
      offset: compensation,
      avgDrift: this.driftCompensator.calculateAverageDrift(
        timingState.driftHistory,
      ),
    });
  }
  ...
  private performSyncCheck(): void {
    ...
    const report = this.syncMonitor.generateSyncReport(
      trackStates,
      audioWorkletTime,
    );
  ...
  private getCurrentAudioWorkletTime(): number {
    ...
    if (this.transport) {
      // Use the transport's position converted to seconds
      const position = this.transport.getPosition();
      // Convert position (in bars:beats:sixteenths) to seconds
      if (this.tone?.Transport) {
        return this.tone.Transport.toSeconds(position as any);
      }
    }

    // Fallback to audio context time
    if (this.tone?.getContext) {
      return this.tone.getContext().currentTime;
    }
