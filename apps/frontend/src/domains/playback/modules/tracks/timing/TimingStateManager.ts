...
import type { TrackTimingState, TrackRegistrationOptions } from './types.js';
...
export class TimingStateManager {
  ...
  getActiveTrackStates(): TrackTimingState[] {
    return Array.from(this.trackStates.values()).filter(
  ...
  getTracksByPriority(): TrackTimingState[] {
    return Array.from(this.trackStates.values()).sort(
