  ...
  implements IPerformanceOptimizer
{
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      this.deviceCapabilities =
        await this.deviceDetector.detectDeviceCapabilities();
  ...
  async adaptQualityToDevice(): Promise<OptimizationResult> {
    if (!this.initialized || !this.currentQualitySettings) {
  ...
  getCurrentMetrics(): PerformanceMetrics {
    if (!this.performanceMetrics) {
      throw new Error(
        'Performance metrics not available - optimizer not initialized',
      );
    }
  ...
  getDeviceCapabilities(): DeviceCapabilities {
    if (!this.deviceCapabilities) {
      throw new Error(
        'Device capabilities not available - optimizer not initialized',
      );
    }
  ...
  getCurrentQualitySettings(): QualitySettings {
    if (!this.currentQualitySettings) {
      throw new Error(
        'Quality settings not available - optimizer not initialized',
      );
    }
  ...
  private calculateQualityImpact(
    ...
    if (newSettings.audio.compression !== oldSettings.audio.compression) {
      ...
      qualityDelta +=
        compressionImpact[newSettings.audio.compression] -
    ...
    if (newSettings.instruments.polyphony < oldSettings.instruments.polyphony) {
      qualityDelta -=
        (oldSettings.instruments.polyphony -
          newSettings.instruments.polyphony) *
        0.5;
    }
    if (
      newSettings.instruments.velocityLayers <
      oldSettings.instruments.velocityLayers
    ) {
      qualityDelta -=
        (oldSettings.instruments.velocityLayers -
          newSettings.instruments.velocityLayers) *
    ...
    if (
      !newSettings.processing.advancedArticulation &&
      oldSettings.processing.advancedArticulation
    ) {
      qualityDelta -= 3;
    }
    if (
      !newSettings.processing.microTiming &&
      oldSettings.processing.microTiming
    ) {
      qualityDelta -= 2;
    }
  ...
  async dispose(): Promise<void> {
    ...
    try {
      ...
      logger.error(
        '❌ Error disposing Performance Optimizer:',
        error instanceof Error ? error : new Error(String(error)),
      );
      throw error;
    }
