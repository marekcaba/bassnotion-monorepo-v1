...
export class MobileOptimizer implements IMobileOptimizer {
  ...
  async optimize(
    ...
    return {
      performanceGain,
      qualityImpact: this.calculateMobileQualityImpact(
        capabilities,
        optimizations,
      ),
      recommendations,
  ...
  private generateMobileRecommendations(
    ...
    if (capabilities.battery.temperature > 35) {
      recommendations.push(
        'Device temperature is elevated - reduce processing intensity',
      );
    ...
    if (capabilities.memory.total < 4096) {
      recommendations.push(
        'Limited memory detected - close other apps for better performance',
      );
    }
    ...
    if (capabilities.cpu.performance === 'low') {
      recommendations.push(
        'Limited CPU performance - some features may be reduced',
      );
