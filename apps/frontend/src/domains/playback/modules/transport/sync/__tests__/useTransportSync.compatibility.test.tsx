...
import { useTransportSync } from '../../../../widgets/hooks/useTransportSync';
...
vi.mock('../../../../services/core/TransportSyncManager', () => {
  return {
...
describe('useTransportSync compatibility with WidgetSyncManager', () => {
  ...
  beforeEach(() => {
    ...
    mockTransport = {
      ...
      getPosition: vi.fn().mockReturnValue({ bars: 0, beats: 0, sixteenths: 0, ticks: 0 }),
      getTempo: vi.fn().mockReturnValue(120),
      isLooping: vi.fn().mockReturnValue(false),
      getLoopStart: vi.fn().mockReturnValue({ bars: 0, beats: 0, sixteenths: 0, ticks: 0 }),
      getLoopEnd: vi.fn().mockReturnValue({ bars: 4, beats: 0, sixteenths: 0, ticks: 0 }),
  ...
  it('should handle transport play event', async () => {
    ...
    const { result } = renderHook(() => 
      useTransportSync({ widgetId, onPlay })
    );
  ...
  it('should handle transport stop event', async () => {
    ...
    const { result } = renderHook(() => 
      useTransportSync({ widgetId, onStop })
    );
