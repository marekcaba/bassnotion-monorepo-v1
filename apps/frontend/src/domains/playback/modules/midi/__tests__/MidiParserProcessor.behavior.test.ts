...
// Mock MidiCCLoader to prevent file loading errors
vi.mock('../loaders/MidiCCLoader.js', () => ({
  MidiCCLoader: {
    getInstance: vi.fn(() => ({
      loadCCConfig: vi.fn().mockResolvedValue({
        version: '1.0',
        mappings: {},
        categories: {},
        aliases: {},
      ...
      getCCMapping: vi.fn(),
      getCCDescription: vi.fn().mockReturnValue('Mock CC'),
      normalizeCCValue: vi.fn((mapping, value) => value / 127),
      getCCSwitchState: vi.fn(),
      getCCsByCategory: vi.fn(),
      findCCByAlias: vi.fn(),
      clearCache: vi.fn(),
    })),
  },
...
describe('MidiParserProcessor', () => {
  ...
  describe('Initialization', () => {
    ...
    it('should handle WebMidi initialization errors gracefully', async () => {
      ...
      // Since initialization happens async and logger is used instead of console.warn,
      // we just verify that construction doesn't throw
      expect(() => new MidiParserProcessor()).not.toThrow();

      // Wait for the async initialization to complete
      await new Promise((resolve) => setTimeout(resolve, 50));

      // The error is handled gracefully internally
    });
  });
