...
export class MusicalFeatureExtractor {
  ...
  async extractFeatures(
    ...
    const genreFeatures = this.extractGenreFeatures(
      channelData,
      onsets,
      harmonicRatio,
    );
    ...
    const instrumentFeatures = this.extractInstrumentFeatures(
      channelData,
      context,
    );
    const instrumentClassification =
      this.classifyInstrument(instrumentFeatures);
   ...
   * Analyze energy distribution
   */
  private analyzeEnergyDistribution(
    channelData: Float32Array,
  ): MusicalFeatures['energyDistribution'] {
    ...
    // Analyze envelope
    const attackFrames = energyFrames.slice(
      0,
    ...
    const sustain =
      sustainFrames.length > 0
        ? sustainFrames.reduce((sum, e) => sum + e, 0) /
    ...
    const decay =
      decayFrames.length > 0
        ? 1 -
          decayFrames.reduce((sum, e) => sum + e, 0) /
