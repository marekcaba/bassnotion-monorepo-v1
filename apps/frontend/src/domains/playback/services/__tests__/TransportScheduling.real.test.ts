...
describe('Transport.scheduleRepeat - Real Behavior Tests', () => {
  ...
  it('should execute callbacks repeatedly when Transport is running', async () => {
    const callbacks: Array<{ time: number; iteration: number; position: string }> = [];
    let iteration = 0;
    ...
    const scheduleId = Tone.Transport.scheduleRepeat((time) => {
      ...
      console.log(`Callback ${iteration}: time=${time}, position=${Tone.Transport.position}`);
    }, '8n', 0);
  ...
  it('should continue scheduling after clearing and recreating', async () => {
    ...
    // First schedule
    let scheduleId = Tone.Transport.scheduleRepeat((time) => {
      callbacks.push(time);
    }, '8n', 0);
    ...
    console.log(`Callbacks before clear: ${callbacksBeforeClear}, total: ${callbacks.length}`);
  });
  ...
  it('should handle Transport state changes correctly', async () => {
    ...
    const callbacksDuringPause = callbacks.filter(
      (_, index) => index >= callbacksBeforePause && callbacks[index].transportState === 'paused'
    ).length;
  ...
  it('reproduces the exact issue from the app', async () => {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      console.log(`After ${(i + 1) * 400}ms: ${logs.length} callbacks, Transport.state: ${Tone.Transport.state}`);
    }
