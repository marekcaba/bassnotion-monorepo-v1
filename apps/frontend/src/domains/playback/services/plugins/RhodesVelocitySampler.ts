...
export class RhodesVelocitySampler {
  ...
  private async ensureToneLoaded(): Promise<void> {
    if (!Tone) {
      ...
      console.log('🎵 Using global Tone.js instance in RhodesVelocitySampler');
    }
  }
  ...
  async initialize(): Promise<void> {
    ...
    console.log('🎹 Initializing Fender Rhodes...');
    ...
    try {
      ...
      console.log('✅ Rhodes ready with initial layers');
    } catch (error) {
      console.error('Failed to initialize Rhodes:', error);
      throw error;
    }
  }
  ...
  private async loadLayer(layer: string): Promise<void> {
    ...
    console.log(`Loading Rhodes velocity layer ${layer}...`);
    ...
    const loadPromise = (async () => {
      try {
        ...
        const sampler = new Tone.Sampler({
          ...
          attack: 0.005,
          onload: () => {
            console.log(`✅ Rhodes ${layer} loaded successfully`);
          },
          onerror: (error) => {
            console.error(`❌ Error loading Rhodes ${layer}:`, error);
            throw error;
          },
        });
        ...
        console.log(`Rhodes ${layer} loaded, testing sample availability...`);

        // Test if we can access a sample
        try {
          const hasC4 = sampler.has('C4');
          console.log(`Rhodes ${layer} has C4: ${hasC4}`);
        } catch (e) {
          console.log(`Rhodes ${layer} sample test failed:`, e);
        }
        ...
        console.error(`❌ Failed to load Rhodes layer ${layer}:`, error);
        throw error;
      } finally {
  ...
  async triggerAttackRelease(
    ...
    if (!this.isInitialized) {
      console.warn('Rhodes not initialized');
      return;
    }
    ...
    this.samplers.forEach((sampler) => {
      if (sampler && sampler.loaded) {
        try {
          ...
          console.warn('Failed to release notes on Rhodes sampler:', error);
        }
      }
    });
  }
  ...
  dispose(): void {
    ...
    console.log('🗑️ Disposed Rhodes sampler');
  }
}
