...
export class WurlitzerVelocitySampler {
  ...
  private async ensureToneLoaded(): Promise<void> {
    if (!Tone) {
      ...
      console.log('🎵 Using global Tone.js instance in WurlitzerVelocitySampler');
    }
  }
  ...
  async initialize(): Promise<void> {
    ...
    console.log('🎹 Initializing Wurlitzer Electric Piano...');
    ...
    try {
      ...
      console.log('✅ Wurlitzer ready with initial layers');
    } catch (error) {
      console.error('Failed to initialize Wurlitzer:', error);
      throw error;
    }
  }
    ...
    try {
      ...
      await Promise.all([
        this.pedalSampler
          ...
          .catch((e) => console.error('Failed to load pedal-press.mp3:', e)),
        this.pedalReleaseSampler
          .load(
            `${this.supabaseUrl}/Keyboards/wurlitzer/pedal/pedal-release.mp3`,
          )
          .catch((e) => console.error('Failed to load pedal-release.mp3:', e)),
      ]);
      ...
      for (const note of this.velocityInfo.keys()) {
        ...
        keyPressPromises.push(
          player
            .load(url)
            .catch((e) =>
              console.warn(`Failed to load clean key for ${note}:`, e),
            ),
        );
      }
      ...
      console.error('Error in loadMechanicalSounds:', error);
  ...
  private async loadInitialVelocityLayers(): Promise<void> {
    ...
    for (const [note, velocityCount] of this.velocityInfo) {
      ...
      const sampler = new Tone.Sampler({
        ...
        attack: 0.002,
        onload: () => {
          console.log(`✅ Wurlitzer ${note} v${middleVelocity} loaded`);
        },
        onerror: (error) => {
          console.error(
  ...
  private async loadVelocityLayer(
    ...
    const sampler = new Tone.Sampler({
      ...
      attack: 0.002,
      onload: () => {
        console.log(`✅ Wurlitzer ${note} v${velocityIndex} loaded on demand`);
      },
      onerror: (error) => {
        console.error(
    ...
    for (let i = 0; i < notes.length; i++) {
      ...
      if (sampler && sampler.loaded) {
        try {
          ...
          console.error(`Error playing note ${n}:`, error);
        }
        ...
        if (
          ...
          try {
            ...
            if (
              ...
              console.warn('[Mechanical] No destination for key release sound');
            }
          } catch (error) {
            console.error(
  ...
  triggerPedalPress(time?: Tone.Unit.Time): void {
    try {
      if (
        ...
        console.warn('Pedal press sampler not loaded or no destination');
      }
    } catch (error) {
      console.warn('Error playing pedal press:', error);
    }
  }
  ...
  triggerPedalRelease(time?: Tone.Unit.Time): void {
    try {
      if (
        ...
        console.warn('Pedal release sampler not loaded or no destination');
      }
    } catch (error) {
      console.warn('Error playing pedal release:', error);
    }
  }
      ...
      samplers.forEach((sampler) => {
        if (sampler && sampler.loaded) {
          try {
            ...
            console.warn('Failed to release notes on sampler:', error);
          }
        }
      });
    }
