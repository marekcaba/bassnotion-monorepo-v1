...
export class TrackPluginManager {
  ...
  public initialize(tone: typeof Tone): void {
    this.tone = tone;
    console.log('🔌 TrackPluginManager: Initialized');
  }
  ...
  public registerPluginFactory(
    ...
    console.log(`🔌 TrackPluginManager: Registered factory for ${pluginType}`);
  }
  ...
  public async createPluginForTrack(
    ...
    if (pooledInstance) {
      console.log(`🔌 TrackPluginManager: Reusing pooled ${pluginType} for track ${trackId}`);
    ...
    console.log(`🔌 TrackPluginManager: Created ${pluginType} instance ${instanceId} for track ${trackId}`);
  ...
  public reorderTrackPlugins(
    ...
    const validPlugins = newOrder.filter(id => 
      chain.plugins.includes(id)
    );
  ...
  public optimizeResources(): void {
    ...
    for (const [instanceId, instance] of this.pluginInstances.entries()) {
      if (!instance.isActive && 
          now - instance.lastUsed > maxIdleTime) {
        ...
        console.log(`🔌 TrackPluginManager: Disposed idle instance ${instanceId}`);
      }
    }
  ...
  private getPooledInstance(
    ...
    const available = Array.from(this.pluginInstances.values())
      .find(instance => 
  ...
  private canPoolInstance(instance: PluginInstance): boolean {
    ...
    const pooledCount = Array.from(this.pluginInstances.values())
      .filter(inst => 
        inst.plugin.type === instance.plugin.type && 
        !inst.isActive
      ).length;
  ...
  private async poolInstance(instance: PluginInstance): Promise<void> {
    ...
    console.log(`🔌 TrackPluginManager: Pooled instance ${instance.instanceId}`);
  }
    ...
    for (const instance of this.pluginInstances.values()) {
      if (instance.isActive) {
        ...
        switch (instance.plugin.type) {
          case 'reverb': cpuEstimate += 5; break;
          case 'delay': cpuEstimate += 3; break;
          case 'compressor': cpuEstimate += 2; break;
          default: cpuEstimate += 1;
        }
      }
    }
  ...
  public async dispose(): Promise<void> {
    ...
    console.log('🔌 TrackPluginManager: Disposed');
  }
}