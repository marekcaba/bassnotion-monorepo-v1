...
export class WamDeviceOptimizer {
  ...
  private readonly qualityPresets: Map<string, PluginQualityPreset> = new Map([
    ...
    ['low', {
      ...
      disabledFeatures: ['convolution', 'granular', 'reverb-tails', 'modulation']
    }]
  ]);
  ...
  private constructor() {
    ...
    try {
      this.deviceCapabilityManager = serviceRegistry.get<DeviceCapabilityManager>('deviceCapabilityManager');
      this.performanceOptimizer = serviceRegistry.get<PerformanceOptimizer>('performanceOptimizer');
  ...
  startMonitoring(): void {
    ...
    console.log('✅ WAM Device Optimizer monitoring started');
  }
  ...
  handleOverload(): void {
    ...
    console.warn('⚠️ Performance overload detected, applying optimizations...');
    ...
    if (profile.reduceSampleRateOnOverload && this.state.cpuUsage > profile.maxCpuUsage) {
  ...
  recoverFromOverload(): void {
    ...
    console.log('✅ Performance recovered, restoring settings...');
  ...
  setProfile(profileName: string): void {
    ...
    console.log(`Device profile set to: ${profileName}`);
  }
  
  // Private methods
  
  private selectOptimalProfile(): void {
    if (!this.deviceCapabilityManager) {
    ...
    console.log(`Selected device profile: ${this.state.currentProfile.name}`);
    console.log(`Selected quality preset: ${this.state.currentQuality.name}`);
  }
  ...
  private monitorPerformance(): void {
    ...
    if (performance.memory) {
      this.state.memoryUsage = performance.memory.usedJSHeapSize / (1024 * 1024);
    }
  ...
  private reduceQuality(): void {
    ...
    if (currentIndex < qualities.length - 1) {
      ...
      console.log(`Reduced quality to: ${newQuality.name}`);
  ...
  private increaseQuality(): void {
    ...
    if (currentIndex > 0) {
      ...
      console.log(`Increased quality to: ${newQuality.name}`);
  ...
  private disableNonEssentialEffects(): void {
    ...
    for (const registration of registrations) {
      ...
      if (!registration.descriptor.isInstrument) {
        ...
        console.log(`Disabled non-essential effect: ${registration.moduleId}`);
      }
    }
  }
  ...
  private reduceSampleRate(): void {
    ...
    console.warn('Sample rate reduction requested (not implemented)');
  }
