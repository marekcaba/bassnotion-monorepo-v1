...
export class WamBassNode implements WamNode {
  ...
  async initialize(): Promise<void> {
    ...
    if (typeof window !== 'undefined' && this.context) {
      ...
      if (Tone) {
        console.log('🎸 WamBassNode: Setting Tone.js to use WAM AudioContext during initialization');
        try {
  ...
  private async initializePlaceholderSynth(): Promise<void> {
    ...
    try {
      ...
      console.log('✅ Placeholder bass synth initialized (waiting for real samples)');
    } catch (error) {
      console.error('Failed to initialize placeholder synth:', error);
    }
  }
  ...
  async loadSamples(): Promise<void> {
    try {
      ...
      */
      ...
      console.log('🎸 Bass plugin ready - waiting for samples to be uploaded to Supabase');
      ...
      console.error('Failed to load bass samples:', error);
    }
  }
   ...
   * Trigger a bass note
   */
  triggerNote(note: number, velocity: number = 80, articulation?: BassArticulation): void {
   ...
   */
  private triggerPlaceholder(note: number, velocity: number, articulation: BassArticulation): void {
    ...
    try {
      ...
      this.placeholderSynth.triggerAttack(freq, this.context.currentTime, volume);
    } catch (error) {
      console.error('Failed to trigger placeholder note:', error);
    }
  }
   ...
   */
  private triggerSample(note: number, velocity: number, articulation: BassArticulation): void {
  ...
  async getParameterValues(): Promise<WamParameterDataMap> {
    return {
      ...
      articulation: Object.values(BassArticulation).indexOf(this.currentArticulation),
      tone: 0.5, // TODO: Implement tone control
      compression: this.compressor ? (this.compressor.ratio.value - 1) / 19 : 0.3
    };
  }
  ...
  private handleMidiCommand(command: number, note: number, velocity: number): void {
    switch (command) {
      case 0x90: // Note On
        if (velocity > 0) {
...
export class WamBass extends WebAudioModuleBase {
   ...
   * Play a bass note
   */
  playNote(note: number, velocity: number = 80, duration?: number): void {
   ...
   */
  playTab(string: number, fret: number, velocity: number = 80, duration?: number): void {
    // Calculate MIDI note from string and fret
    const openStrings = [BASS_TUNING.E, BASS_TUNING.A, BASS_TUNING.D, BASS_TUNING.G];
  ...
  async createGui(): Promise<HTMLElement> {
    ...
    // Wire up controls
    const articulationSelect = container.querySelector('#articulation') as HTMLSelectElement;
    ...
    const compressionSlider = container.querySelector('#compression') as HTMLInputElement;
