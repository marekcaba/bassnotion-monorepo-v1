...
export class WamHostManager {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      console.log('✅ WAM Host Manager initialized');
    } catch (error) {
  ...
  async registerPlugin(
    ...
    if (this.registeredPlugins.has(moduleId)) {
      console.warn(`WAM plugin ${moduleId} already registered`);
      return;
    }
    ...
    console.log(`✅ WAM plugin registered: ${moduleId}`);
  }
  ...
  async createPluginInstance(
    ...
    try {
      ...
      const instanceId = `${moduleId}-${trackId}-${Date.now()}`;
  ...
  async removePluginInstance(instanceId: string): Promise<void> {
    ...
    if (trackPlugins) {
      trackPlugins.delete(instanceId);
    ...
    // Update registration count
    const moduleId = instance.adapter.metadata.id;
    const registration = this.registeredPlugins.get(moduleId);
    if (registration) {
      registration.instanceCount--;
    }
    ...
    this.latencyCompensation.delete(instanceId);
  ...
  updatePerformanceMetrics(
    ...
    if (!metrics) {
      ...
      metrics.averageProcessingTime = metrics.totalProcessingTime / metrics.sampleCount;
    ...
    return Array.from(this.registeredPlugins.values()).filter(reg => {
      ...
      if (tags && tags.length > 0) {
        const hasTag = tags.some(tag => 
          descriptor.keywords.includes(tag)
        );
  ...
  async dispose(): Promise<void> {
    ...
    console.log('🗑️ WAM Host Manager disposed');
  }
  
  // Private helper methods
  
  private async preloadPlugins(): Promise<void> {
    const preloadPromises = this.config.preloadPlugins.map(async pluginUrl => {
      try {
        const moduleId = this.extractModuleId(pluginUrl);
        await this.registerPlugin(moduleId, pluginUrl);
      } catch (error) {
        console.warn(`Failed to preload WAM plugin ${pluginUrl}:`, error);
      }
    });
  ...
  private updateDeviceCapabilities(): void {
    ...
    // Update host capabilities based on device
    this.hostCapabilities.maxPluginsPerTrack = capabilities.performance.recommendedMaxTracks;
    this.hostCapabilities.maxChannelCount = capabilities.audio.maxChannelCount;
    this.hostCapabilities.supportsMultiChannel = capabilities.audio.maxChannelCount > 2;
