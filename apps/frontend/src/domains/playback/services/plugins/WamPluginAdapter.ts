...
import { UnifiedTransport, type MusicalPosition } from '../core/UnifiedTransport.js';
...
export class WamPluginAdapter implements AudioPlugin {
  ...
  constructor(
    ...
    // Initialize config
    this.config = {
      id: `wam-${wamDescriptor.vendor}-${wamDescriptor.name}`.toLowerCase().replace(/\s+/g, '-'),
      name: wamDescriptor.name,
      version: wamDescriptor.version,
      category: wamDescriptor.isInstrument ? PluginCategory.INSTRUMENT : PluginCategory.EFFECT,
      enabled: true,
  ...
  async load(): Promise<void> {
    ...
    try {
      ...
      if (!WamConstructor.isWebAudioModuleConstructor) {
        throw new Error('Invalid WAM module: missing isWebAudioModuleConstructor');
      }
      ...
      console.log(`✅ WAM plugin loaded: ${this.metadata.name}`);
    } catch (error) {
      this._state = PluginState.ERROR;
      throw new PlaybackError(
        `Failed to load WAM plugin: ${error}`,
        'WAM_LOAD_FAILED',
        ErrorSeverity.HIGH
      );
    }
  ...
  async initialize(context: PluginAudioContext): Promise<void> {
    ...
    try {
      ...
      // Get initial latency
      this.timingConfig.reportedLatency = await this.wamNode.getCompensationDelay();
      ...
      console.log(`✅ WAM plugin initialized: ${this.metadata.name}`);
    } catch (error) {
      this._state = PluginState.ERROR;
      throw new PlaybackError(
        `Failed to initialize WAM plugin: ${error}`,
        'WAM_INIT_FAILED',
        ErrorSeverity.HIGH
      );
    }
  ...
  async activate(): Promise<void> {
    if (this._state !== PluginState.INACTIVE && this._state !== PluginState.ACTIVE) {
      throw new PlaybackError(
  ...
  async dispose(): Promise<void> {
    ...
    console.log(`🗑️ WAM plugin disposed: ${this.metadata.name}`);
  }
  ...
  async process(
    ...
    return {
      success: true,
      status: this.config.bypassMode ? ProcessingResultStatus.BYPASSED : ProcessingResultStatus.SUCCESS,
      processingTime,
  ...
  async setParameter(parameterId: string, value: unknown): Promise<void> {
    ...
    if (!mapping) {
      throw new PlaybackError(
        `Unknown parameter: ${parameterId}`,
        'WAM_UNKNOWN_PARAMETER',
        ErrorSeverity.LOW
      );
    }
  ...
  async loadPreset(preset: Record<string, unknown>): Promise<void> {
    ...
    if (preset.parameterValues) {
      await this.wamNode.setParameterValues(preset.parameterValues as WamParameterDataMap);
    }
  }
  ...
  async createGui(): Promise<Element | null> {
    ...
    try {
      ...
      console.warn(`WAM plugin ${this.metadata.name} does not provide GUI`);
      return null;
    }
  }
  ...
  private createMetadataFromDescriptor(descriptor: WamDescriptor): PluginMetadata {
    return {
      id: `wam-${descriptor.vendor}-${descriptor.name}`.toLowerCase().replace(/\s+/g, '-'),
      name: descriptor.name,
      version: descriptor.version,
      vendor: descriptor.vendor,
      description: `${descriptor.name} by ${descriptor.vendor}`,
      category: descriptor.isInstrument ? PluginCategory.INSTRUMENT : PluginCategory.EFFECT,
  ...
  private createCapabilitiesFromDescriptor(descriptor: WamDescriptor): PluginCapabilities {
    return {
  ...
  private startTransportSync(): void {
    ...
    this.transport.on('stateChange', this.handleTransportStateChange.bind(this));
  }
  
  private stopTransportSync(): void {
    this.transport.off('position', this.handleTransportPosition.bind(this));
    this.transport.off('stateChange', this.handleTransportStateChange.bind(this));
  }
  
  private handleTransportPosition(position: MusicalPosition & { seconds: number }): void {
    ...
    const event: WamTransportEvent = {
      ...
      data: {
        ...
        hostCurrentBarStarted: position.seconds - (position.beats * 60 / this.transport.getTempo()),
