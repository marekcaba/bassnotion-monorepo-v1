...
import type { WamDescriptor, WebAudioModule, WamNode } from '../../../types/wam.js';
...
describe('WamPluginAdapter', () => {
  ...
  beforeEach(() => {
    ...
    mockWamNode = {
      ...
      getParameterValues: vi.fn().mockResolvedValue({ gain: 0.5, frequency: 440 }),
      getState: vi.fn().mockResolvedValue({ parameterValues: {}, internalState: {} }),
  ...
  describe('lifecycle', () => {
    ...
    it('should initialize with audio context', async () => {
      ...
      expect(mockEventBus.emit).toHaveBeenCalledWith('wam:initialized', expect.any(Object));
    });
  ...
  describe('error handling', () => {
    ...
    it('should handle initialization failures', async () => {
      ...
      mockWamConstructor.createInstance.mockRejectedValue(new Error('Init failed'));
      ...
      await expect(adapter.initialize(context)).rejects.toThrow('Failed to initialize WAM plugin');
      expect(adapter.state).toBe(PluginState.ERROR);
    });
    ...
    it('should validate state transitions', async () => {
      ...
      await expect(adapter.initialize(context)).rejects.toThrow('Plugin must be loaded before initialization');
    });
  });
