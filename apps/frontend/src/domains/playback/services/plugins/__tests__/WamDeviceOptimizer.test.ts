...
describe('WamDeviceOptimizer', () => {
  ...
  const createMockRegistration = (moduleId: string, isInstrument = false): WamPluginRegistration => ({
    moduleId,
  ...
  beforeEach(() => {
    ...
    vi.mocked(serviceRegistry.get).mockImplementation((name) => {
      if (name === 'deviceCapabilityManager') return mockDeviceCapabilityManager;
  ...
  describe('plugin optimization', () => {
    it('should apply quality settings to plugin', () => {
      ...
      expect(plugin.setParameter).toHaveBeenCalledWith('oversampling', expect.any(Number));
      expect(plugin.setParameter).toHaveBeenCalledWith('bufferSize', expect.any(Number));
      expect(plugin.setParameter).toHaveBeenCalledWith('quality', expect.any(String));
      
      expect(mockEventBus.emit).toHaveBeenCalledWith('wam:optimizer:plugin-optimized', {
    ...
    it('should disable features based on quality preset', () => {
      ...
      expect(plugin.setParameter).toHaveBeenCalledWith('disable_convolution', true);
      expect(plugin.setParameter).toHaveBeenCalledWith('disable_reverb-tails', true);
    });
  });
  ...
  describe('performance monitoring', () => {
    ...
    it('should detect overload conditions', () => {
      ...
      expect(mockEventBus.emit).toHaveBeenCalledWith('wam:optimizer:overload', expect.any(Object));
    });
  ...
  describe('overload handling', () => {
    it('should reduce quality on overload', () => {
      ...
      expect(mockEventBus.emit).toHaveBeenCalledWith('wam:optimizer:quality-changed', {
        quality: 'medium'
      });
    });
