...
describe('WamHostManager', () => {
  ...
  describe('plugin instance management', () => {
    ...
    it('should create plugin instance', async () => {
      ...
      expect(mockWamGroup.addWam).toHaveBeenCalledWith({
    ...
    it('should enforce track plugin limit', async () => {
      ...
      const customHostManager = WamHostManager.getInstance({
        maxPluginsPerTrack: 2,
      });
      await customHostManager.initialize();
      await customHostManager.registerPlugin(
        'test-synth',
        '/plugins/test-synth',
        testDescriptor,
      );
  ...
  describe('plugin search and discovery', () => {
    ...
    it('should combine category and tag search', () => {
      const analogSynths = hostManager.searchPlugins(
  ...
  describe('plugin connections', () => {
    ...
    beforeEach(async () => {
      ...
      // Create instances
      vi.mocked(WamPluginAdapter).mockImplementationOnce(
        () => mockAdapter1 as any,
      );
