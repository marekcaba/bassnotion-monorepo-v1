...
import { UnifiedTransport, type MusicalPosition, type TransportPosition, type TimingMetrics } from './UnifiedTransport.js';
 ...
 */
export class MultiTrackTimingSynchronizer {
  // Singleton instance
  ...
  public static getInstance(): MultiTrackTimingSynchronizer {
    if (!MultiTrackTimingSynchronizer.instance) {
      MultiTrackTimingSynchronizer.instance = new MultiTrackTimingSynchronizer();
    }
  ...
  private initialize(): void {
    ...
    this.transport.on('position', (position: TransportPosition) => {
      this.handleTransportPosition(position);
    });
    
    // Start sync monitoring
    this.startSyncMonitoring();
    
    console.log('🎯 MultiTrackTimingSynchronizer: Initialized with AudioWorklet master clock');
  }
   ...
   */
  public registerTrack(track: Track, priority: number = 50): void {
    ...
    console.log(`🎯 Registered track ${track.id} with priority ${priority}`);
  ...
  public scheduleTrackEvent(
    ...
    const compensatedTime = scheduledTime + (timingState.compensationOffset / 1000);
  ...
  private handleDriftViolation(trackId: string, drift: number): void {
    ...
    console.warn(`⚠️ Track ${trackId} drift violation: ${drift.toFixed(3)}ms`);
  ...
  private applyDriftCompensation(trackId: string): void {
    ...
    const avgDrift = timingState.driftHistory.reduce((a, b) => a + b, 0) / timingState.driftHistory.length;
    
    // Apply compensation (negative of average drift)
    timingState.compensationOffset = -avgDrift;
    
    console.log(`🎯 Applied drift compensation to track ${trackId}: ${timingState.compensationOffset.toFixed(3)}ms`);
  }
  ...
  private isolateTrack(trackId: string): void {
    ...
    console.error(`❌ Track ${trackId} isolated due to timing errors`);
  ...
  private handleTrackError(trackId: string, error: any): void {
    ...
    console.error(`Track ${trackId} execution error:`, error);
  ...
  private performSyncCheck(): void {
    ...
    if (report.warnings.length > 0) {
      console.warn('🎯 Sync warnings:', report.warnings);
    }
  }
  ...
  private calculateTrackMetrics(
    ...
    const errorRate = timingState.errorCount / Math.max(1, driftHistory.length) * 100;
  ...
  private calculateOverallDrift(metrics: TrackSyncMetrics[]): number {
    ...
    const totalDrift = metrics.reduce((sum, m) => sum + Math.abs(m.avgDrift), 0);
  ...
  private generateWarnings(metrics: TrackSyncMetrics[]): string[] {
    ...
    for (const metric of metrics) {
      if (Math.abs(metric.avgDrift) > this.DRIFT_TOLERANCE) {
        warnings.push(`Track ${metric.trackId} exceeds drift tolerance: ${metric.avgDrift.toFixed(3)}ms`);
      }
      
      if (metric.stability < 80) {
        warnings.push(`Track ${metric.trackId} has unstable timing: ${metric.stability.toFixed(1)}% stability`);
      }
      
      if (metric.errorRate > 10) {
        warnings.push(`Track ${metric.trackId} has high error rate: ${metric.errorRate.toFixed(1)}%`);
      }
  ...
  public resetTrackErrors(trackId: string): void {
    ...
    if (timingState) {
      ...
      console.log(`🎯 Reset errors for track ${trackId}`);
    }
  }
  ...
  public dispose(): void {
    ...
    console.log('🎯 MultiTrackTimingSynchronizer: Disposed');
  }
}