...
export class RegionProcessor {
  ...
  private scheduledAudioSources = new Map<AudioBufferSourceNode, { type: 'one-shot' | 'sustained'; hasStopScheduled: boolean }>();

  // Audio buffers for instruments (will be injected)
  ...
  // FAANG FIX: Current harmony instrument type ('grandpiano', 'wurlitzer', etc.)
  // Prevents race conditions where grandPianoKeyboardMap hasn't loaded yet
  private grandPianoKeyboardMap: Record<string, any> | null = null;
  // Grand Piano keyboard map (A0-C8) with pre-calculated pitch shifts
  ...
  private activeHarmonySources = new Map<string, Array<{ source: AudioBufferSourceNode; gain: GainNode; gainValue: number; noteEndTime: number }>>();
  ...
  // FAANG COUNTDOWN SOLUTION: Offset all events by N beats to allow countdown
  private countdownOffsetBeats = 0; // Will be set to time signature numerator (e.g., 4 for 4/4)
  private countdownEnabled = true; // Can be disabled if needed
  ...
  constructor(eventBus: EventBus) {
    ...
    logger.info('🔧 RegionProcessor instance created', { instanceId: this._instanceId });
    this.debugger.log('RegionProcessor', 'initialized');

    // Listen for tempo changes and reschedule events
    // TEMPO CHANGE FIX: Debounce rapid tempo changes to prevent UI freezing
    this.eventBus.on('transport:tempo-change', (data: { tempo: number; bpm: number }) => {
      ...
      if (!this.isRunning) {
        logger.info('⚠️ RegionProcessor: Tempo changed while stopped - will apply on next play', {
          newTempo,
          instanceId: this._instanceId
        });
        return;
      ...
      if (this.tempoChangeDebounce) {
        ...
        logger.debug('🎵 RegionProcessor: Debouncing tempo change', { newTempo });
      }
  ...
  setMetronomeBuffers(accent: AudioBuffer, click: AudioBuffer, destination: AudioNode): void {
    ...
    this.metronomeBuffers.click = click;
    this.audioDestination = destination;
    logger.info('✅ FAANG: Metronome buffers injected', {
      hasAccent: !!accent,
      hasClick: !!click,
  ...
  setDrumBuffers(kick: AudioBuffer, snare: AudioBuffer, hihat: AudioBuffer, destination: AudioNode): void {
    this.drumBuffers.kick = kick;
    this.drumBuffers.snare = snare;
    this.drumBuffers.hihat = hihat;
    this.audioDestination = destination;
    logger.info('✅ FAANG: Drum buffers injected', {
      hasKick: !!kick,
      hasSnare: !!snare,
      hasHihat: !!hihat,
      hasDestination: !!destination,
  ...
  setVoiceCueBuffers(samples: Map<string, AudioBuffer>, destination: AudioNode): void {
    ...
    this.audioDestination = destination;
    logger.info('✅ FAANG: Voice cue buffers injected', {
  ...
  async setHarmonyBuffers(
    ...
    // samples is a flat map: { 'v10-D4': AudioBuffer, 'v10-F#4': AudioBuffer, ... }
    // We need to organize by layer → note
    ...
    if (this.harmonyBuffers && this.harmonyBuffers.size > 0) {
      console.log('[REGION-PROCESSOR] 🗑️ Clearing old harmony buffers', {
    ...
    logger.info(`🎹 Setting harmony instrument type: ${instrument || 'unknown'}`);
    ...
    if (instrument && instrument !== 'grandpiano') {
      ...
      logger.info(`🎹 Cleared Grand Piano keyboard map (loading ${instrument})`);
    }
    ...
    if (instrument === 'grandpiano' && isSparseSampled && !this.grandPianoKeyboardMap) {
    ...
    logger.info('✅ FAANG: Harmony buffers injected', {
    ...
    console.log('[HARMONY-BUFFERS-SET] 🎹 Final state:', {
      ...
      hasKeyboardMap: !!this.grandPianoKeyboardMap,
      keyboardMapSize: this.grandPianoKeyboardMap ? Object.keys(this.grandPianoKeyboardMap).length : 0,
      sampleFirstLayer: this.harmonyBuffers.size > 0 ? Array.from(this.harmonyBuffers.values())[0].size : 0,
      instanceId: this._instanceId
    });
  }
  ...
  private async loadGrandPianoKeyboardMap(): Promise<void> {
    try {
      console.log('🗺️ [KEYBOARD-MAP-LOAD] RegionProcessor attempting to load keyboard map...');

      // Try cache first (populated by HarmonyPreloadStrategy during preload)
      const cached = GlobalSampleCache.getInstance().getCachedMetadata('grandpiano-keyboard-map');
      ...
      if (cached) {
        ...
        logger.info('✅ Retrieved Grand Piano keyboard map from cache (88 keys A0-C8)', {
      ...
      console.log('🗺️ [KEYBOARD-MAP-LOAD] ✅ Loaded from import', {
        totalKeys: this.grandPianoKeyboardMap ? Object.keys(this.grandPianoKeyboardMap).length : 0,
        hasKeyboardMap: !!this.grandPianoKeyboardMap,
      });
      logger.info('✅ Loaded Grand Piano keyboard map from import (88 keys A0-C8)', {
        source: 'direct-import',
        note: 'Cache miss - this is expected if setHarmonyBuffers called before preload completes',
      });
    } catch (error) {
      console.error('🗺️ [KEYBOARD-MAP-LOAD] ❌ Error loading keyboard map:', error);
  ...
  setBassBuffers(samples: Map<string, AudioBuffer>, destination: AudioNode): void {
    // samples is a flat map: { 'normal-D2': AudioBuffer, 'slap-D2': AudioBuffer, ... }
    // We need to organize by articulation → note
    ...
    logger.info('✅ FAANG: Bass buffers injected', {
      articulations,
      totalNotes: totalBuffers,
      hasDestination: !!destination,
      instanceId: this._instanceId
  ...
  private getWamKeyboard(): WamKeyboard | null {
    if (!this.pluginManager) {
      logger.warn('PluginManager not set in RegionProcessor - cannot access WamKeyboard');
      return null;
    }

    try {
      // Get the WamKeyboardPlugin wrapper
      const keyboardPlugin = this.pluginManager.getPlugin<WamKeyboardPlugin>('wam-keyboard');
      if (!keyboardPlugin) {
      ...
      if (!wamKeyboard) {
        logger.warn('WamKeyboard instance not yet initialized in plugin wrapper');
        return null;
      }
  ...
  registerTracks(tracks: Track[]): void {
    ...
    tracks.forEach(track => {
      ...
      if (instrumentType) {
        ...
        if (existingTrackId) {
          const exerciseChanged = existingTrack?.exerciseId !== track.exerciseId;
          ...
          if (instrumentType === 'harmony') {
            ...
            logger.info('✅ Cleared harmony-specific state during track replacement');
          }
      ...
      logger.info(`Registered track: ${trackId} with ${track.regions.length} regions`);
    ...
    const registeredHarmonyTracks = Array.from(this.tracks.values()).filter(t => t.instrumentType === 'harmony');
    if (registeredHarmonyTracks.length > 1) {
      console.error('[ARCHITECTURE-ERROR] Multiple harmony tracks detected:',
      ...
      logger.error('CRITICAL: Multiple harmony tracks registered simultaneously', {
        count: registeredHarmonyTracks.length,
        tracks: registeredHarmonyTracks.map(t => ({ id: t.id, exerciseId: t.exerciseId }))
      });
  ...
  start(): void {
    ...
    if (!this.audioContext && Tone.context) {
      logger.warn('⚠️ AudioContext not set via setAudioContext(), using Tone.context as fallback', {
        instanceId: (this as any)._instanceId || 'unknown',
        hasBuffers: !!(this.metronomeBuffers.accent && this.metronomeBuffers.click),
    ...
    if (this.audioContext) {
      ...
      const startupLookahead = 0.20; // 200ms - ensures first beat has adequate scheduling time
      this.transportStartTime = this.audioContext.currentTime + startupLookahead;

      logger.info('🎯 Transport start anchor captured with FAANG startup lookahead', {
      ...
      logger.error('❌ CRITICAL: No AudioContext available! Time domain conversion will fail completely.');
    }
    ...
    logger.info('🎵 RegionProcessor.start() - Checking Tone.Transport BPM before scheduling', {
    ...
    if (Tone.Transport.loop) {
      logger.warn('⚠️ Tone.Transport.loop was enabled - disabling to prevent double playback', {
  ...
  stop(graceful = false): void {
    ...
    try {
      ...
      logger.error('🎵 RegionProcessor: Failed to cancel Tone.Transport events', e);
    }
    ...
    if (graceful) {
      ...
      logger.info('🎵 GRACEFUL STOP: Allowing one-shot samples to finish, last harmony notes ring naturally');
      ...
      this.scheduledAudioSources.forEach((info, source) => {
        if (info.type === 'one-shot') {
          ...
          logger.debug(`🎵 Harmony note - will ring naturally if it's a last note`);
        }
      });
      ...
      this.scheduledAudioSources.forEach((info, source) => {
        try {
          if (info.type === 'one-shot') {
            ...
            logger.debug(`🛑 Stopped ${info.type} source with ${fadeOutTime * 1000}ms fadeout`);
      ...
      logger.info('🛑 MANUAL STOP: Non-harmony sources stopped, harmony with fast 30ms fadeout');
    }
    ...
    if (graceful) {
      ...
      this.scheduledAudioSources.forEach((info, source) => {
        try {
          ...
          if (info.type === 'sustained' && info.hasStopScheduled) {
            ...
            logger.debug(`🎵 GRACEFUL: Preserving ${info.type} source with scheduled ring-out`);
          } else {
      ...
      logger.info(`🎵 GRACEFUL STOP: Stopped ${futureSourcesStopped} future sources, preserved playing sources with ring-out`);
      ...
      setTimeout(() => {
        ...
        logger.info('🎵 GRACEFUL STOP: Cleared scheduled sources after ring-out period');
      ...
      this.scheduledAudioSources.forEach((info, source) => {
        try {
          source.stop(now + fadeOutTime); // 30ms fadeout prevents clicks
          futureSourcesStopped++;
          logger.debug(`🛑 Stopped future ${info.type} source with ${fadeOutTime * 1000}ms fadeout`);
        } catch (e) {
          logger.debug(`🛑 Source already stopped: ${info.type}`);
        }
      });

      logger.info(`🛑 MANUAL STOP: Stopped ${futureSourcesStopped} future sources with ${fadeOutTime * 1000}ms fadeout`);
  ...
  private reschedulePendingEvents(): void {
    ...
    if (this.isScheduling) {
      logger.warn('⚠️ RegionProcessor: Cannot reschedule during active scheduling - deferring');
      return;
    }

    const totalScheduledEvents = Array.from(this.scheduledEvents.values()).reduce((sum, set) => sum + set.size, 0);
    logger.info('♻️ RegionProcessor: Rescheduling all events due to tempo change', {
      instanceId: this._instanceId,
    ...
    this.scheduledAudioSources.forEach((type, source) => {
      try {
        ...
        logger.debug('Source already stopped/ended during reschedule', { type });
      }
    });
    this.scheduledAudioSources.clear();
    ...
    if (this.audioContext) {
      ...
      logger.info('🎯 Recalculated transportStartTime anchor for tempo change', {
    ...
    const newTotalScheduledEvents = Array.from(this.scheduledEvents.values()).reduce((sum, set) => sum + set.size, 0);
  ...
  private calculateExerciseDuration(): void {
    ...
    this.lastBeatThreshold = Math.max(0, this.exerciseEndTime - lastBeatDuration);

    console.log(`[EXERCISE DURATION] Transport: ${maxEndTime.toFixed(3)}s, Countdown offset: ${countdownOffsetSeconds.toFixed(3)}s, Total (audio time): ${this.exerciseEndTime.toFixed(3)}s, Last beat starts: ${this.lastBeatThreshold.toFixed(3)}s (1 beat = ${lastBeatDuration.toFixed(3)}s @ ${currentBpm} BPM)`);
  }
  ...
  private scheduleAllRegions(): void {
    ...
    try {
      ...
      const harmonyTrack = Array.from(this.tracks.values()).find(t => t.instrumentType === 'harmony');
      const exerciseId = harmonyTrack?.exerciseId;
      const cachedSchedule = exerciseId ? this.getCachedSchedule(exerciseId) : null;
      ...
      track.regions.forEach(region => {
        if (!region.pattern?.events) {
          this.debugger.log('RegionProcessor', 'no-pattern-events', { trackId, region: region.id });
          return;
        }
        ...
        const firstThreeNotes = sortedEvents.filter(e => e.type === 'harmony-note').slice(0, 3);
        console.log('[REGIONPROCESSOR] First 3 harmony notes after sort:', firstThreeNotes.map((e, i) => ({
          index: i + 1,
          type: e.type,
        ...
        console.log(`[CC64 DIAGNOSTIC] First 10 events after sorting (region: ${region.id}):`);
        sortedEvents.slice(0, 10).forEach((event, i) => {
          const parsedPos = this.parsePositionToObject(event.position);
          const isCC64 = event.type === 'harmony-control-change' && (event as any).data?.cc === 64;
          console.log(`  ${i}: ${event.type}${isCC64 ? ' (CC64)' : ''} @ ${parsedPos.measure}:${parsedPos.beat}:${parsedPos.subdivision}:${parsedPos.tick}`,
        ...
        if (cachedSchedule && instrumentType === 'harmony') {
          ...
          console.log(`[CC64] ♻️ Using CACHED timeline with ${this.currentCC64Timeline.size} pedal events`);
        } else {
          this.currentCC64Timeline = this.buildCC64Timeline(sortedEvents, region);
          console.log(`[CC64] 🔨 Built NEW timeline with ${this.currentCC64Timeline.size} pedal events`);
        }
        ...
        sortedEvents.forEach((event, eventIndex) => {
          ...
          const originalBpm = eventData?.originalBpm || Tone.Transport.bpm.value; // Fallback to transport BPM if not provided
          ...
          if (eventData?.ticks !== undefined) {
            ...
            if (event.type === 'harmony-note') {
              if (harmonyNoteCount < 3 || harmonyNoteCount === 8) {
                console.log(`[ABSOLUTE TICK SCHEDULING] Harmony Note ${harmonyNoteCount + 1} (eventIndex ${eventIndex}):`, {
                  absoluteTicks,
                  originalBpm,
                  secondsPerBeat,
                  ticksPerBeat,
            ...
            if (event.type === 'harmony-note') {
              if (harmonyNoteCount < 3 || harmonyNoteCount === 8) {
                console.log(`[RELATIVE TICK SCHEDULING] Harmony Note ${harmonyNoteCount + 1} (eventIndex ${eventIndex}):`, {
                  position: event.position,
                  calculatedTime: eventTime.toFixed(6),
                  usingAbsoluteTicks: false,
                  noteName: (event as any).data?.noteName,
                  WARNING: 'event.data.ticks is undefined - using relative position',
                });
              }
              harmonyNoteCount++;
            }
          ...
          const offsetTime = (this.countdownEnabled && !region.skipCountdownOffset)
          ...
          logger.info(`📅 Collected ${instrumentType} event at ${absoluteTime}s: ${event.type}`);
        });
      });
    });

    // Log batching stats
    const batchedEventCount = Array.from(this.scheduledEvents.values()).reduce((sum, set) => sum + set.size, 0);
    logger.info(`📦 Batched ${eventsByTime.size} unique time points with ${batchedEventCount} total events`);
    ...
    eventsByTime.forEach((events, timeKey) => {
      ...
      if (absoluteAudioTime < currentAudioTime) {
        logger.debug(`⏭️  Skipping past event batch at ${timeKey}s (already played)`, {
      ...
      try {
        ...
        events.forEach(({ instrumentType, event, eventKey }, index) => {
          ...
          logger.info(`⏱️ Event ${index + 1}/${events.length} in batch: ${instrumentType}`, {
        ...
        logger.info(`✅ Batch completed: ${events.length} events in ${batchTotalTime.toFixed(3)}ms`);
      } catch (error) {
      ...
      const totalScheduledEvents = Array.from(this.scheduledEvents.values()).reduce((sum, set) => sum + set.size, 0);
      logger.info(`✅ Scheduled ${totalScheduledEvents} audio events total in ${eventsByTime.size} batches (upfront scheduling)`);
  ...
  private processCurrentPosition(): void {
    ...
    this.tracks.forEach((track, trackId) => {
      ...
      track.regions.forEach(region => {
        ...
        if (currentTime < region.startTime || currentTime > region.startTime + regionDurationInSeconds) {
          return;
        }
   ...
   */
  private parsePosition(position: string | {measure: number, beat: number, subdivision: number, tick?: number}): number {
    try {
      // Get current BPM from Tone.Transport (single source of truth)
      const currentBpm = Tone.Transport.bpm.value;
      const secondsPerBeat = 60 / currentBpm;

      let bars: number, beats: number, ticks = 0;
      ...
      const totalBeats =
        (bars * beatsPerBar) +
        beats +
        tickFraction;  // Single source of truth for sub-beat precision
   ...
   */
  private parsePositionToObject(position: string | {measure: number, beat: number, subdivision?: number, tick?: number}): {measure: number, beat: number, subdivision: number, tick: number} {
    if (typeof position === 'object' && position !== null) {
      return {
   ...
   */
  private scheduleAudioDirect(instrumentType: string, event: PatternEvent, audioTime: number, frame: number): boolean {
    // Handle metronome
    ...
    logger.debug(`❌ FAANG: Direct scheduling not yet implemented for: ${instrumentType}`);
    return false;
  }

  private scheduleMetronomeDirect(event: PatternEvent, audioTime: number, frame: number): boolean {
    ...
    if (!this.audioContext || !this.audioDestination || !this.metronomeBuffers.accent || !this.metronomeBuffers.click) {
    ...
    const buffer = event.type === 'accent' ? this.metronomeBuffers.accent : this.metronomeBuffers.click;
    ...
    try {
      ...
      this.scheduledAudioSources.set(source, { type: 'one-shot', hasStopScheduled: false });
  ...
  private scheduleDrumDirect(event: PatternEvent, audioTime: number, frame: number): boolean {
  ...
  private scheduleVoiceCueDirect(event: PatternEvent, audioTime: number, frame: number): boolean {
    // Check if we have the necessary buffers and destination
    if (!this.audioContext || !this.audioDestination) {
    ...
    try {
      ...
      logger.error(`Failed to schedule voice cue audio directly (${cueName})`, error);
      return false; // Fall back to event bus
    }
  }

  private scheduleHarmonyDirect(event: PatternEvent, audioTime: number, frame: number): boolean {
    // 🚨 CRITICAL DIAGNOSTIC: This should ALWAYS log when harmony plays
    console.log('🚨🚨🚨 scheduleHarmonyDirect() ENTRY - HARMONY AUDIO PATH ACTIVATED', {
      eventType: event.type,
      hasData: !!event.data,
    ...
    console.log(`[HARMONY EVENT DEBUG] Received event type: "${event.type}", has cc: ${eventData?.cc !== undefined}, cc value: ${eventData?.cc}`);

    // Handle control change events (sustain pedal, expression, etc.)
    if (event.type === 'harmony-control-change' && eventData?.cc !== undefined) {
      console.log(`[HARMONY EVENT DEBUG] ✅ Matched harmony-control-change, CC = ${eventData.cc}`);
      if (eventData.cc === 64) {
        // CC64 = Sustain Pedal
        ...
        console.log(`[CC64 EVENT] Pedal ${eventData.value >= 64 ? 'DOWN' : 'UP'} @ ${audioTime.toFixed(3)}s (value=${eventData.value}) - Pre-calculated in timeline`);
    ...
    if (eventData?.midiNote !== undefined) {
      console.log('✅ Taking MIDI NOTE path - calling scheduleHarmonyMidiNoteDirect', {
    ...
    if (!this.audioContext || !this.audioDestination) {
      logger.warn('❌ FAANG: Cannot use direct scheduling - missing harmony dependencies', {
        hasAudioContext: !!this.audioContext,
        hasDestination: !!this.audioDestination,
        instanceId: this._instanceId
      });
    ...
    const { parseChord, mapVelocityToLayer, parseDuration } = require('@/domains/playback/utils/chordParser.js');
    ...
    if (successCount === 0) {
      logger.warn(`❌ FAANG: No harmony notes scheduled for chord ${chordSymbol}`);
      return false; // Fall back to event bus
    }
   ...
   */
  private getHarmonyLayerForNoteVelocity(noteName: string, velocity: number): string {
  ...
  private detectSparseSampling(): boolean {
    ...
    const allChromaticNotes = ['C', 'Cs', 'D', 'Ds', 'E', 'F', 'Fs', 'G', 'Gs', 'A', 'As', 'B'];
   ...
   */
  private buildCC64Timeline(events: any[], region: Region): Map<number, boolean> {
    ...
    events.forEach(event => {
      if (event.type === 'harmony-control-change' && event.data?.cc === 64) {
        ...
        if (eventIndex < 5) {
          console.log(`[CC64 TIMELINE #${eventIndex}] timelineKey=${timelineKey.toFixed(6)}s (with transportStartTime + sample precision), pedal=${pedalDown ? 'DOWN' : 'UP'}, value=${event.data.value}, absoluteTime=${absoluteTime.toFixed(6)}s, audioTime=${audioTime.toFixed(6)}s`);
        }
        eventIndex++;
      }
    });
   ...
   */
  private isPedalDownAtTime(time: number, cc64Timeline: Map<number, boolean>): boolean {
    ...
    console.log(`[CC64 CHECK] isPedalDownAtTime(${time.toFixed(6)}s): lastEvent=${lastEventTime.toFixed(6)}s (${eventsFound} events scanned), state=${lastPedalState ? 'DOWN' : 'UP'}, timelineSize=${cc64Timeline.size}, precision=${time === lastEventTime ? 'EXACT' : 'APPROXIMATE'}`);
   ...
   */
  private findNextCC64Up(noteStartTime: number, cc64Timeline: Map<number, boolean>): number | null {
    const sortedTimes = Array.from(cc64Timeline.keys()).sort((a, b) => a - b);
  ...
  private isNoteHeldUntilExerciseEnd(midiNoteEndTime: number): boolean {
    ...
    const THRESHOLD_MS = 0.250; // 250 milliseconds
    const timeDifference = this.exerciseEndTime - midiNoteEndTime;
    const heldUntilEnd = timeDifference >= 0 && timeDifference <= THRESHOLD_MS;

    console.log(`[HELD BY HAND CHECK] midiNoteEnd=${midiNoteEndTime.toFixed(3)}s, exerciseEnd=${this.exerciseEndTime.toFixed(3)}s, diff=${timeDifference.toFixed(3)}s, threshold=${THRESHOLD_MS}s, result=${heldUntilEnd}`);
    ...
    for (const eventTime of sortedTimes) {
      if (eventTime > noteStart && eventTime < noteEnd) {
        if (timeline.get(eventTime) === true) {
          console.log(`[CC64 MID-NOTE] Pedal goes DOWN at ${eventTime.toFixed(3)}s during note playing ${noteStart.toFixed(3)}s-${noteEnd.toFixed(3)}s`);
   ...
   */
  private scheduleHarmonyMidiNoteDirect(event: PatternEvent, audioTime: number, frame: number): boolean {
    ...
    if (!this.audioContext || !this.audioDestination) {
      ...
      logger.warn('❌ Cannot schedule harmony MIDI note - missing audio dependencies');
      return false;
    }
    ...
    if (octaveShift !== 0 && this._noteLogCount < 3) {
      console.log(`[INSTRUMENT OCTAVE SHIFT] ${isGrandPiano ? 'Grand Piano' : 'Wurlitzer'}: MIDI ${eventData.midiNote} → ${midiNote} (shift: -${octaveShift}) [instrument=${this.currentHarmonyInstrument}]`);
    }
    ...
    if (this._noteLogCount < 3) {
      console.log(`[NOTE SCHEDULE #${this._noteLogCount}] audioTime=${audioTime.toFixed(3)}s, midiNote=${midiNote}, position=${JSON.stringify(event.position)}`);
      this._noteLogCount++;
    }
    ...
    if (this.currentHarmonyInstrument === 'grandpiano') {
      console.log('🗺️ [NOTE-MAPPING] Grand Piano detected, checking keyboard map...', {
        hasKeyboardMap: !!this.grandPianoKeyboardMap,
        requestedNote: noteName,
        mapKeys: this.grandPianoKeyboardMap ? Object.keys(this.grandPianoKeyboardMap).length : 0,
      });
      ...
      if (this.grandPianoKeyboardMap) {
        ...
        if (mapping) {
          ...
          console.warn('🗺️ [NOTE-MAPPING] ⚠️ No mapping found for note', { noteName });
        }
      } else {
        console.error('🗺️ [NOTE-MAPPING] ❌ Keyboard map NOT LOADED - note mapping will fail!', {
          requestedNote: noteName,
          currentInstrument: this.currentHarmonyInstrument,
        });
      }
    ...
    if (!buffer) {
      ...
      if (buffer) {
        console.log(`[CACHE FALLBACK] Found ${cacheKey} in GlobalSampleCache (note=${noteName} → sample=${actualSampleNote})`);
      } else {
        // CRITICAL DIAGNOSTIC: Show what we're looking for vs what exists
        const allCachedKeys = Array.from((GlobalSampleCache.getInstance() as any).samples?.keys() || []);
        const instrumentKeys = allCachedKeys.filter(k => k.startsWith(`${instrument}-`));
        ...
        console.log(`   Layer: ${layer}, Requested: ${noteName}, Mapped: ${actualSampleNote}`);
        console.log(`   Available keys (${instrumentKeys.length} total):`, instrumentKeys.slice(0, 15));
      }
    }
    ...
    if (this.grandPianoKeyboardMap && this.grandPianoKeyboardMap[noteName]) {
      ...
      if (!buffer) {
        ...
        const layerOrder = [
          ...
          ...allLayers.filter(l => l !== layer).sort((a, b) => {
        ...
        for (const fallbackLayer of layerOrder) {
          ...
          if (!buffer) {
            ...
            buffer = GlobalSampleCache.getCachedBuffer(cacheKey);
            if (buffer) {
              console.log(`[CACHE FALLBACK + MAP] ${noteName} → ${sampleNote} from GlobalSampleCache (${cacheKey})`);
            }
          }

          if (buffer) {
            console.log(`[GRAND PIANO MAP] ${noteName} → ${sampleNote} in ${fallbackLayer} (${mapping.semitones > 0 ? '+' : ''}${mapping.semitones} semitones, rate ${playbackRate.toFixed(3)})`);
            layer = fallbackLayer;
            break;
          }
        }
        ...
        if (Math.abs(playbackRate - 1.0) > 0.001 && this._noteLogCount < 10) {
          console.log(`[GRAND PIANO PITCH] ${noteName}: rate=${playbackRate.toFixed(3)} (${mapping.semitones > 0 ? '+' : ''}${mapping.semitones} semitones) - buffer found directly`);
        }
      }
    }
    ...
    if (!buffer) {
      ...
      for (const fallbackLayer of layerOrder) {
        ...
        if (!buffer) {
          ...
          buffer = GlobalSampleCache.getCachedBuffer(cacheKey);
          if (buffer) {
            console.log(`[CACHE FALLBACK + VELOCITY] ${noteName} → ${sampleNote} from GlobalSampleCache (${cacheKey})`);
          }
        }

        if (buffer) {
          console.log(`[VELOCITY FALLBACK] ${noteName} → ${sampleNote}: ${layer} → ${fallbackLayer}`);
          layer = fallbackLayer;
          break;
        }
      }
    ...
    if (!buffer) {
      ...
      console.log(`❌ Could not find buffer for: ${layer}/${noteName} (MIDI ${midiNote})`);
      ...
      console.log(`\n🔧 State:`);
      console.log(`   Current instrument: ${this.currentHarmonyInstrument}`);
      console.log(`   Requested note: ${noteName}, Mapped sample: ${sampleNote}`);
      console.log(`   Layer: ${layer}, Velocity: ${velocity}`);
      console.log(`========================================================\n`);

      // For Grand Piano, also check what the keyboard map says
      const keyboardMapInfo = this.grandPianoKeyboardMap && this.grandPianoKeyboardMap[noteName]
        ? {
      ...
      console.error(`❌ MISSING BUFFER: ${layer}/${noteName} (MIDI ${midiNote})`, {
        layer,
        noteName,
        requestedNote: noteName,
        ...
        availableNotesInLayer: this.harmonyBuffers.get(layer) ? Array.from(this.harmonyBuffers.get(layer)!.keys()) : 'LAYER_NOT_FOUND',
        allAvailableNotes: Array.from(this.harmonyBuffers.entries()).map(([l, notes]) =>
          `${l}: ${Array.from(notes.keys()).join(', ')}`
        ),
      });
      logger.debug(`❌ Missing harmony buffer for ${layer}/${noteName} (MIDI ${midiNote}, original: ${eventData.midiNote}, shift: ${octaveShift})`);
      return false;
    }
    ...
    try {
      ...
      if (this.currentHarmonyInstrument === 'grandpiano' && Math.abs(playbackRate - 1.0) > 0.001) {
      ...
      if (midiNote >= 46 && midiNote <= 48) { // Log A#2, B2, C3
      ...
      if (this.currentCC64Timeline.size > 0) {
        ...
        if (pedalDownTime !== null) {
          const pedalUpTime = this.findNextCC64Up(pedalDownTime, this.currentCC64Timeline);
      ...
      if (willBeSustained) {
        ...
        console.log(`[CC64 LOOP] ${noteName}: Enabling loop (${source.loopStart.toFixed(3)}s-${source.loopEnd.toFixed(3)}s) for ${sustainedDuration.toFixed(3)}s sustain`);
      }
      ...
      if (this.currentCC64Timeline.size > 0) {
        console.log(`[CC64 SUSTAIN] Checking note ${noteName} @ ${audioTime.toFixed(3)}s (timeline has ${this.currentCC64Timeline.size} events)`);
        ...
        if (pedalDownTime !== null) {
          ...
          if (pedalUpTime !== null) {
            ...
            if (sustainDuration > 0) {
              ...
              if (pedalUpTime > midiNoteEndTime) {
                ...
                if (pedalDownTime > audioTime) {
                  console.log(`[CC64 MID-SUSTAIN] ${noteName}: Pedal went DOWN at ${pedalDownTime.toFixed(3)}s (during note), sustains until ${pedalUpTime.toFixed(3)}s (+${(actualDuration - duration).toFixed(3)}s)`);
                } else {
                  console.log(`[CC64 EXTEND] ${noteName}: MIDI would end at ${midiNoteEndTime.toFixed(3)}s, extended to pedal UP @ ${pedalUpTime.toFixed(3)}s (+${(actualDuration - duration).toFixed(3)}s sustain)`);
                }
              } else {
                ...
                console.log(`[CC64 IGNORED] ${noteName}: Pedal UP @ ${pedalUpTime.toFixed(3)}s while note held until ${midiNoteEndTime.toFixed(3)}s - using MIDI duration (legato pedaling)`);
              }
            } else {
              // Edge case: Note starts after pedal UP
              console.warn(`[CC64 WARNING] ${noteName}: Note @ ${audioTime.toFixed(3)}s starts after pedal UP @ ${pedalUpTime.toFixed(3)}s - using MIDI duration`);
              actualDuration = duration;
            }
          } else {
            ...
            if (noteStartsInLastBeat && this.exerciseEndTime > 0) {
              ...
              actualDuration = Math.max(duration, Math.min(cappedDuration, buffer.duration));
              console.log(`[CC64] ${noteName}: Extended ${duration.toFixed(3)}s → ${actualDuration.toFixed(3)}s (no pedal UP, capped at exercise end + 3s)`);
            } else {
              // Use full buffer duration for notes not in last beat
              actualDuration = Math.max(duration, buffer.duration);
              console.log(`[CC64] ${noteName}: Extended ${duration.toFixed(3)}s → ${actualDuration.toFixed(3)}s (no pedal UP, using buffer)`);
            }
          }
        }
      }
      ...
      const midiNoteEndTime = audioTime + duration;
      const heldByHandUntilEnd = this.isNoteHeldUntilExerciseEnd(midiNoteEndTime);
      ...
      const isLastNote = (heldByHandUntilEnd || extendedByCC64) && extendsIntoLastBeat && isHeldNote && startsBeforeEnd;

      // DIAGNOSTIC: Log ALL notes in last beat to see why they might not qualify
      if (extendsIntoLastBeat) {
        console.log(`[LAST NOTE CHECK] ${noteName} @ ${audioTime.toFixed(3)}s:`, {
      ...
      if (isLastNote) {
        ...
        console.log(`[LAST NOTE DECAY] ${noteName} @ ${audioTime.toFixed(3)}s: ${previousDuration.toFixed(3)}s → ${actualDuration.toFixed(3)}s`);
        console.log(`  Exercise end: ${this.exerciseEndTime.toFixed(3)}s, Desired end: ${desiredEndTime.toFixed(3)}s, Reason: ${reason}`);
      }
      ...
      const existingSourcesCount = this.activeHarmonySources.get(noteName)?.length || 0;
      if (!this.activeHarmonySources.has(noteName)) {
        this.activeHarmonySources.set(noteName, []);
      }
      this.activeHarmonySources.get(noteName)!.push({ source, gain, gainValue: targetGain, noteEndTime: finalNoteEndTime });
      ...
      if (isLastNote) {
        ...
        console.log(`[LAST NOTE FADEOUT] ${noteName}: Hold ${fadeStartTime.toFixed(3)}s, quick drop ${fadeStartTime.toFixed(3)}s-${midFadeTime.toFixed(3)}s (to 50%), smooth fade ${midFadeTime.toFixed(3)}s → ${finalNoteEndTime.toFixed(3)}s (1s hold + 1s quick drop + 2s smooth fade)`);
        ...
        gain.gain.exponentialRampToValueAtTime(0.001, finalNoteEndTime + fadeOutDuration);

        scheduledStopTime = finalNoteEndTime + fadeOutDuration + 0.01;
        source.stop(scheduledStopTime);

        if (isSustained) {
          console.log(`[CC64] ${noteName} @ ${audioTime.toFixed(3)}s sustained until ${finalNoteEndTime.toFixed(3)}s (extended by ${(actualDuration - duration).toFixed(3)}s), exponential fade starts AT pedal UP (${fadeStartTime.toFixed(3)}s) for ${fadeOutDuration * 1000}ms`);
        } else {
          console.log(`[CC64] ${noteName} @ ${audioTime.toFixed(3)}s normal duration ${actualDuration.toFixed(3)}s, exponential fade starts at ${fadeStartTime.toFixed(3)}s for ${fadeOutDuration * 1000}ms`);
        }
      }
  ...
  private scheduleBassDirect(event: PatternEvent, audioTime: number, frame: number): boolean {
   ...
   */
  private emitEvent(instrumentType: string, event: PatternEvent, time: number): void {
    ...
    if (event.type === 'harmony-control-change') {
      console.log(`[EMIT EVENT] ⚠️ harmony-control-change detected: cc=${(event.data as any)?.cc}, value=${(event.data as any)?.value}, audioTime=${audioTime.toFixed(3)}`);
    }
    ...
    switch (instrumentType) {
      case 'metronome':
        ...
        logger.debug(`Emitted metronome-trigger: ${event.type} at ${audioTime.toFixed(3)}`);
        break;
   ...
   */
  private setCachedSchedule(exerciseId: string, schedule: CachedSchedule): void {
   ...
   */
  updateTracks(tracks: Track[], exerciseMetadata?: { harmonyInstrument?: string }): void {
    ...
    if (exerciseMetadata?.harmonyInstrument) {
      ...
      logger.info(`🎹 Early harmony instrument detection: ${exerciseMetadata.harmonyInstrument}`);
      ...
      if (exerciseMetadata.harmonyInstrument === 'grandpiano' && !this.grandPianoKeyboardMap) {
        console.log('🗺️ [EARLY-LOAD] Loading keyboard map immediately in updateTracks()...');
        this.loadGrandPianoKeyboardMap().then(() => {
          console.log('🗺️ [EARLY-LOAD] ✅ Keyboard map loaded early', {
            hasKeyboardMap: !!this.grandPianoKeyboardMap,
            mapKeys: this.grandPianoKeyboardMap ? Object.keys(this.grandPianoKeyboardMap).length : 0,
          });
        }).catch(error => {
          console.error('🗺️ [EARLY-LOAD] ❌ Failed to load keyboard map early:', error);
        });
      }
    } else {
      ...
      if (harmonyTrack && (harmonyTrack as any).harmonyInstrument) {
        ...
        logger.info(`🎹 Inferred harmony instrument from track: ${this.currentHarmonyInstrument}`);

        // CRITICAL FIX: Load keyboard map immediately for Grand Piano
        if (this.currentHarmonyInstrument === 'grandpiano' && !this.grandPianoKeyboardMap) {
          console.log('🗺️ [EARLY-LOAD] Loading keyboard map immediately (inferred)...');
          this.loadGrandPianoKeyboardMap().then(() => {
            console.log('🗺️ [EARLY-LOAD] ✅ Keyboard map loaded early (inferred)', {
              hasKeyboardMap: !!this.grandPianoKeyboardMap,
              mapKeys: this.grandPianoKeyboardMap ? Object.keys(this.grandPianoKeyboardMap).length : 0,
            });
          }).catch(error => {
            console.error('🗺️ [EARLY-LOAD] ❌ Failed to load keyboard map early (inferred):', error);
          });
        }
      }
    }
    ...
    if (this.isRunning) {
      ...
      logger.info('⚡ [RegionProcessor] Adding tracks dynamically while running', {
      ...
      tracks.forEach((track) => {
        ...
        logger.info(`📝 [RegionProcessor] Added track dynamically: ${track.id}`);
      });
      ...
      logger.info('🔄 [RegionProcessor] Scheduling events for newly added tracks');
      this.scheduleAllRegions();
  ...
  private trackTimingAccuracy(frame: number, transportTime: number): void {
    ...
    const expectedFrame = Math.round(expectedBeatNumber * this.timingMetrics.expectedFrameInterval);
    ...
    const avgFrames = this.timingMetrics.frameDeltas.reduce((a, b) => a + b, 0) / this.timingMetrics.frameDeltas.length;
  ...
  private startMetricsReporting(): void {
    ...
    this.metricsInterval = setInterval(() => {
      ...
      if (newEvents >= 8 || (newEvents > 0 && this.timingMetrics.totalEvents > 0)) {
        const accuracy = (this.timingMetrics.perfectFrames / this.timingMetrics.totalEvents) * 100;
        ...
        logger.info('⏱️  Timing Metrics', {
          ...
          grade: accuracy >= 99 ? '🟢 EXCELLENT' : accuracy >= 95 ? '🟡 GOOD' : '🔴 NEEDS IMPROVEMENT'
        });
   ...
   */
  getTimingMetrics() {
    const accuracy = this.timingMetrics.totalEvents > 0
      ? (this.timingMetrics.perfectFrames / this.timingMetrics.totalEvents) * 100
      : 0;
    ...
    return {
      ...
      grade: accuracy >= 99 ? 'EXCELLENT' : accuracy >= 95 ? 'GOOD' : 'NEEDS_IMPROVEMENT',
  ...
  private logCC64DiagnosticTable(sortedEvents: any[], region: Region): void {
    ...
    sortedEvents.forEach(event => {
      if (event.data?.midiNote !== undefined) {
        ...
        if (event.data?.ticks !== undefined) {
          ...
          // 🚨 CRITICAL FIX: Use original MIDI file BPM, not current transport BPM
          const originalBpm = event.data?.originalBpm || Tone.Transport.bpm.value;
          const secondsPerBeat = 60 / originalBpm;
          const ticksPerBeat = 480; // PPQ standard
        ...
        const duration = typeof event.duration === 'number' ? event.duration : 0.5;
    ...
    // Table header
    console.log('┌─────┬──────────┬───────────┬──────────────┬──────────────┬────────────────┬─────────────────┬──────────────┬──────────────┐');
    console.log('│ #   │ Note     │ Start (s) │ MIDI Dur (s) │ MIDI End (s) │ Pedal Down (s) │ Pedal Up (s)    │ Final Dur(s) │ Extension(s) │');
    console.log('├─────┼──────────┼───────────┼──────────────┼──────────────┼────────────────┼─────────────────┼──────────────┼──────────────┤');
    ...
    harmonyNotes.forEach((note, index) => {
      ...
      if (this.currentCC64Timeline.size > 0) {
        ...
        if (pedalDownTime !== null) {
          pedalUpTime = this.findNextCC64Up(pedalDownTime, this.currentCC64Timeline);
      ...
      const pedalDown = (pedalDownTime !== null ? pedalDownTime.toFixed(3) : 'N/A').padStart(14);
      const pedalUp = (pedalUpTime !== null ? pedalUpTime.toFixed(3) : 'N/A').padStart(15);
      const finalDur = actualDuration.toFixed(3).padStart(12);
      const ext = (extension > 0 ? `+${extension.toFixed(3)}` : '-').padStart(12);

      console.log(`│ ${num} │ ${noteName} │ ${start} │ ${midiDur} │ ${midiEnd} │ ${pedalDown} │ ${pedalUp} │ ${finalDur} │ ${ext} │`);
    });
    ...
    if (extendedCount > 0) {
      console.log(`   Average extension: ${(totalExtension / extendedCount).toFixed(3)}s | Total added: ${totalExtension.toFixed(3)}s`);
    }
    console.log('\n' + '='.repeat(120) + '\n');
  }
}