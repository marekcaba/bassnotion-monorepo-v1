...
export class TimingIsolationManager {
  ...
  constructor() {
    ...
    try {
      ...
      logger.warn('EventBus not found in ServiceRegistry', { correlationId: 'system' });
    }
  ...
  private initialize(): void {
    ...
    logger.info('🛡️ TimingIsolationManager: Initialized', { correlationId: 'system' });
  }
  ...
  public setTrackPolicy(
    ...
    logger.info(`🛡️ Updated isolation policy for track ${trackId}:`, { policy, correlationId: 'system' });
  }
  ...
  private handleDriftViolation(data: {
    ...
    if (
      Math.abs(data.drift) > policy.maxDriftMs ||
      data.errorCount >= policy.maxErrorCount
    ) {
      this.isolateTrack(
        data.trackId,
        `Drift violation: ${data.drift.toFixed(3)}ms`,
      );
    }
  }
  ...
  private evaluateTrackHealth(report: any): void {
    ...
    for (const metric of report.tracks) {
      ...
      if (
        metric.stability < policy.minStability ||
  ...
  private isolateTrack(trackId: string, reason: string): void {
    ...
    logger.warn(`🛡️ Isolated track ${trackId}: ${reason}`, { correlationId: 'system' });
  ...
  private attemptRecovery(trackId: string): void {
    ...
    logger.info(
      `🛡️ Attempting recovery for track ${trackId} (attempt ${isolationInfo.recoveryAttempts})`,
    );
    ...
    setTimeout(() => {
      ...
      if (timingState && timingState.isActive && timingState.errorCount === 0) {
        ...
        logger.info(`✅ Track ${trackId} recovered successfully`);
        ...
        if (isolationInfo.recoveryAttempts >= policy.recoveryAttempts) {
          ...
          logger.error(
            `❌ Track ${trackId} recovery failed after ${isolationInfo.recoveryAttempts} attempts`,
            undefined,
  ...
  public recoverTrack(trackId: string): boolean {
    ...
    logger.info(`🛡️ Manually recovered track ${trackId}`);
  ...
  private checkIsolatedTracks(): void {
    for (const [trackId, info] of this.isolatedTracks.entries()) {
      ...
      if (
        timeSinceIsolation >
        policy.recoveryDelayMs * (info.recoveryAttempts + 1)
      ) {
        this.attemptRecovery(trackId);
      }
    }
  ...
  public clearAllIsolations(): void {
    ...
    logger.info('🛡️ Cleared all track isolations');
  }
  ...
  public dispose(): void {
    ...
    logger.info('🛡️ TimingIsolationManager: Disposed');
  }
}
