...
import { ServiceRegistry, Service, ServiceError, HealthCheckResult } from '../ServiceRegistry.js';
...
describe('ServiceRegistry - Enhanced Features', () => {
  ...
  describe('Service Configuration', () => {
    ...
    it('should throw error if service does not support config updates', async () => {
      ...
      await expect(
        registry.updateServiceConfig('basic', {})
      ).rejects.toThrow('does not support configuration updates');
    });
  });
  ...
  describe('Edge Cases', () => {
    ...
    it('should prevent operations after disposal', async () => {
      await registry.dispose();
      await expect(registry.initialize()).rejects.toThrow('ServiceRegistry has been disposed');
    });
    ...
    it('should handle health check for services without healthCheck method', async () => {
      ...
      expect(report.services.basic.message).toContain('Service status: started');
    });
