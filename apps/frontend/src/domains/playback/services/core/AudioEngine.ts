...
// Dynamic Tone.js reference
// Store on window to ensure true global access across all instances
declare global {
  interface Window {
...
// Use window storage for true global access
const getTone = (): ToneModule | null => {
  if (typeof window !== 'undefined') {
    return window.__globalTone || null;
  }
  return null;
};

const setTone = (tone: ToneModule): void => {
  if (typeof window !== 'undefined') {
...
// Audio error class moved to errors/AudioErrors.ts
...
export class AudioEngine implements Service {
  ...
  private browserInfo: { name: string; version: number } | null = null;

  constructor(eventBus: EventBus, config: AudioEngineConfig = {}) {
    this.eventBus = eventBus;
    this.config = {
      sampleRate: 48000,
      latencyHint: 'balanced', // Changed from 'interactive' for more stable timing
      maxInitRetries: config.maxInitRetries || AudioEngine.MAX_INIT_RETRIES,
      initRetryDelay: config.initRetryDelay || AudioEngine.INIT_RETRY_DELAY,
      enableBrowserCheck: config.enableBrowserCheck ?? true,
    ...
    // this.performanceOptimizer = new PerformanceOptimizer(this.eventBus, {
    ...
    this.circuitBreaker = new CircuitBreaker('AudioEngine', {
      ...
      timeout: 5000, // 5 seconds should be plenty for just loading Tone.js
  ...
  async preInitialize(): Promise<void> {
    ...
    // Load Tone.js module
    Tone = getTone(); // Get from global storage first
    if (!Tone) {
      try {
  ...
  /**
   * Initialize the audio engine with 99%+ reliability
  ...
  /**
   * Initialize with retry logic for 99%+ reliability
   */
  ...
  /**
   * Create AudioContext with browser-specific optimizations
   */
  private async createOptimalAudioContext(): Promise<AudioContext> {
    // DO NOT use pre-warmed context - it's suspended and can't be resumed
    ...
    try {
      ...
      if (this.isIOS()) {
        // iOS requires user gesture, but we can prepare the context
        this.eventBus.emit('audio:ios-detected', { timestamp: Date.now() });
      }
  ...
  private async ensureContextRunning(): Promise<void> {
    if (!this.context) throw new AudioContextError('No AudioContext available');
    ...
    while (
      ...
      if (this.context.state === 'suspended') {
        try {
          ...
          // Ignore resume errors during wait
        }
      }
    }
  ...
  private async validateAudioSystem(): Promise<void> {
    ...
    try {
      ...
      testAnalyser.connect(this.context.destination);
  ...
  /**
   * Get Tone.js instance (single access point)
   */
  getTone(): ToneModule {
    // First try to get from global storage
    const globalTone = getTone();
    if (globalTone) {
  ...
  /**
   * Create a sampler with circuit breaker protection
   */
  async createSampler(config: SamplerConfig): Promise<AudioSampler> {
    ...
    return this.circuitBreaker.execute(async () => {
      ...
      try {
        ...
        // Track sampler creation performance
        const creationTime = performance.now() - startTime;
        this.eventBus.emit('audio:sampler-created', {
        ...
        return {
          ...
          dispose: () => {
            sampler.dispose();
            this.samplerCount--;
            this.eventBus.emit('audio:sampler-disposed', {
              samplerCount: this.samplerCount,
              timestamp: Date.now(),
            });
          },
  ...
  async getOptimizationMetrics() {
    // const metrics = await this.performanceOptimizer.monitorPerformance();
    // const report = this.performanceOptimizer.getOptimizationReport();
    // Return stub optimization metrics
    return {
      metrics: {
        cpuUsage: 0,
        memoryUsage: 0,
  ...
  getPooledBuffer(size = 4096): Float32Array | null {
    ...
    //   this.performanceOptimizer.returnToPool('audioBuffers', buffer);
    //   return new Float32Array(size);
    // }
    // return buffer || new Float32Array(size);
    // Return new buffer since optimizer is not available
    return new Float32Array(size);
  }
  ...
  returnPooledBuffer(buffer: Float32Array): void {
    // this.performanceOptimizer.returnToPool('audioBuffers', buffer);
    // No-op since optimizer is not available
  }
   ...
   */
  isReady(): boolean {
    return this.isInitialized && this.context !== null && Tone !== null;
  }
  ...
  async dispose(): Promise<void> {
    ...
    if (this.samplerCount > 0) {
      this.eventBus.emit('audio:warning', {
    ...
    this.eventBus.emit('audio:disposed', { timestamp: Date.now() });
  }
  ...
  private isBrowserSupported(): boolean {
    ...
    for (const api of REQUIRED_APIS) {
      if (!(api in window)) {
        this.eventBus.emit('audio:api-missing', { api });
        return false;
      }
    }
  ...
  /**
   * Get optimal sample rate for browser
   */
  private getOptimalSampleRate(): number {
    // Use configured sample rate if specified
