...
import type { InstrumentType } from '../plugins/TrackManagerProcessor.js';
...
import { compareMusicalPositions, addMusicalTime, isPositionInRange } from '../../utils/regionUtils.js';
...
export class Track implements ITrack, TrackLifecycle {
  ...
  constructor(config: TrackConfig) {
    ...
    try {
      ...
      // ErrorReporter might not be registered in tests
      console.warn('ErrorReporter not found in ServiceRegistry');
    }
  }
  ...
  validate(): boolean {
    try {
      ...
      if (velocityRange.min < 0 || velocityRange.max > 127 || 
  ...
  clone(newId?: string): Track {
    ...
    cloned.musical = {
      ...
      noteRange: this.musical.noteRange ? { ...this.musical.noteRange } : undefined,
  ...
  private generateColor(): string {
    const colors = [
      ...
      '#98D8C8', '#F7DC6F', '#BB8FCE', '#85C1F2',
      '#F8B195', '#F67280', '#C06C84', '#6C5CE7'
    ];
   ...
   */
  private musicalPositionToSeconds(position: ITrack['musical']['timeSignature']): number {
  ...
  addRegion(region: Region): void {
    ...
    this.regions.sort((a, b) => compareMusicalPositions(a.startPosition, b.startPosition));
    ...
    // Log the action
    console.debug(`[Track] Added region ${region.id} to track ${this.id}`);
  }
  ...
  removeRegion(regionId: string): void {
    ...
    console.debug(`[Track] Removed region ${regionId} from track ${this.id}`);
  }
  ...
  updateRegion(regionId: string, updates: Partial<Region>): void {
    ...
    console.debug(`[Track] Updated region ${regionId} in track ${this.id}`);
  }
  ...
  getRegionsInRange(startPos: MusicalPosition, endPos: MusicalPosition): Region[] {
    return this.regions.filter(region => {
      ...
      return isPositionInRange(region.startPosition, startPos, endPos) ||
  ...
  getRegionsAtPosition(position: MusicalPosition): Region[] {
    return this.regions.filter(region => {
      const regionEnd = addMusicalTime(region.startPosition, region.duration);
      return compareMusicalPositions(position, region.startPosition) >= 0 &&
  ...
  clearRegions(): void {
    ...
    console.debug(`[Track] Cleared all regions from track ${this.id}`);
  }
