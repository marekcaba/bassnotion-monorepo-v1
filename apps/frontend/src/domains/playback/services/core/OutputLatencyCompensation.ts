...
export class OutputLatencyCompensation {
  ...
  private constructor(config?: Partial<LatencyCompensationConfig>) {
    ...
    try {
      ...
      this.mixingEngine =
        serviceRegistry.get<TrackMixingEngine>('mixingEngine');
    } catch (e) {
   ...
   */
  static getInstance(
    config?: Partial<LatencyCompensationConfig>,
  ...
  async initialize(audioContext: AudioContext): Promise<void> {
    ...
    logger.info('✅ Output Latency Compensation initialized');
    logger.info(
      `System latency: ${this.systemLatency?.latencyMs.toFixed(2)}ms`,
    );
  }
  ...
  dispose(): void {
    ...
    logger.info('🗑️ Output Latency Compensation disposed');
  }

  // Private methods

  private async measureSystemLatency(
  ...
  private updateTrackCompensation(trackId: string): void {
    ...
    if (this.config.bufferAlignment) {
      ...
      info.compensationDelaySamples =
        Math.round(compensationSamples / this.blockSize) * this.blockSize;
    } else {
      info.compensationDelaySamples = compensationSamples;
    }

    info.compensationDelayMs =
      (info.compensationDelaySamples / this.sampleRate) * 1000;
