...
export class SimpleDrumSampler {
  ...
  async loadSamples(baseUrl: string): Promise<void> {
    console.log('🥁 SimpleDrumSampler: Loading samples from', baseUrl);
    ...
    const loadPromises = Object.entries(samples).map(async ([drum, filename]) => {
      const url = `${baseUrl}${filename}`;
      
      try {
        console.log(`🥁 Fetching ${drum} from ${url}`);
        ...
        console.log(`✅ ${drum} loaded: ${audioBuffer.duration}s, ${audioBuffer.sampleRate}Hz`);
        
      } catch (error) {
        console.error(`❌ Failed to load ${drum}:`, error);
        // Create synthetic fallback
        this.buffers[drum as keyof DrumBuffers] = this.createSyntheticDrum(drum as keyof DrumBuffers);
      }
    });
    
    await Promise.all(loadPromises);
    this.isLoaded = true;
    console.log('🥁 SimpleDrumSampler: All samples loaded or synthesized');
  }
  ...
  private createSyntheticDrum(drumType: keyof DrumBuffers): AudioBuffer {
    ...
    console.log(`🎵 Created synthetic ${drumType} drum`);
    return buffer;
  }
  ...
  play(drumType: keyof DrumBuffers, time?: number, velocity = 1): void {
    ...
    if (!buffer) {
      console.warn(`❌ No buffer loaded for ${drumType}`);
      return;
    }
   ...
   */
  triggerAttackRelease(drumType: string, duration: string, time?: number, velocity = 1): void {
