...
export class RhodesVelocitySampler {
  ...
  private async ensureToneLoaded(): Promise<void> {
    await loadGlobalTone(undefined, this.audioEngine);
  ...
  private async loadLayer(layer: string): Promise<void> {
    ...
    const loadPromise = (async () => {
      try {
        ...
        logger.info(`Loading Rhodes ${layer} with samples:`, {
          samples: Object.keys(sampleSubset),
        });
        ...
        try {
          ...
          logger.info(`Rhodes ${layer} sample test failed:`, {
        ...
        logger.error(
          `❌ Failed to load Rhodes layer ${layer}:`,
          error instanceof Error ? error : new Error(String(error)),
        );
        throw error;
      } finally {
    ...
    this.samplers.forEach((sampler) => {
      if (sampler && sampler.loaded) {
        try {
          ...
          // Release all notes
          sampler.releaseAll((Tone as any).immediate());
          ...
          logger.warn('Failed to release notes on Rhodes sampler:', {
  ...
  private createSampler(options?: any): any {
    ...
    return new (Tone as any).Sampler(options);
  }
}
