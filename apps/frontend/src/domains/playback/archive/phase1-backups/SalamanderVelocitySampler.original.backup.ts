/**
 * Professional 16-velocity Salamander Grand Piano sampler - REFACTORED
 * Now loads configuration from external JSON file
 * Reduced from 2,370 lines to ~500 lines
 */
...
const CONFIG_PATH = 'instruments/piano/salamander-piano.json';
...
export class SalamanderVelocitySampler {
  ...
  private damperReleaseSampler: any | null = null;
  ...
  async initialize(
    ...
    try {
      ...
      logger.info('🎹 Loaded Salamander configuration', {
      ...
      const checkPreloadedSamples = () => {
        ...
        const layersToCheck = velocityLayers ||
          this.config!.defaultLayers || ['v1', 'v6', 'v10', 'v14', 'v16'];
      ...
      const layersToLoad = velocityLayers ||
        ...
        this.config.defaultLayers || ['v10', 'v1', 'v16'];
      ...
      // Load mechanical sounds if configured
      if (this.config.mechanicalSounds?.damperRelease) {
        await this.loadDamperReleaseSounds();
      }

      this.isInitialized = true;
      logger.info('✅ Salamander Grand Piano ready!', {
      ...
      logger.error(
        'Failed to initialize Salamander:',
  ...
  private async loadLayer(
    ...
    const loadPromise = (async () => {
      try {
        ...
        logger.info(`🎹 Loading Salamander layer ${layer}`, {
        ...
        logger.info(`✅ Loaded Salamander layer ${layer}`);
      } catch (error) {
        logger.error(
  ...
  private async createSampler(urls: Record<string, string>): Promise<any> {
    return new Promise((resolve, reject) => {
      ...
      const sampler = new (Tone as any).Sampler({
        urls,
        baseUrl,
        release: this.config?.samplerConfig?.release || 0.05,
        attack: this.config?.samplerConfig?.attack || 0,
        onload: () => {
  ...
  /**
   * Load damper release sounds
   */
  private async loadDamperReleaseSounds(): Promise<void> {
    if (!this.config?.mechanicalSounds?.damperRelease) return;

    try {
      const damperConfig = this.config.mechanicalSounds.damperRelease;
      const damperUrl = this.loader.buildSampleUrl(
        this.config,
        Object.keys(damperConfig.samples)[0],
        'mechanical',
      );

      this.damperReleaseSampler = new (Tone as any).Player({
        url: damperUrl,
        volume: damperConfig.volume || -10,
        fadeOut: 0.05,
      });
      ...
      if (this.destination) {
        this.damperReleaseSampler.connect(this.destination);
      }

      logger.info('✅ Damper release sound loaded');
    } catch (error) {
  ...
  async triggerAttackRelease(
    ...
    if (!this.isInitialized || !this.config) {
      logger.warn('🎹 Salamander not initialized, cannot play');
      return;
    }
    ...
    if (sampler && sampler.loaded) {
      try {
        ...
        // Track notes for damper release
        for (const n of notes) {
          this.activeNotes.set(n, velocity);

          // Schedule damper release sound
          if (this.damperReleaseSampler && typeof duration === 'number') {
            const releaseTime = (time || Tone.now()) + duration;
            this.damperReleaseSampler.start(releaseTime);
          }
        }
  ...
  triggerRelease(note: string | string[], time?: any): void {
    ...
    for (const n of notes) {
      ...
      if (velocity !== undefined) {
        ...
        // Trigger damper release sound
        if (this.damperReleaseSampler) {
          this.damperReleaseSampler.start(time || Tone.now());
        }
      }
    }
  }
  ...
  connect(destination: any): void {
    ...
    if (this.damperReleaseSampler) {
      this.damperReleaseSampler.connect(destination);
    }
  }
  ...
  disconnect(): void {
    ...
    if (this.damperReleaseSampler) {
      this.damperReleaseSampler.disconnect();
    }
  }
   ...
   */
  getStatus(): any {
    const memoryUsage = this.loadedLayers.size * 15; // ~15MB per layer estimate
  ...
  async dispose(): Promise<void> {
    ...
    logger.info('💀 Salamander disposed');
  }
}
