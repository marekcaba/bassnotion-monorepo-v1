...
// Import createStructuredLogger
import { createStructuredLogger } from '../../../shared/index.js';

const logger = createStructuredLogger('SalamanderVelocitySampler');
...
function _syncToneContext(audioContext?: BaseAudioContext): void {
  ...
  if (toneContext !== audioContext) {
    ...
    if (
      persistentContext &&
      (toneContext === persistentContext || audioContext === persistentContext)
    ) {
    ...
    logger.info(
      '🎹 AudioContext provided but NOT switching Tone.js context to avoid buffer invalidation',
    );
...
function safeConnect(source: any, destination: any, label?: string): boolean {
  try {
    ...
    if (!isValidDest) {
      logger.error(
        `Invalid destination type for connection ${label || ''}: ${typeof destination}`,
      );
      return false;
    }
    ...
    if (sourceContext && destContext && sourceContext !== destContext) {
      ...
      const _isSourceNative = sourceContext instanceof AudioContext;
      const _isDestNative = destContext instanceof AudioContext;

      // Get the underlying native contexts
      const sourceNativeContext = isSourceTone
        ? sourceContext._context ||
          sourceContext.rawContext ||
          sourceContext._nativeAudioContext
        : sourceContext;
      const destNativeContext = isDestTone
        ? destContext._context ||
          destContext.rawContext ||
          destContext._nativeAudioContext
        : destContext;
      ...
      const usesPersistentContext =
        persistentContext &&
        (sourceNativeContext === persistentContext ||
          destNativeContext === persistentContext);
      ...
      if (!contextsMatch && !usesPersistentContext && !bothUseTone) {
        ...
        logger.warn(
          `⚠️ AudioContext mismatch in ${label || 'connection'}, but proceeding anyway`,
          {
    ...
    try {
      ...
      if (connectError.name === 'InvalidAccessError') {
        ...
        logger.debug(
          `${label || 'Nodes'} already connected, skipping reconnection`,
        );
        return true;
      }
...
export class SalamanderVelocitySampler {
  ...
  async initialize(
    ...
    const checkPreloadedSamples = () => {
      ...
      const layersToCheck = velocityLayers || ['v1', 'v6', 'v10', 'v14', 'v16'];
    ...
    logger.info('🎹 Initializing Salamander Grand Piano', {
      ...
      expectedSamples: (velocityLayers?.length || 5) * 30,
    });
    ...
    try {
      ...
      logger.error(
        '❌ Failed to start Tone.js context',
        error instanceof Error ? error : new Error(String(error)),
      );
      throw error;
    }
    ...
    logger.info('🎹 Loading velocity layers', { layers: layersToLoad });
    ...
    for (const layer of layersToLoad) {
      ...
      if (sampler && !sampler.loaded) {
        ...
        try {
          ...
          logger.error(`🎹 Failed to wait for layer ${layer} to load`);
        }
      }
    }
      ...
      try {
        ...
        notes.forEach((note) => {
          ...
          if (this.sampleMapping[note as keyof typeof this.sampleMapping]) {
            ...
            const _noteName = note.replace(/\d+/, '');
        ...
        if (allCached) {
          ...
          sampler = this.createSampler({
            ...
            onerror: (error: any) => {
              logger.error(
                `❌ Failed to smart load layer ${layer}:`,
                error instanceof Error ? error : new Error(String(error)),
              );
            },
          });
  ...
  private async loadLayer(layer: string): Promise<void> {
    ...
    const loadPromise = (async () => {
      try {
        // Ensure Tone.js context is running before creating sampler
        logger.info(
          `🎹 Tone.context state before creating sampler: ${Tone.context.state}`,
        );
        ...
        if (Tone.context.state !== 'running') {
          ...
          try {
            await Tone.start();
            logger.info(
              '🎹 Tone.start() completed, state:',
              Tone.context.state,
            );
          } catch (e: unknown) {
            logger.warn('🎹 Tone.start() failed', {
        ...
        try {
          ...
          firstFewNotes.forEach((note) => {
            logger.info(
              `🎹   ${note}: ${baseUrl}${this.sampleMapping[note as keyof typeof this.sampleMapping] || ''}`,
            );
          });
          ...
          try {
            ...
            if (!response.ok) {
              logger.error(
                `❌ Failed to fetch test sample: ${response.status} ${response.statusText}`,
              );
            } else {
            ...
            logger.error(
              `❌ Network error fetching test sample:`,
          ...
          if (allCached) {
            ...
            const loadTimeout = setTimeout(() => {
              loadReject(
                new Error(`Timeout loading layer ${layer} after 30 seconds`),
              );
            }, 30000);
            ...
            try {
              ...
              clearTimeout(loadTimeout);
              logger.error(
                `Failed to load sampler for layer ${layer}:`,
          ...
          logger.error(
            ...
            samplerError instanceof Error
              ? samplerError
              : new Error(String(samplerError)),
          );
          // Try creating a basic sampler without any options
          try {
            sampler = this.createSampler();
            logger.warn(`Created basic sampler for layer ${layer} as fallback`);
          } catch (fallbackError) {
            logger.error(
        ...
        try {
          ...
          try {
            ...
            logger.debug(
              `🎹 Tone.loaded() had issues for layer ${layer}, but continuing:`,
          ...
          logger.debug(
            ...
            loadError as Record<string, unknown>,
          );
        ...
        if (!sampler.output) {
          logger.error(
            `🎹 CRITICAL: Sampler for layer ${layer} has no output property!`,
          );
          logger.info(`🎹 Sampler properties:`, {
            keys: Object.keys(sampler).join(', '),
          });
        ...
        if (this.destination) {
          try {
            ...
            if (sampler && sampler.connect) {
              logger.info(
                `🎹 Connecting layer ${layer} sampler to destination...`,
              );
              sampler.connect(this.destination);
              logger.info(
                `✅ Connected newly loaded layer ${layer} to destination`,
              );
            ...
            try {
              logger.info(
                `🎹 Trying fallback connection for layer ${layer}...`,
              );
              sampler.toDestination();
              logger.info(
                `✅ Connected layer ${layer} to Tone.Destination as fallback`,
              );
            } catch (fallbackErr) {
              logger.error(
                `Failed fallback connection:`,
          ...
          logger.info(
            `🎹 No destination specified, connecting to Tone.Destination`,
          );
          sampler.toDestination();
        }
  ...
  private async loadDamperReleaseSound(): Promise<void> {
    try {
      ...
      await ensureToneLoaded(undefined, this.audioEngine);
      ...
      this.damperReleaseSampler = this.createPlayer({
        url: damperUrl,
        onload: () => {
          logger.info('Damper release sound loaded successfully');
        },
        onerror: (error: any) => {
          logger.warn('Failed to load damper release sound:', {
      ...
      logger.error(
        'Failed to load damper release sound:',
        error instanceof Error ? error : new Error(String(error)),
      );
      ...
      this.samplers.forEach((sampler, layer) => {
        if (sampler && sampler.loaded) {
          if (sampler.volume && sampler.volume.value !== undefined) {
            ...
            logger.info(
              `🎹 Sampler ${layer} volume: ${currentValue}dB (not modifying)`,
            );

            // If volume is extremely low or muted, warn but don't change
            if (currentValue === -Infinity || currentValue < -40) {
              logger.warn(
                `🎹 WARNING: Sampler ${layer} volume is very low: ${currentValue}dB`,
              );
              logger.warn(
                `🎹 If no sound is heard, check the audio routing and gain stages`,
              );
            }
          } else if (
            sampler.volume &&
            typeof sampler.volume.gain !== 'undefined'
          ) {
    ...
    for (let i = 0; i < notes.length; i++) {
      ...
      if (!this.loadedLayers.has(layer) || !this.samplers.has(layer)) {
        ...
        if (this.loadedLayers.has('v10') && this.samplers.has('v10')) {
          logger.info(
            `🎹 Layer ${layer} not ready, using v10 for instant playback`,
          );
          layer = 'v10';
        } else {
          ...
          if (availableLayers.length > 0) {
            layer = availableLayers[0] || originalLayer;
      ...
      if (sampler) {
        try {
          ...
          const noteDurationSeconds = (Tone as any).Time(duration).toSeconds();
          // Only convert to absolute time when we need it for calculations
          const actualNoteTime =
            noteTime !== undefined
              ? Tone.Time(noteTime).toSeconds()
              : Tone.now();
          const _releaseTime = actualNoteTime + noteDurationSeconds;
          ...
          try {
            ...
            if (sampler.volume) {
              ...
              if (sampler.volume.mute) {
                logger.warn(
                  `🎹 WARNING: Sampler ${layer} is muted! Unmuting...`,
                );
              ...
              if (sampler.volume.value !== undefined) {
                logger.info(
                  `🎹 Sampler ${layer} volume: ${sampler.volume.value}dB (not modifying)`,
                );

                // If volume is extremely low or muted, warn but don't change
                if (
                  sampler.volume.value === -Infinity ||
                  sampler.volume.value < -40
                ) {
                  logger.warn(
                    `🎹 WARNING: Sampler ${layer} volume is very low: ${sampler.volume.value}dB`,
                  );
                  logger.warn(
                    `🎹 If no sound is heard, check the audio routing and gain stages`,
                  );
              ...
              logger.info(
                `🎹 Sampler ${layer} has no volume property - volume controlled through gain node chain`,
              );
            }
            ...
            logger.info(
              `🎹 SUCCESS: triggerAttackRelease completed for ${currentNote} on layer ${layer}`,
              {
            ...
            if (error instanceof Error && error.message.includes('buffer')) {
              ...
              const _velocityRange = this.getLayerForVelocity(velocity);
              ...
              for (const commonLayer of commonLayers) {
                ...
                if (candidateSampler) {
                  logger.info(`🎹 Checking layer ${commonLayer}: exists=true`);
              ...
              if (fallbackLayer) {
                ...
                logger.info(`🎹 Checked samplers:`, {
                  samplers: Array.from(this.samplers.keys()).join(', '),
                });

                // Emergency fallback: Create a simple sine wave for the note
                logger.info(
                  `🎹 Creating emergency sine wave fallback for ${currentNote}`,
                );
                try {
                  // Simple MIDI to frequency conversion (A4 = 440Hz, MIDI 69)
                  const noteNum =
                    parseInt(currentNote?.replace(/[A-G#]/g, '') || '') || 60; // Default to C4
                  ...
                  const ctx =
                    ...
                    this.preferredContext ||
                    Tone?.context?._context ||
                    Tone?.getContext?.()?._context;
                  if (!ctx) {
                    throw new Error(
                      'No audio context available for emergency fallback',
                    );
                  }
                  ...
                  logger.info(
                    `🎹 Emergency fallback played for ${currentNote} at ${freq}Hz`,
                  );
                } catch (emergencyError) {
                  logger.error(
                    `🎹 Emergency fallback also failed for ${currentNote}:`,
                    emergencyError instanceof Error
                      ? emergencyError
                      : new Error(String(emergencyError)),
                  );
                }
                logger.info(`🎹 Loaded layers:`, {
                  layers: Array.from(this.loadedLayers).join(', '),
                });
              }
            }
          ...
          if (
            ...
            setTimeout(() => {
              try {
                ...
                const releasePlayer = this.createPlayer(
                  (this.damperReleaseSampler as any)?.buffer,
                );
                ...
                volumeNode = this.createVolume(totalVolumeDb);
                ...
                logger.warn('Failed to play damper release sound:', {
                  error:
                    error instanceof Error ? error : new Error(String(error)),
                });
              }
    ...
    if (
      ...
      for (const currentNote of notes) {
        try {
          ...
          logger.warn('Failed to play damper release sound for note:', {
            note: currentNote,
            error: error instanceof Error ? error : new Error(String(error)),
          });
        }
  ...
  connect(destination: any): this {
    ...
    if (
      ...
      if (destinationContext) {
        ...
        (async () => {
          await ensureToneLoaded(destinationContext as AudioContext);
      ...
      if (sampler) {
        try {
          ...
          if (sampler.connect) {
            ...
            logger.info(
              `✅ Connected sampler layer ${layer} to destination using Tone.connect()`,
            );
          ...
          logger.error(
            `Failed to connect sampler layer ${layer}:`,
            err instanceof Error ? err : new Error(String(err)),
          );
  ...
  async preloadNotes(notes: string[]): Promise<void> {
    logger.info('🎹 Smart loading: Preloading only notes:', {
      notes: notes.join(', '),
    });
  ...
  stopAll(): void {
    ...
    if (Tone && Tone.Transport) {
      try {
        ...
        logger.warn('Failed to cancel Transport events:', {
          error: error as Record<string, unknown>,
        });
      }
    }
    ...
    this.samplers.forEach((sampler, layer) => {
      try {
        ...
        try {
          ...
          logger.warn(`Failed to disconnect/reconnect sampler ${layer}:`, {
        ...
        logger.warn(`Failed to stop voices on layer ${layer}:`, {
          error: error as Record<string, unknown>,
        });
      }
    });
    ...
    this.samplers.forEach((sampler, layer) => {
      if (sampler && sampler.loaded) {
        try {
          ...
          logger.error(
            `🚨 Failed to emergency stop layer ${layer}:`,
            error instanceof Error ? error : new Error(String(error)),
          );
        }
      }
  ...
  public midiPanic(): void {
    ...
    try {
      ...
      logger.error(
        '🚨 SalamanderVelocitySampler.midiPanic() failed:',
        error instanceof Error ? error : new Error(String(error)),
      );
    }
  }
  ...
  public previewNote(note: string, velocity = 127, duration = '8n'): void {
    ...
    try {
      ...
      logger.warn('🎹 Failed to play preview note:', {
        error: error as Record<string, unknown>,
      });
    }
