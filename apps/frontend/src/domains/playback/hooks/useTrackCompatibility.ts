...
export function useTrackCompatibility(
  ...
  const debug = useCallback((message: string, data?: any) => {
    if (debugMode) {
      console.log(`🎵 useTrackCompatibility[${widgetId}]: ${message}`, data);
    }
  }, [widgetId, debugMode]);
  ...
  useEffect(() => {
    ...
    const unsubscribe = eventBusRef.current.on('widget:triggerEvent', handleTrigger);
  ...
  useEffect(() => {
    ...
    const unsubscribe = eventBusRef.current.on('widget:stateUpdated', handleStateChange);
    cleanupRef.current.push(unsubscribe);
   ...
   * Update pattern
   */
  const updatePattern = useCallback((newPattern: Pattern) => {
  ...
  const migrateToTrack = useCallback(async (): Promise<Track | null> => {
    ...
    try {
      ...
      const migratedTrack = await adapterRef.current.migrateWidgetToTrack(widgetId);
      ...
      console.error('Failed to migrate widget to track:', error);
      return null;
    }
  }, [widgetId, isRegistered, debug]);
