...
import { TrackMixingEngine, type TrackChannel, type MixBus } from '../services/core/TrackMixingEngine.js';
...
export interface TrackMixingControls {
  ...
  createSend: (auxBusId: string, level?: number, sendPoint?: 'pre-fader' | 'post-fader') => string;
 ...
 */
export function useTrackMixing(options: UseTrackMixingOptions): TrackMixingControls {
  ...
  // State
  const [mixingState, setMixingState] = useState<TrackMixingState>(() => track.mixing);
  const [channel, setChannel] = useState<TrackChannel | undefined>();
  
  // Debug logging
  const debug = useCallback((message: string, data?: any) => {
    if (debugMode) {
      console.log(`🎛️ useTrackMixing[${track.id}]: ${message}`, data);
    }
  }, [track.id, debugMode]);
  ...
  useEffect(() => {
    ...
    const unsubUpdate = eventBusRef.current.on('mixing:trackUpdated', (data: any) => {
    ...
    const unsubSolo = eventBusRef.current.on('mixing:soloStateChanged', (data: any) => {
   ...
   * Add effect to track
   */
  const addEffect = useCallback((effect: Tone.ToneAudioNode) => {
    if (!mixingEngineRef.current) return;
  ...
  const createSend = useCallback((
    ...
    return mixingEngineRef.current.createSend(track.id, auxBusId, level, sendPoint);
  }, [track.id, debug]);
  
  /**
   * Update send level
   */
  const updateSendLevel = useCallback((auxBusId: string, level: number) => {
    if (!mixingEngineRef.current) return;
...
export interface UseMixBusesState {
  ...
  // Effects returns
  createReverbReturn: (name?: string, roomSize?: number, dampening?: number) => string;
  createDelayReturn: (name?: string, delayTime?: string, feedback?: number) => string;
  createCompressionReturn: (name?: string, threshold?: number, ratio?: number) => string;
...
export function useMixBuses(): UseMixBusesState {
  ...
  const createSubBus = useCallback((name: string, parentBusId: string = 'master'): string => {
    if (!mixingEngineRef.current) throw new Error('Mixing engine not initialized');
  ...
  const createAuxBus = useCallback((name: string): string => {
    if (!mixingEngineRef.current) throw new Error('Mixing engine not initialized');
  ...
  const createReverbReturn = useCallback((
    ...
    const busId = mixingEngineRef.current.createReverbReturn(name, roomSize, dampening);
    updateBuses();
    return busId;
  }, [updateBuses]);
  ...
  const createDelayReturn = useCallback((
    ...
    const busId = mixingEngineRef.current.createDelayReturn(name, delayTime, feedback);
    updateBuses();
    return busId;
  }, [updateBuses]);
  ...
  const createCompressionReturn = useCallback((
    ...
    const busId = mixingEngineRef.current.createCompressionReturn(name, threshold, ratio);
    updateBuses();
    return busId;
  }, [updateBuses]);
  
  const addBusEffect = useCallback((busId: string, effect: Tone.ToneAudioNode) => {
    if (!mixingEngineRef.current) return;
  ...
  const removeBusEffect = useCallback((busId: string, effectIndex: number) => {
    if (!mixingEngineRef.current) return;
