...
export interface UsePluginsResult {
  isReady: boolean;
  error: Error | null;
  getPlugin: <T extends AudioPlugin = AudioPlugin>(pluginId: string) => T | null;
  getAllPlugins: () => Map<string, AudioPlugin>;
  getPluginsByCapability: (capability: string) => AudioPlugin[];
  activatePlugin: (pluginId: string) => Promise<void>;
  deactivatePlugin: (pluginId: string) => Promise<void>;
  getPluginState: (pluginId: string) => PluginState | undefined;
  registerPlugin: (plugin: AudioPlugin, dependencies?: string[]) => Promise<void>;
}

export function usePlugins(serviceRegistry?: ServiceRegistry): UsePluginsResult {
  ...
  useEffect(() => {
    try {
      ...
      if (!registry) {
        throw new Error('ServiceRegistry not found. Ensure AudioProvider is properly configured.');
      }
  ...
  useEffect(() => {
    ...
    const handlePluginLoaded = (data: { pluginId: string; state: PluginState }) => {
      setPluginStates(prev => new Map(prev).set(data.pluginId, data.state));
    };

    const handlePluginActivated = (data: { pluginId: string; state: PluginState }) => {
      setPluginStates(prev => new Map(prev).set(data.pluginId, data.state));
    };

    const handlePluginDeactivated = (data: { pluginId: string; state: PluginState }) => {
      setPluginStates(prev => new Map(prev).set(data.pluginId, data.state));
    };
    ...
    const unsubscribeLoaded = eventBus.on('plugin-manager:plugin-loaded', handlePluginLoaded);
    const unsubscribeActivated = eventBus.on('plugin-manager:plugin-activated', handlePluginActivated);
    const unsubscribeDeactivated = eventBus.on('plugin-manager:plugin-deactivated', handlePluginDeactivated);
    const unsubscribeError = eventBus.on('plugin-manager:error', handlePluginError);
  ...
  const getPlugin = useCallback(<T extends AudioPlugin = AudioPlugin>(pluginId: string): T | null => {
    ...
    try {
      ...
      console.error(`Failed to get plugin ${pluginId}:`, err);
      return null;
    }
  }, []);
  ...
  const getAllPlugins = useCallback((): Map<string, AudioPlugin> => {
    ...
    try {
      ...
      console.error('Failed to get all plugins:', err);
      return new Map();
    }
  }, []);

  // Get plugins by capability
  const getPluginsByCapability = useCallback((capability: string): AudioPlugin[] => {
    if (!pluginManagerRef.current) {
      return [];
    }
    ...
    try {
      ...
      console.error(`Failed to get plugins by capability ${capability}:`, err);
      return [];
    }
  }, []);
  ...
  const deactivatePlugin = useCallback(async (pluginId: string): Promise<void> => {
    ...
    try {
      ...
      const error = err instanceof Error ? err :
        new Error(`Failed to deactivate plugin ${pluginId}: Unknown error`);
      throw error;
    }
  }, []);
  ...
  const registerPlugin = useCallback(async (plugin: AudioPlugin, dependencies?: string[]): Promise<void> => {
    if (!pluginManagerRef.current) {
      throw new Error('PluginManager not available');
    }
