...
export function useWAMPlugin(options: UseWAMPluginOptions): UseWAMPluginReturn {
  ...
  const debug = useCallback(
    (message: string, data?: any) => {
      if (debugMode) {
        logger.info(`🔌 useWAMPlugin[${pluginUrl}]: ${message}`, data);
      }
    },
    [pluginUrl, debugMode],
  );
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (typeof track.loadWAMPlugin === 'function') {
        ...
        logger.warn(
          'Track does not have loadWAMPlugin method. Manual plugin loading not supported.',
        );
  ...
  const setParameter = useCallback(
    (name: string, value: any) => {
      ...
      if (plugin.setParameter) {
        ...
        plugin.audioNode &&
        plugin.audioNode.parameters &&
        plugin.audioNode.parameters[name]
      ) {
