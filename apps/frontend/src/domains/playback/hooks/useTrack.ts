...
import { TrackState, type TrackConfig, type InstrumentType } from '../types/track.js';
...
function waitForServices(timeout = 10000): Promise<void> {
  return new Promise((resolve, reject) => {
    ...
    const checkServices = () => {
      ...
      // Also check if serviceRegistry proxy works
      try {
...
interface UseTrackReturn {
  ...
  // Region management
  regions: Region[];
  createRegionFromPattern: (pattern: Pattern, config?: Partial<Region>) => Region;
...
export function useTrack(options: UseTrackOptions): UseTrackReturn {
  ...
  const debug = useCallback((message: string, data?: any) => {
    if (debugMode) {
      console.log(`🎵 useTrack[${trackId}]: ${message}`, data);
    }
  }, [trackId, debugMode]);
  ...
  useEffect(() => {
    ...
    const initializeTrack = async () => {
      try {
        ...
        const trackInstance = await trackManagerRef.current.createTrack(trackConfig);
        ...
        if (eventBusRef.current) {
          eventBusRef.current.on('transport:play', () => setIsPlaying(true));
          eventBusRef.current.on('transport:stop', () => setIsPlaying(false));
          eventBusRef.current.on('transport:pause', () => setIsPlaying(false));
          eventBusRef.current.on('transport:tempo-change', (newTempo: number) => setTempo(newTempo));
          eventBusRef.current.on('transport:time-update', (time: number) => setCurrentTime(time));
        }
        
      } catch (err) {
        console.error(`Failed to initialize track ${trackId}:`, err);
    ...
    return () => {
      if (track && trackManagerRef.current) {
        trackManagerRef.current.deleteTrack(trackId).catch(err => {
          console.error(`Failed to delete track ${trackId}:`, err);
        });
      }
      if (eventBusRef.current) {
        eventBusRef.current.off('transport:play');
        eventBusRef.current.off('transport:stop');
        eventBusRef.current.off('transport:pause');
        eventBusRef.current.off('transport:tempo-change');
        eventBusRef.current.off('transport:time-update');
      }
    };
   ...
   * Solo/unsolo track
   */
  const solo = useCallback((soloed?: boolean) => {
    if (!track) return;
   ...
   */
  const loadWAMPlugin = useCallback(async (pluginUrl: string, options?: any) => {
    if (!track) {
   ...
   */
  const createRegionFromPattern = useCallback((pattern: Pattern, config?: Partial<Region>) => {
    if (!track) {
      throw new Error('Track not initialized');
    }
   ...
   */
  const migratePatternToRegion = useCallback((widgetId: string, pattern: Pattern) => {
   ...
   */
  const getRegionsInRange = useCallback((startPos: MusicalPosition, endPos: MusicalPosition) => {
    if (!track) return [];
   ...
   */
  const getRegionsAtPosition = useCallback((position: MusicalPosition) => {
    if (!track) return [];
   ...
   * Select region
   */
  const selectRegion = useCallback((regionId: string, addToSelection = false) => {
    if (!track) return;
