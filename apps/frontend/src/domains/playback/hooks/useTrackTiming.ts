...
import { MultiTrackTimingSynchronizer, type TrackTimingState, type TrackSyncMetrics } from '../services/core/MultiTrackTimingSynchronizer.js';
import { TimingIsolationManager, type IsolatedTrackInfo } from '../services/core/TimingIsolationManager.js';
 ...
 */
export function useTrackTiming(options: UseTrackTimingOptions): TrackTimingHookState {
  const {
    track,
    priority = 50,
    onIsolated,
  ...
  const [isolationInfo, setIsolationInfo] = useState<IsolatedTrackInfo | null>(null);
  ...
  const debug = useCallback((message: string, data?: any) => {
    if (debugMode) {
      console.log(`🎯 useTrackTiming[${track.id}]: ${message}`, data);
    }
  }, [track.id, debugMode]);
  ...
  useEffect(() => {
    ...
    const unsubDrift = eventBusRef.current.on('timing:driftViolation', (data: any) => {
    ...
    const unsubSync = eventBusRef.current.on('timing:syncReport', (report: any) => {
      const trackMetric = report.tracks?.find((t: any) => t.trackId === track.id);
      if (trackMetric) {
    ...
    const unsubIsolated = eventBusRef.current.on('isolation:trackIsolated', (info: IsolatedTrackInfo) => {
    ...
    const unsubRecovered = eventBusRef.current.on('isolation:trackRecovered', (data: any) => {
      if (data.trackId === track.id) {
