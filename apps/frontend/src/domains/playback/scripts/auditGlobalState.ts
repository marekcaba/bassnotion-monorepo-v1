  ...
  lines.forEach((line, index) => {
    ...
    GLOBAL_STATE_PATTERNS.forEach(pattern => {
      if (pattern.test(line)) {
        ...
        if (/window\.AudioContext\s*\|\|/.test(line) || 
            /typeof\s+window\.AudioContext/.test(line)) {
          return; // This is just feature detection, not pollution
        }
        
        // Special case: Allow test mocks
        if (filePath.includes('__tests__') || filePath.includes('.test.') || filePath.includes('.spec.') || filePath.includes('mock')) {
...
function runAudit(baseDir: string): void {
  console.log('🔍 Running Global State Audit...\n');
  ...
  if (allResults.length === 0) {
    console.log('✅ PASS: No global state patterns found!\n');
    console.log('The playback domain is clean of global state pollution.');
  } else {
    console.log(`❌ FAIL: Found ${allResults.length} global state violations:\n`);
