...
export function extractExerciseNotes(exercise: Exercise | undefined): string[] {
  if (!exercise?.chord_progression || exercise.chord_progression.length === 0) {
    console.warn('No chord progression found in exercise');
    return [];
  }

  console.log('📝 Extracting notes from exercise:', {
  ...
  for (const chord of exercise.chord_progression) {
    ...
    if (notes) {
      console.log(`📝 Chord ${chord} maps to notes:`, notes);
  ...
  if (exercise.notes && exercise.notes.length > 0) {
    for (const noteData of exercise.notes) {
      if (noteData.note) {
        ...
        const noteWithOctave = noteData.note.length === 1 
  ...
  console.log(`📝 Extracted ${result.length} unique notes from exercise:`, result);
 ...
 */
export function getVelocityLayersForExercise(exercise: Exercise | undefined): string[] {
  if (!exercise) {
