...
export class BeatTimingAnalyzer {
  ...
  start(tempo: number, timeSignature = { numerator: 4, denominator: 4 }) {
    ...
    this.beatHistory = [];
    console.log(`🎯 Beat Timing Analyzer started: ${tempo} BPM, ${timeSignature.numerator}/${timeSignature.denominator}, startTime: ${this.startTime}`);
  }
  
  /**
   * Record a beat event
   */
  recordBeat(source: string, beatNumber: number, measureNumber: number, actualTime?: number) {
    if (!this.startTime) {
    ...
    if (this.beatHistory.filter(b => b.source === source).length === 0) {
      console.log(`🎯 First beat recorded from ${source}: beat ${beatNumber}, measure ${measureNumber}, startTime: ${this.startTime}`);
    }
    ...
    const recentDuplicate = this.beatHistory.slice(-10).find(beat => 
      beat.source === source && 
    ...
    if (recentDuplicate) {
      console.warn(`⚠️ Double beat report detected: ${source} measure ${measureNumber} beat ${beatNumber} (${elapsedTime.toFixed(0)}ms vs ${recentDuplicate.actualTime.toFixed(0)}ms)`);
    ...
    if (Math.abs(drift) > 10) {
      console.warn(`⚠️ ${source} beat ${beatNumber} drift: ${drift.toFixed(1)}ms ${drift > 0 ? 'late' : 'early'}`);
    }
  ...
  getStatistics(source?: string): TimingStatistics {
    ...
    const variance = drifts.reduce((sum, d) => {
   ...
   */
  compareSourceTiming(source1: string, source2: string): {
    ...
    beats1.forEach(beat1 => {
      const matchingBeat = beats2.find(
        b => b.measureNumber === beat1.measureNumber && b.beatNumber === beat1.beatNumber
      );
    ...
    const averageDifference = timingDifference.reduce((sum, d) => sum + d, 0) / timingDifference.length;
    
    // Calculate correlation (how well they track together)
    const variance = timingDifference.reduce((sum, d) => {
