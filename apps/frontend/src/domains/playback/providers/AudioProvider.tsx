...
export function AudioProvider({ children, config }: AudioProviderProps) {
  ...
  useEffect(() => {
    if (!hasLoggedFlags) {
      console.log('AudioProvider: Feature flags:', flags);
  ...
  if (!isNewAudioArchitectureEnabled()) {
    logMigrationEvent('Using legacy ToneProvider', { reason: 'feature flags disabled' });
  ...
  useEffect(() => {
    ...
    async function initializeServices() {
      try {
        ...
        if (existingInstance) {
          console.log('AudioProvider: Using existing global audio system instance');
          ...
          console.log('AudioProvider: Context state updated with existing services - isInitialized:', true);
          
          logMigrationEvent('AudioProvider reusing existing global instance');
          return;
        }
        
        logMigrationEvent('Initializing new AudioProvider with global singleton');
        
        // Add timeout to detect if initialization hangs
        const timeoutPromise = new Promise<never>((_, reject) => {
          setTimeout(() => reject(new Error('createCoreServices timed out after 10 seconds')), 10000);
        });
        
        console.log('AudioProvider: Starting createCoreServicesWithPreInit...');
        ...
        try {
          ...
          console.log('AudioProvider: createCoreServicesWithPreInit completed successfully');
        } catch (timeoutError) {
          console.error('AudioProvider: createCoreServicesWithPreInit timed out or failed:', timeoutError);
          throw timeoutError;
        }
        ...
        const handleAudioInitialized = async () => {
          console.log('AudioProvider: audio:initialized event received, fully initializing services...');
          try {
            // Now fully initialize all services (including UnifiedTransport)
            await services.initialize();
            console.log('AudioProvider: All services fully initialized after user gesture');
          } catch (error) {
            console.error('AudioProvider: Failed to fully initialize services:', error);
          }
        };
        ...
        console.log('AudioProvider: Context state updated - isInitialized:', true);
        
        logMigrationEvent('AudioProvider initialized successfully', {
          services: registry.getServiceNames ? registry.getServiceNames() : []
        });
      } catch (err) {
        const error = err instanceof Error ? err : new Error('Failed to initialize audio services');
        console.error('AudioProvider: Initialization failed', {
          error,
        ...
        setError(error);
        logMigrationEvent('AudioProvider initialization failed', { error: error.message });
      }
    }
    
    initializeServices().catch(err => {
      console.error('AudioProvider: Failed to initialize services:', err);
    ...
    return () => {
      ...
      if (cleanupRef.current) {
        console.log('AudioProvider: Skipping cleanup (already cleaned up)');
        return;
      }
      ...
      if (services) {
        ...
        setTimeout(() => {
          if (cleanupRef.current) {
            ...
            console.log('AudioProvider: Cleanup called but preserving global singleton');
          }
