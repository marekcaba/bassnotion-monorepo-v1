...
describe('Performance Benchmarking', () => {
  ...
  describe('Baseline Performance (Old Architecture)', () => {
    it('should measure baseline performance without protection', async () => {
      ...
      console.log(
        `- Average Duration: ${(duration / iterations).toFixed(3)}ms`,
      );
  ...
  describe('Enhanced Architecture Performance', () => {
    it('should measure performance with full protection stack', async () => {
      ...
      for (let i = 0; i < iterations; i++) {
        await errorBoundary.protect('benchmark', 'lightOperation', () =>
          circuitBreaker.execute(() =>
            performanceMonitor.measure('benchmark', 'lightOperation', () =>
    ...
    it('should measure error recovery performance', async () => {
      ...
      for (let i = 0; i < iterations; i++) {
        try {
          await errorBoundary.protect('benchmark', 'operation', () =>
  ...
  describe('Resource Pooling Performance', () => {
    it('should measure resource pool efficiency', async () => {
      ...
      for (let i = 0; i < iterations; i++) {
        operations.push(
          performanceMonitor.measure(
            ...
            async () => {
              const resource =
                performanceMonitor.acquireFromPool('connections');
              if (!resource) {
      ...
      console.log(
        `- Throughput: ${(successful / (duration / 1000)).toFixed(0)} ops/sec`,
      );
  ...
  describe('Overhead Analysis', () => {
    it('should measure protection overhead', async () => {
      ...
      console.log(
        `- Overhead per Operation: ${(overhead / iterations).toFixed(3)}ms`,
      );
    ...
    it('should measure memory impact', () => {
      ...
      for (let i = 0; i < 10; i++) {
        ...
        breakers.push(
          circuitBreakerFactory.create(`breaker-${i}`, 'high-throughput', {
            retryPolicy: {
              maxRetries: 0,
              retryableErrors: [],
            },
      ...
      console.log(
        `- Per Instance: ${(memoryIncrease / 10 / 1024).toFixed(2)}KB`,
      );
  ...
  describe('Scalability Analysis', () => {
    it('should measure scalability with increasing load', async () => {
      ...
      results.forEach((r) => {
        console.log(
          `- Load ${r.load}: ${r.duration.toFixed(2)}ms total, ${r.avgDuration.toFixed(3)}ms avg, ${r.throughput.toFixed(0)} ops/sec`,
        );
      });
