...
import { EnhancedPerformanceMonitor, PerformanceReport } from '../PerformanceMonitor.js';
...
describe('EnhancedPerformanceMonitor', () => {
  ...
  beforeEach(() => {
    ...
    vi.stubGlobal('setInterval', vi.fn((callback: Function, delay: number) => {
  ...
  describe('Performance Measurement', () => {
    ...
    it('should record metrics for failed operations', async () => {
      ...
      let offset = 0;
      vi.spyOn(performance, 'now').mockImplementation(() => baseTime + (offset += 10));
  ...
  describe('Performance Reporting', () => {
    ...
    it('should calculate percentiles correctly', async () => {
      ...
      vi.spyOn(performance, 'now').mockImplementation(() => {
        ...
        const duration = durationIndex < durations.length ? durations[durationIndex] : 0;
        const result = isStart ? baseTime + (callIndex * 1000) : baseTime + ((callIndex - 1) * 1000) + duration;
        callIndex++;
        return result;
      });
    ...
    it('should emit periodic reports', async () => {
      ...
      await monitor.measure('test-service', 'test-operation', async () => 'result');
  ...
  describe('Baseline Comparison', () => {
    it('should compare performance with baseline', () => {
      ...
      expect(comparison['test-service'].successRateImprovement.improvement).toBeCloseTo(3, 10); // 3% improvement
    });
  ...
  describe('Memory Monitoring', () => {
    it('should monitor memory pressure', async () => {
      ...
      vi.stubGlobal('setInterval', vi.fn((callback: Function, delay: number) => {
        if (delay === 5000) { // Memory monitoring interval
  ...
  describe('Garbage Collection Optimization', () => {
    ...
    it('should not suggest GC under high load', async () => {
      ...
      let offset = 0;
      vi.spyOn(performance, 'now').mockImplementation(() => baseTime + (offset += 1));
