...
interface CreatorRepositoryState {
  ...
  fetchAll: (options?: PaginationOptions) => Promise<Result<PaginatedResult<Creator>>>;
  fetchByIds: (ids: CreatorId[]) => Promise<Result<Creator[]>>;
  search: (query: string, filters?: CreatorFilters) => Promise<Result<Creator[]>>;
  fetchStale: (hoursThreshold?: number, limit?: number) => Promise<Result<Creator[]>>;
  fetchVerified: (options?: PaginationOptions) => Promise<Result<PaginatedResult<Creator>>>;
  fetchTop: (sortBy: CreatorSortOptions, limit?: number) => Promise<Result<Creator[]>>;
  ...
  devtools(
    (set, get) => ({
      ...
      delete: async (id: CreatorId) => {
        ...
        if (result.isSuccess) {
          set((state) => {
            ...
            return { creators, verifiedCreators, topCreators, isLoading: false };
          });
        } else {
          set({ isLoading: false, error: result.error || null });
        }
      ...
      getStaleCreators: (hoursThreshold: number = 24) => {
        return Array.from(get().creators.values()).filter(
          creator => creator.isStale(hoursThreshold)
        );
      },
    }),
    {
...
export const useCreator = (id: string) => {
  const creator = useCreatorRepositoryStore(state => state.getCreatorById(id));
...
export const useCreatorByChannelUrl = (url: string) => {
  const creator = useCreatorRepositoryStore(state => state.getCreatorByChannelUrl(url));
  const fetchByChannelUrl = useCreatorRepositoryStore(state => state.fetchByChannelUrl);
  const isLoading = useCreatorRepositoryStore(state => state.isLoading);
  const error = useCreatorRepositoryStore(state => state.error);
...
export const useVerifiedCreators = (options?: PaginationOptions) => {
  ...
  const fetchVerified = useCreatorRepositoryStore(state => state.fetchVerified);
  const isLoading = useCreatorRepositoryStore(state => state.isLoading);
  const error = useCreatorRepositoryStore(state => state.error);
