...
export interface PositionWarning {
  type: 'large_stretch' | 'difficult_shift' | 'string_crossing' | 'awkward_position';
  message: string;
...
export function useMidiConversion() {
  ...
  const convertMidi = useCallback(async (
    ...
    try {
      ...
      if (typeof exerciseIdOrMeasures === 'object' && 'measures' in exerciseIdOrMeasures) {
        // Stateless mode: Use /api/v1/midi/convert with measures
        logger.info('Converting MIDI to fretboard (stateless mode - Story 4.4)', {
      ...
      const error = err instanceof Error ? err : new Error('Failed to convert MIDI');
      setError(error);

      // Check if this is a validation error (expected user error) vs system error
      const isValidationError = error.message?.includes('outside bass guitar range') ||
      ...
      if (isValidationError) {
        logger.warn('MIDI conversion validation failed', {
          ...
          mode: typeof exerciseIdOrMeasures === 'object' ? 'stateless' : 'legacy',
        });
      } else {
