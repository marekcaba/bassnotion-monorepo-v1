...
export class ExerciseLoadingMetrics {
  ...
  startExerciseMetadataLoad() {
    ...
    logger.info('📊 [METRICS] Starting exercise metadata load');
  }
  ...
  completeExerciseMetadataLoad(success: boolean, error?: string) {
    ...
    this.metrics.exerciseMetadata.duration =
      endTime - this.metrics.exerciseMetadata.startTime;
    this.metrics.exerciseMetadata.success = success;
    this.metrics.exerciseMetadata.error = error;

    logger.info(
      `📊 [METRICS] Exercise metadata load ${success ? 'completed' : 'failed'} in ${this.metrics.exerciseMetadata.duration.toFixed(2)}ms`,
    );
    ...
    if (success && !this.metrics.userInteractionReady) {
      ...
      logger.info(
        `📊 [METRICS] UI ready for user interaction after ${this.metrics.userInteractionReady.toFixed(2)}ms from page load`,
      );
    }
  }
  ...
  startAudioEngineInit() {
    ...
    logger.info('📊 [METRICS] Starting audio engine initialization');
  }
  ...
  completeAudioEngineInit(success: boolean, error?: string) {
    ...
    this.metrics.audioEngine.duration =
      endTime - this.metrics.audioEngine.startTime;
    this.metrics.audioEngine.success = success;
    this.metrics.audioEngine.error = error;

    logger.info(
      `📊 [METRICS] Audio engine init ${success ? 'completed' : 'failed'} in ${this.metrics.audioEngine.duration.toFixed(2)}ms`,
    );
  }
  ...
  startAudioAssetsLoad() {
    ...
    logger.info('📊 [METRICS] Starting audio assets load');
  }

  // Complete audio assets loading
  completeAudioAssetsLoad(
    success: boolean,
    samplesLoaded: number,
    error?: string,
  ) {
    ...
    if (this.metrics.audioAssets.startTime) {
      this.metrics.audioAssets.duration =
        endTime - this.metrics.audioAssets.startTime;
    }
    ...
    logger.info(
      `📊 [METRICS] Audio assets load ${success ? 'completed' : 'failed'} in ${this.metrics.audioAssets.duration?.toFixed(2)}ms (${samplesLoaded} samples)`,
    );
    ...
    if (
      ...
      logger.info(
        `📊 [METRICS] Total load time: ${this.metrics.totalLoadTime.toFixed(2)}ms`,
      );
      this.printSummary();
    }
  }
  ...
  printSummary() {
    logger.info('📊 [METRICS] === LOADING PERFORMANCE SUMMARY ===');
    logger.info(
      `📊 User interaction ready: ${this.metrics.userInteractionReady?.toFixed(2)}ms`,
    );
    logger.info(
      `📊 Exercise metadata: ${this.metrics.exerciseMetadata.duration?.toFixed(2)}ms`,
    );
    logger.info(
      `📊 Audio engine init: ${this.metrics.audioEngine.duration?.toFixed(2)}ms`,
    );
    logger.info(
      `📊 Audio assets load: ${this.metrics.audioAssets.duration?.toFixed(2)}ms`,
    );
    logger.info(
    ...
    if (
      ...
      logger.info(`📊 Parallel loading saved: ${savings.toFixed(2)}ms`);
    }
