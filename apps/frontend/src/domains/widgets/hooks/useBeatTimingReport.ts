...
export function useBeatTimingReport(source: string) {
  const reportBeat = useCallback((beatNumber: number, measureNumber: number, actualTime?: number) => {
    beatTimingAnalyzer.recordBeat(source, beatNumber, measureNumber, actualTime);
  }, [source]);
