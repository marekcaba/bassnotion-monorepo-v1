 ...
 */
export function useWamDrummer(options: UseWamDrummerOptions): UseWamDrummerReturn {
  ...
  const debug = useCallback((message: string, data?: any) => {
    if (debugMode) {
      console.log(`🥁 useWamDrummer[${widgetId}]: ${message}`, data);
    }
  }, [widgetId, debugMode]);
    ...
    const initializeWamDrummer = async () => {
      try {
        ...
        const [
          ...
          import('@/domains/playback/services/plugins/wam/WamDrummer'),
        ...
        // Get services
        audioEngineRef.current = serviceRegistry.get<AudioEngine>('audioEngine');
        ...
        console.error('Failed to initialize WAM Drummer:', err);
   ...
   */
  const loadSample = useCallback(async (padNumber: number, url: string) => {
    if (!drummerNodeRef.current) {
   ...
   */
  const loadKit = useCallback(async (kitPath: string) => {
    if (!drummerNodeRef.current) {
      throw new Error('WAM Drummer not initialized');
    }
   ...
   */
  const triggerPad = useCallback((padNumber: number, velocity: number = 0.8) => {
   ...
   */
  const schedulePattern = useCallback((pattern: DrumPattern) => {
    if (!drummerNodeRef.current || !transportRef.current || !adapterRef.current) return;
