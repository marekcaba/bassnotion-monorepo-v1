...
export function useTransportSync({
   ...
   */
  const getCompensatedTime = useCallback((serverTime: number): number => {
  ...
  const handlePositionUpdate = useCallback((data: any) => {
    ...
    if (enableLatencyCompensation && syncState.isPlaying) {
      ...
      const ticks = Tone.Time(data.position).toTicks() + 
    ...
    performanceMetrics.current.avgProcessingTime = 
      (performanceMetrics.current.avgProcessingTime * (performanceMetrics.current.totalUpdates - 1) + 
       processingTime) / performanceMetrics.current.totalUpdates;
  }, [syncState.isPlaying, syncState.latency, enableLatencyCompensation, onPositionUpdate]);
   ...
   * Handle stop event
   */
  const handleStop = useCallback((data: any) => {
    setSyncState(prev => ({ 
      ...prev, 
      isPlaying: false,
      position: '0:0:0'
   ...
   * Handle tempo change
   */
  const handleTempoChange = useCallback((data: any) => {
   ...
   */
  const handleBatchUpdate = useCallback((batch: any[]) => {
