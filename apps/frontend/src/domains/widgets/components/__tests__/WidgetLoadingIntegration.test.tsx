...
vi.mock('@/domains/playback/services/InitialSamplePreloader', () => ({
...
describe('Widget Loading Integration Tests', () => {
  ...
  describe('Cache Hit Scenarios', () => {
    ...
    it('HarmonyWidget should use cached harmony instrument', async () => {
      ...
      expect(mockHarmonyInstrument.audioNode.setParameterValues).toHaveBeenCalledWith({
        volume: 0.8
      });
  ...
  describe('Fallback Scenarios', () => {
    it('should fall back to creating new instruments when cache is empty', async () => {
      ...
      const MockWidget = () => {
        React.useEffect(() => {
          const load = async () => {
            const cached = GlobalSampleCache.getCachedInstrument('test-instrument');
            if (!cached) {
    ...
    it('should handle AudioContext not ready gracefully', async () => {
      ...
      (global as any).window.__globalCoreServices.getAudioEngine = vi.fn().mockReturnValue({
        isReady: vi.fn().mockReturnValue(true),
  ...
  describe('Performance Scenarios', () => {
    it('should not create duplicate instruments', async () => {
      ...
      const promises = Array(5).fill(null).map(() => 
        mockSingleton.getOrCreateKeyboardPlugin({} as any)
      );
