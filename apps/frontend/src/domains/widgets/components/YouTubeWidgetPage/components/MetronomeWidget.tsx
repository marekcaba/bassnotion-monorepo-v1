  ...
  return (
    ...
    >
      {(syncProps: SyncedWidgetRenderProps) => {
        ...
        const [polyrhythmRatio, setPolyrhythmRatio] = useState({ main: 4, poly: 3 });
        ...
        useEffect(() => {
          setMetronomeDots(Array.from({ length: beats }, (_, i) => ({
            id: i + 1,
            isActive: true,
            isCurrent: i === 0,
          })));
                              ...
                              <div
                                ...
                                className={`
                                  ...
                                  ${
                                    mutedBeats.has(i)
                                      ? 'bg-slate-600'
                                      : metronomeDots[i]?.isCurrent && isEnginePlay
                        ...
                        <div className="flex items-center justify-between flex-1">
                          ...
                          <div className="flex items-center gap-1">
                            ...
                            <div className="flex items-center">
                              <span className="text-base font-mono text-emerald-400 w-4 text-center">{beats}</span>
                              <span className="text-base text-emerald-400/50 mx-0.5">/</span>
                              <span className="text-base font-mono text-emerald-400 w-5 text-center">{noteValue}</span>
                            </div>
                            <div className="flex flex-col gap-0.5">
                              <button
                                onClick={() => setNoteValue(noteValue === 16 ? 4 : noteValue === 8 ? 16 : 8)}
                                className="w-4 h-4 rounded bg-slate-800 shadow-[2px_2px_4px_rgba(0,0,0,0.5),-2px_-2px_4px_rgba(255,255,255,0.1)] hover:shadow-[inset_1px_1px_2px_rgba(0,0,0,0.5),inset_-1px_-1px_2px_rgba(255,255,255,0.1)] transition-all duration-200 text-emerald-400 text-xs flex items-center justify-center"
                              >
                                +
                              </button>
                              <button
                                onClick={() => setNoteValue(noteValue === 4 ? 16 : noteValue === 8 ? 4 : 8)}
                                className="w-4 h-4 rounded bg-slate-800 shadow-[2px_2px_4px_rgba(0,0,0,0.5),-2px_-2px_4px_rgba(255,255,255,0.1)] hover:shadow-[inset_1px_1px_2px_rgba(0,0,0,0.5),inset_-1px_-1px_2px_rgba(255,255,255,0.1)] transition-all duration-200 text-emerald-400 text-xs flex items-center justify-center"
                              >
                                -
                              </button>
                          ...
                          <div className="flex items-center gap-2">
                            <button
                              onClick={() => setPolyrhythmEnabled(!polyrhythmEnabled)}
                            ...
                            >
                              {/* Outer circle dots */}
                              {Array.from({ length: polyrhythmRatio.main }, (_, i) => {
                                const angle = (i / polyrhythmRatio.main) * 360 - 90;
                                const x = 18 * Math.cos(angle * Math.PI / 180) + 24;
                                const y = 18 * Math.sin(angle * Math.PI / 180) + 24;
                                return (
                                  <div
                                    key={`outer-${i}`}
                                    className={`absolute w-2 h-2 rounded-full transition-all duration-200 ${
                                      polyrhythmEnabled ? 'bg-emerald-400' : 'bg-slate-600'
                                    }`}
                              ...
                              {/* Inner circle dots */}
                              {polyrhythmEnabled && Array.from({ length: polyrhythmRatio.poly }, (_, i) => {
                                const angle = (i / polyrhythmRatio.poly) * 360 - 90;
                                const x = 10 * Math.cos(angle * Math.PI / 180) + 24;
                                const y = 10 * Math.sin(angle * Math.PI / 180) + 24;
                                return (
                                  <div
                                    key={`inner-${i}`}
                            ...
                            {polyrhythmEnabled && (
                              <div className="flex flex-col gap-1">
                                <button
                                  onClick={() => setPolyrhythmRatio(prev => ({ 
                                    ...prev, 
