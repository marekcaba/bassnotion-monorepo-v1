...
export function HarmonyWidget({
  ...
  const [currentInstrument, setCurrentInstrument] =
    useState<KeyboardInstrumentType>(KeyboardInstrument.SALAMANDER_PIANO);
  ...
  const createAudioNodeAttempt = useCallback(async () => {
      ...
      if (globalServices && globalServices.getAudioEngine) {
        ...
        if (audioEngine && audioEngine.getContext) {
          try {
            ...
            logger.info(
              '🎹 HarmonyWidget: AudioEngine not ready yet, will retry...',
            );
    ...
    if (context && context instanceof AudioContext) {
      ...
      if (context.state === 'suspended') {
        logger.info(
          '🎹 HarmonyWidget: AudioContext is suspended, waiting for user gesture...',
        );
      ...
      try {
        ...
        if (audioNode.hasInstrumentLoaded && !audioNode.hasInstrumentLoaded()) {
          ...
          audioNode.hasInstrumentLoaded &&
          audioNode.hasInstrumentLoaded()
        ) {
          logger.info(
            '🎹 HarmonyWidget: Instrument already loaded, skipping sample loading',
          );
        } else {
          // Fallback if hasInstrumentLoaded method doesn't exist
          logger.info(
            '🎹 HarmonyWidget: Loading instrument samples (no check method)...',
          );
          await audioNode.loadInstrument(currentInstrument);
          logger.info('🎹 HarmonyWidget: Instrument samples loaded!');
        }
