...
export function LoopGridStrip({
  ...
  const [dragType, setDragType] = useState<'select' | 'resize-start' | 'resize-end' | 'move-region' | null>(null);
  ...
  const areRegionsEqual = (a: LoopRegion | null, b: LoopRegion | null): boolean => {
    if (!a || !b) return a === b;
  ...
  const measures = useMemo((): MeasureData[] => {
    ...
    const timeSignature: TimeSignature = exercise.timeSignature || { numerator: 4, denominator: 4 };
    ...
    const totalMeasures = Math.max(4, Math.ceil(durationInSeconds / measureDurationInSeconds));
  ...
  const getMeasureAndBeatFromPosition = useCallback((clientX: number): { measure: number; beat: number } => {
  ...
  const handleMeasureClick = useCallback((measureIndex: number, event: React.MouseEvent) => {
    event.preventDefault();
    
    // Check if clicking on already selected measure(s) - clear if so
    if (loopRegion && measureIndex >= loopRegion.startMeasure && measureIndex <= loopRegion.endMeasure) {
      // Clear selection
  ...
  // Handle resize start
  const handleResizeStart = useCallback((type: 'resize-start' | 'resize-end', event: React.MouseEvent) => {
    event.preventDefault();
  ...
  const handleRegionInteraction = useCallback((event: React.MouseEvent) => {
    ...
    const regionStartBeat = (loopRegion.startMeasure - 1) * beatsPerMeasure + (loopRegion.startBeat || 1) - 1;
  ...
  const handleResizeDrag = useCallback((clientX: number) => {
    if (!loopRegion) return;
    ...
    if (dragType?.includes('resize')) {
      ...
      if (dragType === 'resize-start') {
        ...
        const endPosition = (loopRegion.endMeasure - 1) * beatsPerMeasure + (loopRegion.endBeat || beatsPerMeasure);
        ...
        const startPosition = (loopRegion.startMeasure - 1) * beatsPerMeasure + (loopRegion.startBeat || 1);
      ...
      const regionLength = (loopRegion.endMeasure - loopRegion.startMeasure) * beatsPerMeasure + 
                           (loopRegion.endBeat || beatsPerMeasure) - (loopRegion.startBeat || 1) + 1;
      
      // Ensure region stays within bounds
      const clampedStartBeat = Math.max(0, Math.min(totalBeats - regionLength, newStartBeatPosition));
      const clampedEndBeat = clampedStartBeat + regionLength - 1;
      
      // Convert back to measure.beat format
      const newStartMeasure = Math.floor(clampedStartBeat / beatsPerMeasure) + 1;
  ...
  }, [loopRegion, dragType, getMeasureAndBeatFromPosition, beatsPerMeasure, dragOffset, measures.length, onLoopRegionChange, areRegionsEqual]);

  // Handle selection drag start
  const handleSelectionDragStart = useCallback((measureIndex: number, event: React.MouseEvent) => {
    event.preventDefault();
    setIsDragging(true);
  ...
  // Handle drag over
  const handleDragOver = useCallback((measureIndex: number) => {
  ...
  const isMeasureSelected = useCallback((measureIndex: number): boolean => {
    if (!loopRegion) return false;
    
    // Calculate the start and end positions in beats
    const selectionStartBeat = (loopRegion.startMeasure - 1) * beatsPerMeasure + (loopRegion.startBeat || 1);
    const selectionEndBeat = (loopRegion.endMeasure - 1) * beatsPerMeasure + (loopRegion.endBeat || beatsPerMeasure);
    ...
    return !(measureEndBeat < selectionStartBeat || measureStartBeat > selectionEndBeat);
  ...
  React.useEffect(() => {
    ...
    const handleTouchMove = (e: TouchEvent) => {
      e.preventDefault();
      if ((dragType?.includes('resize') || dragType === 'move-region') && e.touches[0]) {
              ...
              >
                ...
                <div className="absolute bottom-1 left-0 right-0 flex justify-evenly">
                  {Array.from({ length: beatsPerMeasure }, (_, i) => {
                    ...
                    const currentBeatPosition = (measure.index - 1) * beatsPerMeasure + beatNumber;
                    const selectionStartBeat = loopRegion ? (loopRegion.startMeasure - 1) * beatsPerMeasure + (loopRegion.startBeat || 1) : 0;
                    const selectionEndBeat = loopRegion ? (loopRegion.endMeasure - 1) * beatsPerMeasure + (loopRegion.endBeat || beatsPerMeasure) : 0;
                    const isBeatSelected = loopRegion && currentBeatPosition >= selectionStartBeat && currentBeatPosition <= selectionEndBeat;
                    
                    // Calculate if this beat has been played (yellow trail)
                    const totalBeats = measures.length * beatsPerMeasure;
                    const playbackBeatPosition = Math.floor((playbackPosition / 100) * totalBeats) + 1;
                    const hasBeenPlayed = currentBeatPosition <= playbackBeatPosition;
            ...
            <>
              ...
              <div
                ...
                style={(() => {
                  ...
                  const startBeatPosition = (loopRegion.startMeasure - 1) * beatsPerMeasure + (loopRegion.startBeat || 1) - 1;
                  const endBeatPosition = (loopRegion.endMeasure - 1) * beatsPerMeasure + (loopRegion.endBeat || beatsPerMeasure) - 1;
                  const totalBeats = measures.length * beatsPerMeasure;
                  
                  // Position overlay exactly where handles are
                  const leftPercent = (startBeatPosition / totalBeats) * 100;
                  const rightPercent = ((endBeatPosition + 1) / totalBeats) * 100;
              ...
              <div
                ...
                style={(() => {
                  ...
                  const endBeatPosition = (loopRegion.endMeasure - 1) * beatsPerMeasure + (loopRegion.endBeat || beatsPerMeasure);
                  const totalBeats = measures.length * beatsPerMeasure;
