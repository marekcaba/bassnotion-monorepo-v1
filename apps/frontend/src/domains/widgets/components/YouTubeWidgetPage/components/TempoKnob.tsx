...
export function TempoKnob({
  ...
  const handleMove = (clientX: number) => {
    ...
    const newValue = Math.max(
      min,
      Math.min(max, Math.round(startValueRef.current + valueChange)),
    );
    ...
    >
      ...
      <div className="relative" style={{ width: size, height: size }}>
        ...
        >
          ...
          <div className="absolute top-1/2 left-1/4 -translate-x-1/2 -translate-y-1/2 text-xs text-slate-500 font-bold pointer-events-none">
            −
          </div>
          <div className="absolute top-1/2 right-1/4 translate-x-1/2 -translate-y-1/2 text-xs text-slate-500 font-bold pointer-events-none">
            +
          </div>
        </div>
          ...
          >
            ...
            <g
              style={{
                ...
                transformOrigin: `${(size + 16) / 2}px ${(size + 16) / 2}px`,
              }}
            >
              {/* Background track */}
              <circle
      ...
      <div className="flex flex-col items-center mt-3">
        <div className="text-lg font-mono-display font-medium text-slate-200 tabular-nums">
          {value}
        </div>
