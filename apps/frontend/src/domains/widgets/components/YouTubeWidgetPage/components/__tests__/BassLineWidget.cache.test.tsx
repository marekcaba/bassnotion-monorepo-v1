...
vi.mock('@/infrastructure/supabase/client', () => ({
  supabase: {
    storage: {
      from: () => ({
        getPublicUrl: () => ({
          data: { publicUrl: 'https://test.url/bass.mp3' },
        }),
      }),
    },
  },
...
describe('BassLineWidget - Cache Integration Tests', () => {
  ...
  describe('Bass Sample Playback', () => {
    it('should play bass notes with cached instrument', async () => {
      ...
      expect(
        mockBassInstrument.audioNode.setParameterValues,
      ).toHaveBeenCalledWith({
        technique: 'slap',
      });
      expect(
        mockBassInstrument.audioNode.scheduleEvents,
      ).toHaveBeenLastCalledWith([{ note: 36, time: 0, velocity: 0.9 }]);
    });
    ...
    it('should handle different bass playing techniques', async () => {
      ...
      const MockBassWidget = () => {
        ...
        return (
          <div>
            <button onClick={() => playTechnique('fingerstyle', 33)}>
              Finger
            </button>
  ...
  describe('Bass Effects and Parameters', () => {
    it('should apply tone and effects settings', async () => {
      ...
      const MockBassWidget = () => {
        ...
        return (
          <div>
            <button onClick={() => applySettings({ tone: 0.2 })}>
              Dark Tone
            </button>
            <button onClick={() => applySettings({ tone: 0.8 })}>
              Bright Tone
            </button>
            <button onClick={() => applySettings({ drive: 0.6 })}>
              Add Drive
            </button>
            <button onClick={() => applySettings({ compression: 0.7 })}>
              Compress
            </button>
          </div>
        );
  ...
  describe('Fallback Loading', () => {
    it('should fall back to creating new instrument when cache miss', async () => {
      ...
      const mockCreateBassInstrument = vi
        .fn()
        .mockResolvedValue(mockBassInstrument);
