...
type BassArticulationType = typeof BassArticulation[keyof typeof BassArticulation];
...
export function BassLineWidgetV2({
  pattern,
  isVisible,
  isPlaying,
  exercise,
  ...
  const [currentArticulation, setCurrentArticulation] = useState<BassArticulationType>(BassArticulation.FINGERSTYLE);
  ...
  useEffect(() => {
    ...
    const loadPluginClass = async () => {
      console.log('🎸 BassLineWidgetV2: Loading plugin class...');
      
      try {
        // Dynamic import to avoid SSR issues
        const { default: WamBass } = await import('@/domains/playback/services/plugins/wam/WamBass');
        wamPluginClassRef.current = WamBass;
        setPluginClassLoaded(true);
        console.log('✅ WAM Bass plugin class loaded successfully');
      } catch (error) {
        console.error('❌ Failed to load WAM Bass plugin class:', error);
      }
    };
  ...
  useEffect(() => {
    ...
    if (bassPluginRef.current) {
      console.log('🎸 BassLineWidgetV2: Plugin already loaded, skipping creation');
      return;
    }
    
    const createAudioNode = async () => {
      console.log('🎸 BassLineWidgetV2: Attempting to create audio node...', {
      ...
      try {
        ...
        if (!WamBass) {
          console.error('🎸 BassLineWidgetV2: Plugin class not loaded');
          return;
        }
        ...
        if (globalServices && globalServices.getAudioEngine) {
          ...
          if (audioEngine && audioEngine.getContext) {
            try {
              ...
              console.log('🎸 BassLineWidgetV2: AudioEngine not ready yet, will retry...');
        ...
        if (context && context instanceof AudioContext) {
          ...
          if (context.state === 'suspended') {
            console.log('🎸 BassLineWidgetV2: AudioContext is suspended, waiting for user gesture...');
          ...
          console.log('🎸 BassLineWidgetV2: Created plugin instance:', plugin);
          ...
          console.log('✅ WAM Bass plugin loaded and connected (waiting for samples)');
        } else {
          console.log('🎸 BassLineWidgetV2: AudioContext not ready yet', {
            hasContext: !!context,
            contextState: context?.state
          });
        }
      } catch (error) {
        console.error('❌ Failed to create WAM Bass audio node:', error);
      }
    };
  ...
  useEffect(() => {
    ...
    const handleAudioReady = () => {
      console.log('🎸 BassLineWidgetV2: Audio services ready event received');
      // Do nothing - the effect will handle retries
    };
    
    const handleAudioContextStarted = () => {
      console.log('🎸 BassLineWidgetV2: AudioContext started event received');
      // Do nothing - the effect will handle retries
    };
  ...
  const handleArticulationChange = useCallback(async (articulation: BassArticulationType) => {
    setCurrentArticulation(articulation);
    if (bassPluginRef.current) {
      const articulationIndex = Object.values(BassArticulation).indexOf(articulation);
  ...
  const patternNotes = useMemo(() => {
    ...
    return bassPatterns[pattern as keyof typeof bassPatterns] || bassPatterns['Root-Fifth'];
  }, [pattern, exercise]);
                    ...
                    {[4, 3, 2, 1].map(string => (
                      <div key={`string-${string}`} className="flex gap-px">
                        {Array.from({ length: 8 }, (_, fretIndex) => {
                          ...
                          const isFretMarker = [3, 5, 7, 9, 12, 15, 17, 19].includes(fret);
