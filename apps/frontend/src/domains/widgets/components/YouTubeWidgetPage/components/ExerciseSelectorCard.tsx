  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{
          background:
            'radial-gradient(ellipse at center, rgba(139, 92, 246, 0.25) 0%, rgba(139, 92, 246, 0.08) 40%, transparent 70%)',
          filter: 'blur(12px)',
          transform: 'scale(1.5)',
        }}
      />
...
export const ExerciseSelectorCard = React.memo(function ExerciseSelectorCard({
  ...
  const getExerciseDetails = useCallback((exercise: any) => {
    const difficulty =
      safeString(exercise.difficulty).toLowerCase() || 'beginner';
  ...
  useEffect(() => {
    ...
    const loadFullSamplesWhenVisible = async (
      ...
      if (entry.isIntersecting && !hasTriggeredFullLoadRef.current) {
        ...
        logger.info(
          'ExerciseList visible - loading samples for ALL exercises',
          {
    ...
    <ZoneCard className="zone-card overflow-visible border-0 shadow-none bg-transparent">
      <ZoneCardContent className="p-0 overflow-visible">
        ...
        >
          ...
          {headerContent && <div className="relative">{headerContent}</div>}
            ...
            <div className="relative p-4">
              ...
              {!exerciseListLoading &&
                ...
                (() => {
                  ...
                  const totalCompleted = unlockedExercises.reduce(
                    (sum, { exercise }) => {
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="pb-2">
                        <div className="flex items-stretch justify-evenly gap-0">
                          {allExerciseItems.map(
                            (
                              { exercise, index, isLocked, isRewardCard },
                              i,
                            ) => {
                              ...
                              const completedCount =
                                practiceCompletions[id]?.count || 0;
                              const isCompleted =
                                completedCount >= REQUIRED_COMPLETIONS;
                              const isLastItem =
                                i === allExerciseItems.length - 1;
                              // Show progress bar connector between unlocked and locked sections
                              const isProgressBoundary =
                                i === unlockedExercises.length - 1 &&
                                lockedExercises.length > 0;
                              // Reward card: the last locked-difficulty exercise (always gets animated border)
                              const isRewardReady =
                                isRewardCard && advancedUnlocked;
                              ...
                              const cardButton = (
                                <button
                                  ...
                                  title={
                                    isRewardCard
                                      ? isRewardReady
                                        ? 'Click to unlock the groove'
                                        : 'Complete all exercises to unlock'
                                      : details.title
                                  }
                                >
                                  {/* Selected glow */}
                                  ...
                                  <div className="relative flex items-center justify-center w-8 h-8">
                                    {isRewardCard && isRewardReady ? (
                                      ...
                                      <div
                                        className={`w-3 h-3 rounded-full bg-violet-400 shadow-[0_0_8px_rgba(139,92,246,0.5)]`}
                                      />
                                    ) : (
                                  ...
                                  {isRewardCard ? (
                                    ...
                                    <span
                                      className={`text-[9px] font-semibold px-2.5 py-1 rounded-md border transition-all duration-300 ${
                                        isRewardReady
                                    ...
                                    <>
                                      ...
                                      <div
                                        className={`w-6 h-0.5 rounded-full ${isLocked ? 'bg-slate-600/30' : details.colors.dot} mt-0.5`}
                                      />
                                    </>
                                  )}
                                </button>
                        ...
                        return (
                          <div>
                            ...
                            <div className="flex items-start justify-between gap-3 mb-3">
                              ...
                              <span
                                className={`px-2 py-0.5 rounded-md text-xs font-semibold ${details.colors.text} bg-white/5 border border-current/20`}
                              >
                                {details.difficulty}
                              </span>
                            </div>
                            ...
                            <p className="text-slate-400/90 text-sm leading-relaxed mb-4 line-clamp-3 h-[4.5rem]">
                              {details.description}
                            </p>
                  ...
                  <div className="flex flex-col items-center gap-1.5 px-3 py-3 rounded-xl bg-white/5 border border-white/10 animate-pulse min-w-[80px]">
                    ...
                    <div className="flex gap-1">
                      {[1, 2, 3, 4].map((d) => (
                        <div
                          key={d}
                          className="skeleton-shimmer w-1.5 h-1.5 rounded-full"
                        />
                      ))}
                    </div>
