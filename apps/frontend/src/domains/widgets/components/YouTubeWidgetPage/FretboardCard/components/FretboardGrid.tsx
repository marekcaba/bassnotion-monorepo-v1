...
export const FretboardGrid: React.FC<FretboardGridProps> = React.memo(
  ({
    ...
    const handleMeasureChange = useCallback((newMeasure: number) => {
      setMeasureChangeCounter((prev) => {
        ...
        if (window.__DEBUG_MEASURE_SYNC__ === true) {
          ...
          console.log(
            `🔄 [GRID] Measure change trigger: measure=${newMeasure} counter=${prev + 1}`,
          );
        }
        return prev + 1;
      });
    ...
    const { registerNoteRef, timeline, getCurrentMeasure } =
      ...
      const checkLineDOM = (label: string) => {
        ...
        allLines.forEach((line) => {
          ...
          const targetInWindow =
            parseInt(tgtMeasure || '-1', 10) >= currentMeasureFromNote &&
            parseInt(tgtMeasure || '-1', 10) <= currentMeasureFromNote + 1;
          if (targetInWindow || hasPlayed) {
            lineStates.push(
              `src=${srcNote} m${srcMeasure}→${tgtMeasure} played=${hasPlayed} opacity=${computedOpacity}`,
            );
          }
        });
    ...
    useEffect(() => {
      ...
      if (getMeasureHighlight && exerciseNotes.length > 0) {
        ...
        exerciseNotes.forEach((note) => {
          ...
          const highlight = getMeasureHighlight(
            stringIndex,
            fret,
          ...
          const expectedState: 'current' | 'next' | 'other' =
            noteMeasure === currentMeasureFromNote
              ? 'current'
              : noteMeasure === currentMeasureFromNote + 1
                ? 'next'
                : 'other';
      ...
      if (prevState) {
        ...
        if (measureChanged || noteChanged || highlightsChanged) {
          ...
          if (highlightsChanged) {
            ...
            if (removed.length > 0) {
              console.log(
                `🎨 [RENDER-STATE]   - REMOVED: ${removed.join(', ')}`,
              );
            }
          }
          ...
          if (measureChanged) {
            console.log(
              `🔍 [RENDER-STATE] Full position analysis for measure ${currentState.measure}:`,
            );
            const mismatches = allPositionStates.filter(
              (ps) =>
                ps.highlight.shouldHighlight &&
                ps.highlight.state !== ps.expectedState,
            );
            if (mismatches.length > 0) {
              ...
              console.log(
                `✅ [RENDER-STATE] No mismatches - all states correct`,
              );
            }
          }
        }
    ...
    const memoizedConnectionLines = useMemo(() => {
      ...
      const getConnectionPriority = (conn: {
        measure1: number;
        measure2: number;
      }): number => {
      ...
      connections.forEach(
        ({ pos1, pos2, measure1, measure2, sourceNoteIndex }) => {
      ...
      if (
        ...
        window.__DEBUG_LINE_MEMO__ === true
      ) {
        // eslint-disable-next-line no-console
        console.log(
          `📐 [LINE-MEMO] Recalculated! measure=${currentMeasureFromNote} | ` +
            `lines=${lineDataArray.length} | ` +
            `positions: ${lineDataArray
              .slice(0, 3)
              .map((l) => `(${l.x1.toFixed(0)},${l.y1.toFixed(0)})`)
              .join(', ')}${lineDataArray.length > 3 ? '...' : ''}`,
        );
      }
    ...
    const isDotBeingDragged = (stringIndex: number, fret: Fret) => {
      return (
        draggedDot?.stringIndex === stringIndex && draggedDot?.fret === fret
      );
    };
    ...
    const renderDot = (
      ...
      const positionIndex =
        visualStringIndex !== undefined
          ? visualStringIndex
          : logicalStringIndex; // Use visual index for positioning
      ...
      const isOrangeMeasure =
        dotMeasure0Based >= 0 && dotMeasure0Based % 2 === 1;
      ...
      const dotElement = (
        ...
        >
          ...
          {isSelected && orderNumbers.length > 0 && orderNumbers.includes(1) ? (
            ...
            <span
              className={`text-xs font-semibold select-none ${shouldShowAsHighlighted || isExerciseNoteAtPos ? 'text-black' : 'text-white'}`}
            >
              {stringName}
            </span>
          ) : fret === 12 ? (
    ...
    return (
      <div className="relative">
        ...
        <div id="fretboard-instructions" className="sr-only">
          ...
          navigate between positions, Enter or Space to select. Selected
          positions can be dragged to reorder them.
                ...
                <defs>
                  <mask id="dot-cutout-mask">
                    ...
                    {Array.from(selectedDots.keys()).map((posKey) => {
                      ...
                      const fret =
                        fretStr === 'open' ? 'open' : parseInt(fretStr, 10);
                      ...
                      const dotCenterX =
                        fret === 'open'
                          ...
                          : CENTER_OFFSET +
                            ...
                            DOT_RADIUS;
                      const dotCenterY =
                        absoluteVisualPosition * STRING_SPACING + DOT_RADIUS;
                  ...
                  {memoizedConnectionLines.map((lineData) => {
                    ...
                    return (
                      <line
                        ...
                        data-is-transition={
                          lineData.isTransitionLine ? 'true' : 'false'
                        }
