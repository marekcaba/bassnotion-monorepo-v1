    ...
    <div className="p-6 space-y-6">
      ...
      <div className="bg-gray-100 p-4 rounded-lg">
        ...
        <div className="grid grid-cols-2 gap-4 text-sm">
          <div>
            ...
            <span className={drumTiming.drift < 1 ? 'text-green-600' : 'text-red-600'}>
              {drumTiming.drift.toFixed(2)}ms
            </span>
          </div>
          <div>
            <span className="font-medium">Sync Health: </span>
            <span className={!drumTiming.isIsolated ? 'text-green-600' : 'text-red-600'}>
      ...
      <div className="bg-gray-100 p-4 rounded-lg">
        ...
        <div className="space-y-3">
          ...
          {/* Send to Reverb */}
          <div>
            <label className="block text-sm font-medium mb-1">Reverb Send</label>
            <input
              type="range"
              min="0"
              max="1"
              ...
              onChange={(e) => drumMixing.setSendLevel('reverb', parseFloat(e.target.value))}
              className="w-full"
            />
          </div>
        </div>
      ...
      <div className="border-2 border-gray-300 rounded-lg p-4">
        <DrummerWidget
          ...
          isActive={true}
          tempo={120}
          // The widget automatically integrates with the track system
          // through the WidgetTrackAdapter!
        />
      </div>
      ...
      <div className="bg-blue-50 p-4 rounded-lg">
        ...
        <p className="text-sm mb-4">
          You can now have multiple drum tracks playing simultaneously with perfect sync!
        </p>
        <button
...
async function createAdditionalDrumTrack() {
  ...
  const compressor = new DynamicsCompressorNode(services.audioEngine.getContext(), {
    threshold: -12,
    knee: 2,
    ratio: 4,
    attack: 0.003,
  ...
  console.log('✅ Added percussion track with bus routing!');
}