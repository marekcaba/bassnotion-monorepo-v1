...
export function diagnoseAudioFlow() {
  ...
  if (Tone && Tone.context) {
    ...
    console.log('   - Tone context same as persistent:', toneCtx === persistentContext);
    console.log('   - Tone context state:', Tone.context.state);
    console.log('   - Tone Destination volume:', Tone.Destination?.volume?.value);
  ...
  const GlobalSampleCache = (window as any).GlobalSampleCache || 
                           require('@/domains/playback/services/storage/GlobalSampleCache').GlobalSampleCache;
  if (GlobalSampleCache) {
    ...
    if (preloaded && preloaded.audioNode) {
      ...
      console.log('   - Pre-loaded connected:', preloaded.audioNode.isConnected);
    }
  }
  ...
  if (!harmonyPlugin && GlobalSampleCache) {
    harmonyPlugin = GlobalSampleCache.getCachedInstrument('wam-keyboard-singleton') ||
  ...
  if (harmonyPlugin && harmonyPlugin.audioNode) {
    ...
    if (audioNode.activeSampler) {
      ...
      console.log('   - Has getStatus:', typeof sampler.getStatus === 'function');
      ...
      console.log('   - Connected to gain:', sampler.destination === audioNode.gainNode);
    }
  } else {
  ...
  if (harmonyPlugin && harmonyPlugin.audioNode && !harmonyPlugin.audioNode.isConnected) {
