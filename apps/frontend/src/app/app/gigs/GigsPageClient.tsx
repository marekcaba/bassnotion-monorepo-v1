...
function GigsContent({ initialGigs }: { initialGigs: Gig[] | null }) {
  const { isAuthenticated } = useAuth();
  // Seed from the SSR read (full-SSR): the list is on the page on first paint — no "Loading…"
  // flash. null = server couldn't prefetch it → resolve live below, exactly as before.
  const [gigs, setGigs] = React.useState<Gig[] | null>(initialGigs);
  const [error, setError] = React.useState(false);

  React.useEffect(() => {
    if (!isAuthenticated) return;
    // Already have the server-seeded list → don't refetch on mount (it's fresh from this request).
    if (initialGigs !== null) return;
    let cancelled = false;
    fetchMyGigs()
      .then((list) => {
...
/**
 * The gigs CLIENT tree. Split out of page.tsx (now a server component) so the server can prefetch
 * the gig list and pass initialGigs in — see ./page.tsx.
 */
export function GigsPageClient({ initialGigs }: { initialGigs: Gig[] | null }) {
  return (
    <>
      <PageErrorBoundary pageName="Gigs">
        <GigsContent initialGigs={initialGigs} />
      </PageErrorBoundary>
    </>
  );
}
