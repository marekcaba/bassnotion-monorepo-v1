...
function GymPageInner({
  ...
  /** SSR seed (P3): the server read today-rep-STATUS (read-only, no mint) so the gym renders the
   *  completed-vs-fresh screen on first paint without a flip. null = server couldn't determine it
   *  → resolve live as before. */
  initialDoneTodayUtc: boolean | null;
}) {
  ...
  const {
    ...
    chooseDoor,
    refresh,
  } = useGymSession(undefined, { enabled: isMember, initialDoneTodayUtc });
...
/**
 * The gym's CLIENT tree. Split out of page.tsx (now a server component) so the server can prefetch
 * today-rep-status and pass initialDoneTodayUtc in — see ./GymServerPage / page.tsx.
 */
export function GymPageClient({
  ...
  return (
    <Suspense fallback={<GymLoading />}>
      <GymPageInner initialDoneTodayUtc={initialDoneTodayUtc} />
    </Suspense>
  );
}
