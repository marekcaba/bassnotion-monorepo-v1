...
export default function WurlitzerAdminPage() {
  ...
  const updateVelocityRange = (layerIndex: number, field: 'min' | 'max', value: number) => {
  ...
  const saveConfiguration = async () => {
    ...
    setHasChanges(false);
    alert('Configuration downloaded! Replace the perNoteVelocityRanges in wurlitzer-piano.json');
  };
  ...
  return (
    <>
      <div className="container mx-auto p-6 max-w-7xl">
        <div className="mb-6">
          <h1 className="text-3xl font-bold mb-2">Wurlitzer Velocity Configuration</h1>
          <p className="text-gray-600">
            Fine-tune velocity thresholds for each note. Each note can have 3-5 velocity layers.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <Card className="lg:col-span-1">
            <CardHeader>
              ...
              <CardDescription>Choose a note to edit its velocity ranges</CardDescription>
            </CardHeader>
            <CardContent>
                  ...
                  .map(([octave, notes]) => (
                    <div key={octave}>
                      ...
                      <div className="grid grid-cols-4 gap-2">
                        {notes.map(note => {
                          const noteConfig = config.find(c => c.note === note);
          ...
          <Card className="lg:col-span-2">
            <CardHeader>
              <CardTitle>
                {selectedNote ? `Velocity Ranges for ${selectedNote}` : 'Select a note'}
              </CardTitle>
              <CardDescription>
                ...
                <div className="space-y-6">
                  ...
                  <div className="space-y-4">
                    {selectedNoteConfig.ranges.map((range, idx) => (
                      ...
                      >
                        <div className="font-semibold mb-3 text-lg">{range.layer.toUpperCase()}</div>
                        <div className="grid grid-cols-2 gap-4">
                          <div>
                            <Label htmlFor={`min-${idx}`}>Minimum Velocity</Label>
                            <Input
                              id={`min-${idx}`}
                              type="number"
                              min={0}
                              ...
                              value={range.min}
                              onChange={e =>
                                updateVelocityRange(idx, 'min', parseInt(e.target.value) || 0)
                              }
                              className="mt-1"
                            />
                          </div>
                          <div>
                            <Label htmlFor={`max-${idx}`}>Maximum Velocity</Label>
                            <Input
                              id={`max-${idx}`}
                              type="number"
                              min={0}
                              ...
                              value={range.max}
                              onChange={e =>
                                updateVelocityRange(idx, 'max', parseInt(e.target.value) || 0)
                              }
                              className="mt-1"
                            />
                          </div>
                  ...
                  {hasChanges && (
                    <div className="bg-yellow-50 border border-yellow-200 rounded p-3 text-sm text-yellow-800">
                      You have unsaved changes. Click "Download Configuration" to export your
                      settings.
                    </div>
                  )}
                </div>
          ...
          <CardContent>
            <div className="grid md:grid-cols-3 gap-4 text-sm">
              <div>
                ...
                <p className="text-gray-600">
                  High register notes (C3, D4, F4, G4, G#4, and C5-C6) have softer dynamics and use
                  only 3 velocity layers.
                </p>
              </div>
              <div>
                <h4 className="font-semibold mb-2">4-Layer Notes</h4>
                <p className="text-gray-600">
                  Mid-range notes with moderate dynamic range. Most common in the middle octaves.
                </p>
              </div>
              <div>
                <h4 className="font-semibold mb-2">5-Layer Notes</h4>
                <p className="text-gray-600">
                  Low register notes with the fullest dynamic range, capturing subtle tonal
                  variations.
                </p>
              </div>
            </div>
