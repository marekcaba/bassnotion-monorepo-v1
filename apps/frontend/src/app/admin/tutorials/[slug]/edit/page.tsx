...
const DEFAULT_TUTORIAL_TEMPLATE = {
  ...
  description: 'Learn this amazing bass line step by step. Perfect for intermediate players looking to improve their groove and timing.',
  ...
  coreConcept: {
    description: 'Master the fundamentals of groove and timing through focused practice techniques.',
    bulletPoints: [
...
export default function AdminTutorialEditPage({ params }: AdminTutorialPageProps) {
  ...
  const exerciseTempMidiPaths = useRef(new WeakMap<Exercise, {
  ...
  useEffect(() => {
    if (!isProfileLoading && bassStringCount !== undefined && bassMaxFrets !== undefined) {
      logger.info('Loading bass settings from profile', { stringCount: bassStringCount, maxFrets: bassMaxFrets });
  ...
  const loadTutorial = async () => {
    try {
      ...
      const result = await tutorialRepo.findBySlugWithExercises(TutorialSlug.create(tutorialSlug));
      if (result.ok) {
        const { tutorial: loadedTutorial, exercises: loadedExercisesList } = result.value;
        setTutorial(loadedTutorial);

        // Load Core Concept data from tutorial
        if (loadedTutorial.coreConceptDescription || loadedTutorial.coreConceptPoints?.length) {
          setCoreConcept({
        ...
        if (loadedTutorial.teachingTakeaway) {
          setTeachingTakeaway({
            ...
            coreLearningTitle: loadedTutorial.teachingTakeaway.coreLearningTitle || '',
            coreLearningDescription: loadedTutorial.teachingTakeaway.coreLearningDescription || '',
            keyConceptsCount: loadedTutorial.teachingTakeaway.keyConceptsCount || 0,
            duration: loadedTutorial.teachingTakeaway.duration || 0,
            level: loadedTutorial.teachingTakeaway.level || '',
            masterySectionTitle: loadedTutorial.teachingTakeaway.masterySectionTitle || '',
            points: loadedTutorial.teachingTakeaway.points || [],
            practiceTipTitle: loadedTutorial.teachingTakeaway.practiceTipTitle || '',
            practiceTipDescription: loadedTutorial.teachingTakeaway.practiceTipDescription || '',
          });
        }
  ...
  const handleSave = useCallback(async (isAutoSave = false) => {
    ...
    try {
      ...
      if (deletedExercises.length > 0) {
        logger.info('Batch deleting exercises', { count: deletedExercises.length });
      ...
      const result = await tutorialRepo.saveWithExercises(updatedTutorial, exercisesForSave, exerciseTempMidiPaths);
      ...
      if (errorMessage.includes('duplicate key value violates unique constraint "tutorials_slug_key"')) {
        toast({
          title: 'Duplicate Tutorial Slug',
          description: 'A tutorial with this slug already exists. Please change the tutorial slug to a unique value.',
          variant: 'destructive',
        });
      } else if (errorMessage.includes('duplicate key')) {
        toast({
          title: 'Duplicate Entry',
          description: 'This value already exists in the database. Please use a unique value.',
          variant: 'destructive',
        });
      } else if (errorMessage.includes('Internal server error')) {
        toast({
          title: 'Server Error',
          description: 'An error occurred on the server. Please check the console for details and try again.',
          variant: 'destructive',
        });
      } else {
        toast({
          title: 'Save Failed',
          description: errorMessage || 'Failed to save tutorial. Please try again.',
          variant: 'destructive',
        });
      }
    } finally {
  ...
  }, [tutorial, exercises, hasChanges, coreConcept, teachingTakeaway, loadedExercises, queryClient, tutorialSlug]);
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 flex items-center justify-center">
        <div className="text-center text-white">
          ...
          <Button onClick={() => router.push('/admin/tutorials')} variant="secondary">
            Back to Tutorials
          </Button>
        </div>
      </div>
  ...
  const tutorialData = {
    ...
    youtube_url: tutorial.youtubeId ? `https://www.youtube.com/watch?v=${tutorial.youtubeId}` : '',
    ...
    thumbnail_url: tutorial.youtubeId ? `https://img.youtube.com/vi/${tutorial.youtubeId}/maxresdefault.jpg` : tutorial.thumbnailUrl,
  ...
  return (
    <>
      ...
      {/* Edit Modal */}
      {editMode && (
        <div className="fixed inset-0 bg-black/50 z-40" onClick={() => toggleEditMode(null)}>
          <div className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-white rounded-lg p-6 max-w-2xl w-full max-h-[80vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-xl font-bold">Edit {editMode}</h3>
              <Button onClick={() => toggleEditMode(null)} variant="ghost" size="sm">
                <X className="w-4 h-4" />
              </Button>
            </div>
            ...
            {editMode === 'core-concept' && (
              <div className="space-y-4">
                <div>
                  <label className="block text-sm font-medium mb-1">Core Concept Description</label>
                  <textarea
                    value={coreConcept.description}
                    onChange={(e) => {
                      setCoreConcept({ ...coreConcept, description: e.target.value });
                      setHasChanges(true);
                    }}
                    rows={3}
                ...
                <div>
                    ...
                    <div key={index} className="flex gap-2 mb-2">
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          setCoreConcept({ ...coreConcept, bulletPoints: newPoints });
                          setHasChanges(true);
                        }}
                      ...
                      <Button
                        onClick={() => {
                          const newPoints = coreConcept.bulletPoints.filter((_, i) => i !== index);
                          setCoreConcept({ ...coreConcept, bulletPoints: newPoints });
                          setHasChanges(true);
                        }}
                        size="sm"
                  ...
                  <Button
                    onClick={() => {
                      setCoreConcept({
                        ...coreConcept,
                        bulletPoints: [...coreConcept.bulletPoints, 'New point'],
                      });
                      setHasChanges(true);
                    }}
                    size="sm"
                ...
                <Button onClick={() => toggleEditMode(null)} className="w-full">
              ...
              <div className="space-y-4 max-h-[70vh] overflow-y-auto">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      setTeachingTakeaway({ ...teachingTakeaway, title: e.target.value });
                      setHasChanges(true);
                    }}
                    className="w-full px-3 py-2 border rounded-md"
                  />
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      setTeachingTakeaway({ ...teachingTakeaway, subtitle: e.target.value });
                      setHasChanges(true);
                    }}
                    className="w-full px-3 py-2 border rounded-md"
                  />
                ...
                <div>
                  <label className="block text-sm font-medium mb-1">Core Learning Section Title</label>
                  <input
                    type="text"
                    value={teachingTakeaway.coreLearningTitle}
                    onChange={(e) => {
                      setTeachingTakeaway({ ...teachingTakeaway, coreLearningTitle: e.target.value });
                      setHasChanges(true);
                    }}
                    className="w-full px-3 py-2 border rounded-md"
                  />
                ...
                <div>
                  <label className="block text-sm font-medium mb-1">Core Learning Description</label>
                  <textarea
                    value={teachingTakeaway.coreLearningDescription}
                    onChange={(e) => {
                      setTeachingTakeaway({ ...teachingTakeaway, coreLearningDescription: e.target.value });
                      setHasChanges(true);
                    }}
                    className="w-full px-3 py-2 border rounded-md"
                    rows={3}
                ...
                <div className="grid grid-cols-3 gap-4">
                  <div>
                    <label className="block text-sm font-medium mb-1">Key Concepts Count</label>
                    <input
                      type="number"
                      value={teachingTakeaway.keyConceptsCount}
                      onChange={(e) => {
                        setTeachingTakeaway({ ...teachingTakeaway, keyConceptsCount: parseInt(e.target.value) || 0 });
                        setHasChanges(true);
                      }}
                      className="w-full px-3 py-2 border rounded-md"
                    />
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        setTeachingTakeaway({ ...teachingTakeaway, duration: parseInt(e.target.value) || 0 });
                        setHasChanges(true);
                      }}
                      className="w-full px-3 py-2 border rounded-md"
                    />
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        setTeachingTakeaway({ ...teachingTakeaway, level: e.target.value });
                        setHasChanges(true);
                      }}
                      className="w-full px-3 py-2 border rounded-md"
                    />
                ...
                <div>
                  <label className="block text-sm font-medium mb-1">Mastery Section Title</label>
                  <input
                    type="text"
                    value={teachingTakeaway.masterySectionTitle}
                    onChange={(e) => {
                      setTeachingTakeaway({ ...teachingTakeaway, masterySectionTitle: e.target.value });
                      setHasChanges(true);
                    }}
                    className="w-full px-3 py-2 border rounded-md"
                  />
                ...
                <div>
                  <label className="block text-sm font-medium mb-1">Learning Points</label>
                    ...
                    <div key={index} className="flex gap-2 mb-2">
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          newPoints[index] = e.target.value;
                          setTeachingTakeaway({ ...teachingTakeaway, points: newPoints });
                          setHasChanges(true);
                        }}
                        className="flex-1 px-3 py-2 border rounded-md"
                      />
                      <Button
                        onClick={() => {
                          const newPoints = teachingTakeaway.points.filter((_, i) => i !== index);
                          setTeachingTakeaway({ ...teachingTakeaway, points: newPoints });
                          setHasChanges(true);
                        }}
                        size="sm"
                  ...
                  <Button
                    onClick={() => {
                      setTeachingTakeaway({
                        ...teachingTakeaway,
                        points: [...teachingTakeaway.points, 'New learning point'],
                      });
                      setHasChanges(true);
                    }}
                    size="sm"
                ...
                <div>
                  <label className="block text-sm font-medium mb-1">Practice Tip Title</label>
                  <input
                    type="text"
                    value={teachingTakeaway.practiceTipTitle}
                    onChange={(e) => {
                      setTeachingTakeaway({ ...teachingTakeaway, practiceTipTitle: e.target.value });
                      setHasChanges(true);
                    }}
                    className="w-full px-3 py-2 border rounded-md"
                  />
                ...
                <div>
                  <label className="block text-sm font-medium mb-1">Practice Tip Description</label>
                  <textarea
                    value={teachingTakeaway.practiceTipDescription}
                    onChange={(e) => {
                      setTeachingTakeaway({ ...teachingTakeaway, practiceTipDescription: e.target.value });
                      setHasChanges(true);
                    }}
                    className="w-full px-3 py-2 border rounded-md"
                    rows={3}
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2 border rounded-md"
                  />
                  <p className="text-xs text-gray-500 mt-1">YouTube channel name</p>
                </div>
                <div>
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2 border rounded-md"
                  />
                  <p className="text-xs text-gray-500 mt-1">Leave empty to auto-fetch from YouTube</p>
                </div>
                <Button
                  variant="outline"
                  size="sm"
                  onClick={async () => {
                    try {
                      ...
                      const response = await fetch('/api/v1/tutorials/fetch-youtube-channel-info', {
                        method: 'POST',
                        headers: {
                      ...
                      if (!response.ok) {
                        ...
                        throw new Error(error.message || 'Failed to fetch channel info');
                      }
                      ...
                      alert(`Successfully fetched channel info for: ${channelData.creatorName}`);
                    } catch (error) {
                      logger.error('Error fetching YouTube channel info', error);
                      alert(`Failed to fetch channel info: ${error.message}`);
                    }
                  }}
                  className="w-full"
                >
            ...
            {editMode === 'youtube-video' && (
              <div className="space-y-4">
                <div>
                  <label className="block text-sm font-medium mb-1">YouTube Video</label>
                  <input
                    type="text"
                    ...
                    onBlur={(e) => {
                      ...
                      const thumbnailUrl = videoId ? `https://img.youtube.com/vi/${videoId}/maxresdefault.jpg` : tutorial.thumbnailUrl;
                ...
                {tutorial.youtubeId && tutorial.youtubeId.length >= 11 && (
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <div className="text-xs text-gray-600">
                        ...
                        <p>Thumbnail URL: https://img.youtube.com/vi/{tutorial.youtubeId}/maxresdefault.jpg</p>
                      </div>
                    </div>
                  </div>
                )}
          ...
          <div className="space-y-4">
            ...
            <div className="flex justify-between items-center gap-3">
              ...
              <div className="flex items-center gap-3">
                ...
                {lastSavedAt && autoSaveStatus === 'idle' && !hasChanges && (
                  ...
                  <>
                    <Button
                      onClick={() => {
                        ...
                        sessionStorage.setItem('previewingFromEdit', tutorial.slug.value);
            ...
            <EditableSection title="YouTube Video" onEdit={() => toggleEditMode('youtube-video')}>
            ...
            <EditableSection title="Exercises" onEdit={() => toggleEditMode('exercises')}>
              <ExerciseSelector
            ...
            {/* Global Controls */}
            <GlobalControlsCard
              selectedExercise={formattedExercises.find(e => e.id === selectedExerciseId)}
              exercises={formattedExercises}
            />
            ...
            {/* Teaching Takeaway */}
            <EditableSection title="Teaching Takeaway" onEdit={() => toggleEditMode('teaching-takeaway')}>
          ...
          onSave={(exerciseData) => {
            if (editingExercise) {
              ...
              exerciseTempMidiPaths.set(updatedExercise, {
                temp_bassline_midi_path: exerciseData.temp_bassline_midi_path,
                temp_drummer_midi_path: exerciseData.temp_drummer_midi_path,
                temp_harmony_midi_path: exerciseData.temp_harmony_midi_path,
                temp_metronome_midi_path: exerciseData.temp_metronome_midi_path,
              });
