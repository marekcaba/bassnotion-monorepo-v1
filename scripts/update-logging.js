...
const logLevelPatterns = [
  ...
  { pattern: /console\.log.*(?:timing|position|frame|drift)/i, level: 'verbose' },
...
function updateFile(filepath) {
  ...
  if (isTypeScript && hasLogger && !content.includes('private logger =')) {
    ...
    if (classMatch) {
      ...
      content = content.slice(0, insertPosition) + loggerDeclaration + content.slice(insertPosition);
    }
  }
  ...
  if (isTypeScript) {
    content = content.replace(/(['"`])[\u{1F300}-\u{1F9FF}][\u{20}-\u{7E}]*/gu, '$1');
  }
