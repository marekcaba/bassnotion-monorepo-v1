...
if (!fs.existsSync(METADATA_FILE)) {
  console.error(
    '❌ No metadata found. Run download-all-hydrogen-kits.js first',
  );
  process.exit(1);
}
  ...
  for (const file of files) {
    ...
    try {
      ...
      process.stdout.write(
        `\r  ✓ Uploaded ${uploadedCount}/${files.length} samples`,
      );
    } catch (error) {
...
async function createCategoryIndex(category, kits) {
  ...
  try {
    ...
    console.error(
      `❌ Failed to create category index for ${category}:`,
      error.message,
    );
  }
}
...
async function main() {
  ...
  console.log(
    `💾 Total size: ${(metadata.totalSize / 1024 / 1024).toFixed(1)} MB\n`,
  );
