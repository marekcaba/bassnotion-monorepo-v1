...
async function main() {
  ...
  for (let i = 0; i < wavFiles.length; i++) {
    ...
    try {
      ...
      if (converted) {
        ...
        console.log(
          `✓ (${wavSize}MB → ${mp3Size}MB, saved ${savings.toFixed(2)}MB)`,
        );
        successCount++;
      } else {
  ...
  console.log(
    `Average compression: ${(((totalSaved / successCount) * 100) / (totalSaved / successCount + 1)).toFixed(0)}%`,
  );
  ...
  console.log(
    '2. Consider keeping WAV files as backup or for high-quality exports',
  );
