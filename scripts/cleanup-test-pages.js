...
// Directories to keep
const KEEP_DIRS = ['page.tsx', 'metadata.ts', '__tests__', 'layout.tsx'];
...
async function analyzeTestPages() {
  ...
  try {
    ...
    for (const item of libraryItems) {
      ...
      if (
        stats.isDirectory() ||
        (stats.isFile() && !KEEP_DIRS.includes(item))
      ) {
        ...
        if (
          isTest ||
          item.endsWith('.backup.tsx') ||
          item.endsWith('.simple.tsx')
        ) {
          testDirs.push(item);
    ...
    console.log(
      `📊 Found ${testDirs.length} test variations in library/[tutorialId]:`,
    );
    console.log(
      'Test directories to remove:',
      testDirs.slice(0, 10).join(', '),
...
async function createBackup() {
  ...
  const backupPath = path.join(
    __dirname,
    '..',
    `test-pages-backup-${timestamp}.json`,
  );
...
async function cleanupTestPages(dryRun = true) {
  ...
  if (dryRun) {
    ...
    console.log(
      'To actually delete files, run: node scripts/cleanup-test-pages.js --execute',
    );
    return;
  }
