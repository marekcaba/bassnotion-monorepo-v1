...
const PATTERN_RULES = [
  ...
  { type: DRUM_TYPES.HIHAT_CLOSED, patterns: [
    /closed[\s-_]*h(i)?[\s-_]*hat/i,
    /h(i)?[\s-_]*hat[\s-_]*closed/i,
    /hhc/i, /chh/i, /c[\s-_]*hh/i,
    /hihat(?!.*open)(?!.*pedal)/i  // hihat but not open or pedal
  ]},
  { type: DRUM_TYPES.HIHAT_OPEN, patterns: [
    /open[\s-_]*h(i)?[\s-_]*hat/i,
    /h(i)?[\s-_]*hat[\s-_]*open/i,
    /hho/i, /ohh/i, /o[\s-_]*hh/i
  ]},
  { type: DRUM_TYPES.HIHAT_PEDAL, patterns: [
    /pedal/i, /foot[\s-_]*hat/i, /hhp/i
  ]},
  
  // Kick patterns
  { type: DRUM_TYPES.KICK, patterns: [
    /kick/i, /bass[\s-_]*drum/i, /bassdrum/i,
    /\bbd\b/i, /\bb\.?d\.?\b/i, /kik/i, /\bkk\b/i
  ]},
  
  // Snare patterns
  { type: DRUM_TYPES.SNARE, patterns: [
    /snare/i, /\bsn\b/i, /\bsd\b/i, /snr/i
  ]},
  
  // Tom patterns
  { type: DRUM_TYPES.TOM_HIGH, patterns: [
    /high[\s-_]*tom/i, /tom[\s-_]*high/i, /tom[\s-_]*1/i,
    /\bht\b/i, /\bt1\b/i
  ]},
  { type: DRUM_TYPES.TOM_MID, patterns: [
    /mid[\s-_]*tom/i, /tom[\s-_]*mid/i, /tom[\s-_]*2/i,
    /\bmt\b/i, /\bt2\b/i
  ]},
  { type: DRUM_TYPES.TOM_LOW, patterns: [
    /low[\s-_]*tom/i, /tom[\s-_]*low/i, /tom[\s-_]*3/i,
    /\blt\b/i, /\bt3\b/i
  ]},
  { type: DRUM_TYPES.TOM_FLOOR, patterns: [
    /floor[\s-_]*tom/i, /\bft\b/i
  ]},
...
const KIT_CONFIGS = {
  ...
  'classic-808': {
    name: 'Classic TR-808',
    style: 'electronic',
    expectedDrums: ['kick', 'snare', 'hihat-closed', 'hihat-open', 'clap', 'cowbell']
  },
  'tr808909': {
    name: 'TR-808/909 Hybrid',
    style: 'electronic',
    expectedDrums: ['kick', 'snare', 'hihat-closed', 'hihat-open', 'crash', 'ride']
  },
  'boss-dr110': {
    name: 'Boss DR-110',
    style: 'electronic',
  ...
  'yamaha-vintage': {
    ...
    style: 'acoustic',
    expectedDrums: ['kick', 'snare', 'hihat-closed', 'hihat-open', 'tom-high', 'tom-low', 'crash', 'ride']
  },
  'colombo-acoustic': {
    name: 'Colombo Acoustic',
    style: 'acoustic',
    expectedDrums: ['kick', 'snare', 'hihat-closed', 'hihat-open', 'tom-high', 'tom-mid', 'tom-low', 'crash', 'ride']
  },
  ...
  'dave-grohl': {
    name: 'Dave Grohl Kit',
    style: 'rock',
    expectedDrums: ['kick', 'snare', 'hihat-closed', 'hihat-open', 'tom-high', 'tom-mid', 'tom-low', 'crash', 'crash-2', 'ride']
  },
...
async function main() {
  ...
  const categories = ['acoustic', 'electronic', 'hip-hop', 'rock', 'metal', 'jazz'];
  const allManifests = [];
  ...
  for (const category of categories) {
    ...
    for (const kit of kits) {
      if (kit.id) { // It's a directory
        const kitId = kit.name;
        const files = await listFiles(`drums/hydrogen-kits/mp3/${category}/${kitId}`);
        ...
        if (uploadError) {
          console.error(`    ❌ Failed to upload manifest:`, uploadError.message);
        } else {
          console.log(`    ✓ Manifest uploaded (${manifest.statistics.mappedSamples}/${manifest.statistics.totalSamples} mapped)`);
        }
      }
    }
  }
  ...
  const avgConfidence = allManifests.reduce((sum, m) => sum + m.statistics.confidence, 0) / allManifests.length;
  console.log(`Average mapping confidence: ${(avgConfidence * 100).toFixed(1)}%`);
  
  const totalSamples = allManifests.reduce((sum, m) => sum + m.statistics.totalSamples, 0);
  const totalMapped = allManifests.reduce((sum, m) => sum + m.statistics.mappedSamples, 0);
  console.log(`Total samples: ${totalSamples}`);
  console.log(`Mapped samples: ${totalMapped} (${(totalMapped / totalSamples * 100).toFixed(1)}%)`);
