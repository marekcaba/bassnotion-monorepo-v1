...
async function createBackup(): Promise<string> {
  ...
  const backupFile = path.join(BACKUP_DIR, `exercises-backup-${timestamp}.json`);
...
async function analyzeExercises() {
  ...
  for (const exercise of exercises) {
    try {
      const isAlreadyMusical = ExerciseMigration.isMusicalTimingExercise(exercise);
      if (isAlreadyMusical) {
        alreadyMigrated++;
      } else {
        // Check if it has valid legacy timing data
        if (exercise.notes && Array.isArray(exercise.notes) && exercise.notes.length > 0) {
          const hasValidTiming = exercise.notes.every((note: any) => 
          ...
          hasInvalidData++;
          log(`⚠️ Exercise ${exercise.id} has no notes or invalid notes array`, 'warn');
        }
      }
    } catch (error) {
      hasInvalidData++;
...
async function performMigration(dryRun: boolean = false): Promise<void> {
  ...
  for (const exercise of exercises) {
    results.processed++;
    log(`Processing exercise ${results.processed}/${exercises.length}: ${exercise.id}`);
    ...
    if (!dryRun) {
      ...
      if (updateError) {
        log(`❌ Failed to update exercise ${exercise.id}: ${updateError.message}`, 'error');
        results.failed++;
        continue;
      }
    }
...
async function validateMigration(): Promise<boolean> {
  ...
  if (error) {
    throw new Error(`Failed to fetch exercises for validation: ${error.message}`);
  }
  ...
  for (const exercise of exercises) {
    try {
      if (ExerciseMigration.isMusicalTimingExercise(exercise)) {
        ...
        if (exercise.notes && exercise.notes.every((note: any) => 
          typeof note.timestamp === 'number' && typeof note.duration === 'number'
        )) {
          validCount++;
        } else {
          invalidCount++;
          log(`❌ Exercise ${exercise.id} has invalid format after migration`, 'error');
        }
      }
    } catch (error) {
      invalidCount++;
...
async function rollbackMigration(backupFile: string): Promise<void> {
  ...
  for (const exercise of backupData) {
    ...
    if (error) {
      log(`❌ Failed to restore exercise ${exercise.id}: ${error.message}`, 'error');
      failed++;
    } else {
      restored++;
    }
...
async function main() {
  ...
  try {
    switch (command) {
      ...
      default:
        console.log('Usage: npm run migrate [analyze|dry-run|migrate|validate|rollback <backup-file>]');
        console.log('');
