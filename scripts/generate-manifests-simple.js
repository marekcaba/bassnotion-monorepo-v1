...
const PATTERNS = {
  ...
  'hihat-closed': /closed.*h(i.?)?hat|h(i.?)?hat.*closed|hhc|chh|c.?hh|hat.*cl/i,
...
async function analyzeKit(category, kitId) {
  ...
  const manifest = {
    ...
    stats: {
      ...
      coverage: ((audioFiles.length - unmapped.length) / audioFiles.length * 100).toFixed(1) + '%'
    }
  };
  
  console.log(`  ✓ Mapped ${manifest.stats.mapped}/${manifest.stats.total} samples (${manifest.stats.coverage})`);
  
  if (unmapped.length > 0) {
    console.log(`  ⚠️  Unmapped: ${unmapped.slice(0, 5).join(', ')}${unmapped.length > 5 ? '...' : ''}`);
  }
...
async function main() {
  ...
  const categories = ['electronic', 'acoustic', 'hip-hop', 'rock', 'metal', 'jazz'];
  const allManifests = [];
  ...
  const masterIndex = {
    ...
    stats: {
      ...
      avgCoverage: (allManifests.reduce((sum, m) => sum + parseFloat(m.stats.coverage), 0) / allManifests.length).toFixed(1) + '%'
    }
  };
  ...
  console.log("const manifest = await fetch('...drums/hydrogen-kits/mp3/electronic/classic-808/manifest.json')");
  console.log('');
  console.log('// Get kick samples (whatever they\'re named)');
  console.log('const kicks = manifest.mapping.kick;');
  console.log('// Plays: bd-01.mp3, bassdrum.mp3, or whatever the file is named');
  console.log('```');
}
