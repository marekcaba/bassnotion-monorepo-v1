...
// Kit metadata
const KIT_METADATA = {
  'Classic-808': { id: 'classic-808', category: 'electronic', name: 'Classic TR-808' },
  'TR808909': { id: 'tr808909', category: 'electronic', name: 'TR-808/909 Hybrid' },
  'Boss_DR-110': { id: 'boss-dr110', category: 'electronic', name: 'Boss DR-110' },
  'YamahaVintageKit': { id: 'yamaha-vintage', category: 'acoustic', name: 'Yamaha Vintage Kit' },
  'HipHop-1': { id: 'hip-hop-1', category: 'hip-hop', name: 'Hip-Hop Kit 1' },
  'HipHop-2': { id: 'hip-hop-2', category: 'hip-hop', name: 'Hip-Hop Kit 2' },
  'ElectricEmpireKit': { id: 'electric-empire', category: 'electronic', name: 'Electric Empire' },
  'K-27_Trash_Kit': { id: 'k27-trash', category: 'electronic', name: 'K-27 Trash Kit' },
  ...
  'ColomboAcousticDrumkit': { id: 'colombo-acoustic', category: 'acoustic', name: 'Colombo Acoustic' },
  'Millo_MultiLayered3': { id: 'millo-multilayered', category: 'acoustic', name: 'Millo Multi-Layered' },
  'Millo-Drums_v.1': { id: 'millo-drums', category: 'acoustic', name: 'Millo Drums' },
  'Gimme A Hand 1.0': { id: 'gimme-jazz', category: 'jazz', name: 'Gimme That Jazz' },
  'ForzeeStereo': { id: 'forzee-stereo', category: 'acoustic', name: 'Forzee Stereo Kit' },
  'Dave Grohl': { id: 'dave-grohl', category: 'rock', name: 'Dave Grohl Kit' },
  'John Bonham': { id: 'john-bonham', category: 'rock', name: 'John Bonham Kit' },
  'BeatBuddy_Kit': { id: 'beatbuddy', category: 'hip-hop', name: 'BeatBuddy Kit' }
};
...
async function processSamples(kitName, extractPath) {
  ...
  const subdirs = fs.readdirSync(extractPath).filter(item => 
  ...
  const audioFiles = fs.readdirSync(kitDir)
...
async function main() {
  ...
  try {
    const { error } = await supabase.storage
      .from('audio-samples')
      .upload('drums/hydrogen-collection/index.json', JSON.stringify(masterIndex, null, 2), {
