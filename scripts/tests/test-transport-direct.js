...
async function testTransportScheduling() {
  ...
  const scheduleId = Tone.Transport.scheduleRepeat(
    (time) => {
      ...
      console.log(
        `Callback ${callbackCount}: time=${time.toFixed(3)}, position=${Tone.Transport.position}`,
      );
    },
    '8n',
    0,
  ...
  console.log(
    'Expected callbacks (3 seconds at 120 BPM, 8n):',
    Math.floor(3 * 2 * 2),
  ); // ~12

  if (callbackCount <= 1) {
    console.log(
      '\n❌ ISSUE REPRODUCED: Only got',
      callbackCount,
      'callback(s)',
    );
  ...
  const scheduleId2 = Tone.Transport.scheduleRepeat(
    (time) => {
      callbackCount++;
      console.log(
        `Immediate start - Callback ${callbackCount}: time=${time.toFixed(3)}`,
      );
    },
    '8n',
    0,
