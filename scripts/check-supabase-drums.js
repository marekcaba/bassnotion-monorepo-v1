...
async function checkDrumSamples() {
  try {
    ...
    if (hydrogenKits && hydrogenKits.length > 0) {
      ...
      if (mp3Kits && mp3Kits.length > 0) {
        ...
        if (rockFiles && rockFiles.length > 0) {
          ...
          const kicks = rockFiles.filter((f) =>
            f.name.toLowerCase().includes('kick'),
          );
          const snares = rockFiles.filter((f) =>
            f.name.toLowerCase().includes('snare'),
          );
          const hihats = rockFiles.filter(
            (f) =>
          ...
          if (groholFiles && groholFiles.length > 0) {
            ...
            const dgKicks = groholFiles.filter((f) =>
              f.name.toLowerCase().includes('kick'),
            );
            const dgSnares = groholFiles.filter((f) =>
              f.name.toLowerCase().includes('snare'),
            );
            const dgHihats = groholFiles.filter(
              (f) =>
                f.name.toLowerCase().includes('hh') ||
                f.name.toLowerCase().includes('hihat'),
            );
            ...
            console.log(
              `currentKitPath should be: 'drums/hydrogen-kits/mp3/rock/dave-grohl'`,
            );
            console.log('\nExample file URLs:');
            if (dgKicks[0]) {
              console.log(
                `Kick: ${process.env.SUPABASE_URL}/storage/v1/object/public/audio-samples/drums/hydrogen-kits/mp3/rock/dave-grohl/${dgKicks[0].name}`,
              );
            }
          }
        }
    ...
    if (files && files.length > 0) {
      ...
      // Group by drum type
      const kicks = sortedFiles.filter((f) =>
        f.name.toLowerCase().includes('kick'),
      );
      const snares = sortedFiles.filter((f) =>
        f.name.toLowerCase().includes('snare'),
      );
      const hihats = sortedFiles.filter(
        (f) =>
          f.name.toLowerCase().includes('hh') ||
          f.name.toLowerCase().includes('hihat'),
      );
      const others = sortedFiles.filter(
        (f) =>
          !f.name.toLowerCase().includes('kick') &&
          !f.name.toLowerCase().includes('snare') &&
          !f.name.toLowerCase().includes('hh') &&
      ...
      for (const fileName of checkFiles) {
        ...
        if (!exists) {
          ...
          if (similar.length > 0) {
            console.log(
              `  Similar files: ${similar.map((f) => f.name).join(', ')}`,
            );
          }
        }
      }
      ...
      console.log(
        `variations.kick = ['${kickFile}'${kicks[1] ? `, '${kicks[1].name}'` : ''}];`,
      );
      console.log(
        `variations.snare = ['${snareFile}'${snares[1] ? `, '${snares[1].name}'` : ''}];`,
      );
      console.log(
        `variations.hihat = ['${hihatFile}'${hihats[1] ? `, '${hihats[1].name}'` : ''}];`,
      );
    } else {
