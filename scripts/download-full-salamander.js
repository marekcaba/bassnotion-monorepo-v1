...
async function main() {
  ...
  console.log(
    'This will download the COMPLETE library with all velocity layers.',
  );
  ...
  console.log(
    `   MP3 files (128kbps): ~${(sampleCount * avgMp3Size).toFixed(0)}MB`,
  );
  console.log(
    `   Total (both): ~${(sampleCount * (avgFlacSize + avgMp3Size)).toFixed(0)}MB\n`,
  );
    ...
    if (match) {
      ...
      if (copiedCount % 50 === 0) {
        process.stdout.write(
          `\r   Copied ${copiedCount}/${flacFiles.length} files...`,
        );
      }
    }
  }
