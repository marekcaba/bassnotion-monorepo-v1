    ...
    for (const [drumType, patterns] of Object.entries(DRUM_PATTERNS)) {
      for (const pattern of patterns) {
        if (fileName.match(pattern)) {
          ...
          const velocityMatch = fileName.match(/v?(\d+)|soft|med|hard|low|high/i);
...
function main() {
  ...
  if (!fs.existsSync(KITS_DIR)) {
    console.error('❌ No drum kits found. Run process-and-upload-hydrogen.js first');
    process.exit(1);
  }
  
  // Get all kit directories
  const categories = fs.readdirSync(KITS_DIR, { withFileTypes: true })
  ...
  for (const category of categories) {
    ...
    const kits = fs.readdirSync(categoryPath, { withFileTypes: true })
      .filter(d => d.isDirectory());
    ...
    for (const kit of kits) {
      ...
      globalStats.mappedSamples += analysis.totalSamples - analysis.unmapped.length;
  ...
  console.log(`Mapped samples: ${globalStats.mappedSamples} (${(globalStats.mappedSamples / globalStats.totalSamples * 100).toFixed(1)}%)`);
  console.log(`Unmapped samples: ${globalStats.unmappedSamples} (${(globalStats.unmappedSamples / globalStats.totalSamples * 100).toFixed(1)}%)`);
  
  console.log('\n🥁 Drum Type Coverage:');
  const sortedCoverage = Object.entries(globalStats.drumTypeCoverage)
    .sort((a, b) => b[1] - a[1]);
  
  for (const [drumType, count] of sortedCoverage) {
    const percentage = (count / globalStats.totalKits * 100).toFixed(1);
    console.log(`  ${drumType}: ${count}/${globalStats.totalKits} kits (${percentage}%)`);
  }
  ...
  console.log('2. Common unmapped samples need pattern rules or manual mapping');
  console.log('3. Some kits use numeric naming (need special handling)');
  console.log('4. Consider fallback to generic "percussion" for unmapped samples');
}
