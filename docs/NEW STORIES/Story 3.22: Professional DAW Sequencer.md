...
export class PatternScheduler extends Service {
  ...
  async initialize(): Promise<void> {
    try {
      ...
      this.eventBus.on(
        ...
        this.handleTrackRegionsUpdate.bind(this),
      );
  ...
  private convertRegionToEvents(region: Region): SchedulableEvent[] {
    if (region.pattern) {
      return PatternConverter.patternToEvents(
        region.pattern,
        region.startPosition,
      );
    } else if (region.midiEvents) {
      return this.midiEventsToSchedulableEvents(
        region.midiEvents,
        region.startPosition,
      );
    }
...
export class PatternConverter {
  ...
  static patternToEvents(
    ...
    throw new Error(
      `Unknown pattern type: ${(pattern as any).constructor.name}`,
    );
  }
