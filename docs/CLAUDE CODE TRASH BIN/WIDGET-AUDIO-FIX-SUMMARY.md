...
const shouldPlay = loopRef.current && (isPlayingRef.current || isTransportStarted || currentTransportState);
```
...
const shouldPlay = metronomeLoopRef.current && isEnginePlay && transportState === 'started';
```
