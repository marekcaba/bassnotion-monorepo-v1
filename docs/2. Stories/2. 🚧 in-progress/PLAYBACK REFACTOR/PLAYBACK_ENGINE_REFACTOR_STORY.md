...
**Phase 1: Core Module Refactor** - ⏸️ **Not Started** (0/5 tasks)
...
**Overall Progress:** 7/17 tasks complete (41%) - **PHASE 0 COMPLETE! 🎉**
...
- [ ] **Task 1.1:** Implement `Scheduler.ts` - Unified Audio Scheduling (3 days)
    - [ ] Day 1: Create unified scheduling system
      - [ ] Implement instrument configuration system (data-driven, not class-based)
      - [ ] Implement `schedule()` method with configuration dispatch
      - [ ] Implement `scheduleRegion()` batch scheduling
      - [ ] Add source tracking array (`activeSources: AudioBufferSourceNode[]`)
    - [ ] Day 2: Implement cleanup and velocity layers
      - [ ] Inline velocity layer selection logic (from VelocityLayerSelector)
      - [ ] **PRESERVE Bug #3 Fix:** Port existing cleanup logic verbatim (LEAK ALREADY FIXED!)
        - Copy onended callback pattern from [HarmonyScheduler.ts:1151-1167](../../../apps/frontend/src/domains/playback/services/core/region-processing/scheduling/HarmonyScheduler.ts#L1151)
        - Copy onended callback pattern from [SimpleInstrumentScheduler.ts:242-245](../../../apps/frontend/src/domains/playback/services/core/region-processing/scheduling/SimpleInstrumentScheduler.ts#L242)
    ...
    - [ ] Day 3: Testing and validation
      - [ ] Write unit tests for each instrument type (metronome, drums, harmony, bass, voice cues)
      - [ ] Test source cleanup (memory leak prevention - compare against Task 0.7 baseline)
      - [ ] Test cancellation (tempo change support)
      - [ ] Test 1000+ event scheduling performance (<100ms)
      - [ ] Code review with team
    - [ ] **Acceptance Criteria:**
      - [ ] Single `schedule()` method handles all 5 instrument types
      - [ ] Instrument behavior controlled by configuration data (not separate classes)
      - [x] Memory leak is ALREADY FIXED in current code (verified in Task 0.7)
      - [ ] Source cleanup logic PRESERVED in new Scheduler (onended callbacks copied verbatim)
      - [ ] All existing scheduler tests pass with new implementation
      - [ ] No memory leaks in 100-iteration test (0MB growth - matches baseline)
      - [ ] Peak sources <50 during playback (matches Task 0.7 baseline)
      - [ ] 1000 sources clean up in <500ms (matches Task 0.7 baseline)
