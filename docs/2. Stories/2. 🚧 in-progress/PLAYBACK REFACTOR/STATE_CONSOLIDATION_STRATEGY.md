...
**New Code:**
```typescript
if (coreServices.initializationState === 'initialized' &&
...
// OR (more explicit):
if (['idle', 'ready', 'stopped', 'paused', 'error'].includes(playbackEngine.state)) {
  // not playing
}
```
...
export class RegionProcessorAdapter {
  ...
  updateTempo(newTempo: number): void {
    logDeprecationWarning('RegionProcessor.updateTempo()', 'PlaybackEngine.updateTempo()');
  ...
  registerTracks(tracks: Track[]): void {
    logDeprecationWarning('RegionProcessor.registerTracks()', 'PlaybackEngine.registerTracks()');
