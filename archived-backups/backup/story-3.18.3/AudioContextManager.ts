...
export class AudioContextManager {
  ...
  /**
   * Initialize AudioContext (can be called without user gesture)
   * The context will be created in suspended state for browser compliance
   */
  ...
  private async performInitialization(): Promise<void> {
    try {
      ...
      // Create AudioContext with DAW-optimized settings
      this.audioContext = new AudioContext({
        latencyHint: 'interactive', // Optimize for low latency
        sampleRate: 48000, // Professional DAW standard (48kHz)
      });
      ...
      // Don't try to resume here - let it stay suspended until user gesture
      // This is the proper DAW pattern: create suspended, resume on user action
      if (this.audioContext.state === 'suspended') {
        console.log(
          '[AudioContextManager] AudioContext created in suspended state (as expected)',
        );
      }
